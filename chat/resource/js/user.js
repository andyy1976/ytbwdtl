    ...
    $(function(){
		...
		if(user['u_id'] != '') {
			...
			web_info['msg_dialog'] = '<div id="new_msg_dialog" class="msg-windows"><div class="user-tab-bar"><ul class="user-list" id="user_list"></ul></div>'+
									...
									'<form id="msg_form"><textarea name="message" type="textarea" id="message" class="textarea" value="" onkeyup="send_keyup(event);" onfocus="send_focus();" ></textarea>'+
	...
	function msg_dialog_show(id){
		if (dialog_show == 0) {
			...
			function() {
			  $("#send_message").focus();
				var obj_msg = $("div[select_user_msg='"+user_show+"']");
				$("#msg_list").scrollTop(obj_msg.height());
			});
		} else {
			$("#send_message").focus();
		}
	...
	function send_msg(){//发消息
		// alert('11');
		// $("#send_message").val('22222');
		// alert($("#send_message").html());
		// alert($("#send_message").text());
		if(user_show < 1) {
	...
	function get_msg(list){//接收消息
		...
		for (var k in list){
			...
			if(dialog_show == 0 || $("li[select_u_id='"+u_id+"']").size()==0) {//没有打开对话窗口时计数
	...
	function update_dialog(){//显示会员的对话
		...
		for (var u_id in user_list){
			if ( user_list[u_id]['new_msg'] > 0 ) {
				update_user(u_id);
				$("em[unread_id='"+u_id+"']").addClass("unread");
				$("em[unread_id='"+u_id+"']").html(user_list[u_id]['new_msg']);
			}
		}
		select_user = $(".unread").first().attr("unread_id");
	...
	function show_chat_log(){
		...
		if (typeof chat_log['u_id'] === "undefined" || chat_log['u_id'] != user_show) {
			...
			$(".chat-log-btn").removeClass("off");
			$(".chat-log-btn").addClass("on");
			get_chat_log(7);
		} else {
			close_chat_log(user_show);
		}
	...
	function show_msg(u_id){//显示会员的消息
		...
		var u_name = user_info['u_name'];
		if($("div[select_user_msg='"+u_id+"']").size()==0) {
			$("#user_msg_list").prepend('<div class="msg_list" select_user_msg="'+u_id+'"></div>');
		}
		$(".msg_list").hide();
		$("div[select_user_msg='"+u_id+"']").show();
		$("li[select_u_id]").removeClass("select_user");
		$("li[select_u_id='"+u_id+"']").addClass("select_user");
		if(user_show != u_id) {
			close_chat_log(user_show);
			...
			$('#msg_list').perfectScrollbar('destroy');
			$('#msg_list').perfectScrollbar();
		}
	    if(layout == 'layout/seller_layout.php') {
		...
		var obj_msg = $("div[select_user_msg='"+u_id+"']");
		$("#msg_list").scrollTop(obj_msg.height());
		$("#send_message").focus();
	...
	function show_msg_goods(msg){
	    ...
	    if ( goods_id != '' && goods_id > 0 ) {
	        ...
	        $("div.msg_list div[m_id='"+m_id+"'] dd[class$='-msg-text']").append(text_append);
	    }
	}
	...
	function alert_user_msg(u_id){
		if ( user_list[u_id]['new_msg'] > 0 ) {
			...
			$("em[unread_id='"+u_id+"']").html("");
			$("em[unread_id='"+u_id+"']").removeClass("unread");
	...
	function close_chat_log(u_id){
		if (user_show == 0 || chat_log['u_id'] == u_id) {
			...
			$('#chat_log_msg').html('');
			$(".chat-log-btn").removeClass("on");
			$(".chat-log-btn").addClass("off");
			if ( connect === 1 ) $("#web_chat_dialog").show();
		}
	}
	function close_dialog(u_id){
		$("li[select_u_id='"+u_id+"']").remove();
		$("div[select_user_msg='"+u_id+"']").hide();
		if($("li[select_u_id]").size()==0) {
			msg_dialog_close('new_msg_dialog');
		} else {
			if ( user_show == u_id ) $("li[select_u_id]").first().trigger("click");
		}
		if ( user_show == u_id ) {
			user_show = 0;
			close_chat_log(u_id);
		}
		if($("li[select_u_id]").size() < 2) $(".user-tab-bar").hide();
	}
	...
	function update_recent(){
		...
		for (var u_id in recent_list){
			if(obj_recent.parent().find("dd[u_id='"+u_id+"']").size()==0) {
				if(user_list[u_id]['online'] > 0 ) {
					obj_recent.before('<dd u_id="'+u_id+'" title="最后对话:'+recent_list[u_id]['time']+'" ><span class="user-avatar"><img alt="'+user_list[u_id]['u_name']
						+'" src="'+user_list[u_id]['avatar']+'"><i class="online"></i></span><h5>'+user_list[u_id]['u_name']+'</h5><a href="javascript:void(0)" onclick="chat('+u_id+');"></a></dd>');
				} else {
					obj_recent.after('<dd u_id="'+u_id+'" title="最后对话:'+recent_list[u_id]['time']+'" ><span class="user-avatar"><img alt="'+user_list[u_id]['u_name']
						+'" src="'+user_list[u_id]['avatar']+'"><i class="offline"></i></span><h5>'+user_list[u_id]['u_name']+'</h5><a href="javascript:void(0)" onclick="chat('+u_id+');"></a></dd>');
				}
			}
		}
		obj_recent.remove();
	...
	function update_user(u_id){
		...
		if($("li[select_u_id='"+u_id+"']").size()==0) {
			...
			text_append += '</li>';
			$("#user_list").append(text_append);
			$("#user_list").sortable({ items: 'li' });
			$("li[select_u_id='"+u_id+"'] .ac-ico").bind("click", function() { close_dialog(u_id);return false; });
			if($("li[select_u_id]").size() > 1) $(".user-tab-bar").show();
		}
		$(".user-tab-bar").perfectScrollbar();
	}
	function getconnect(){
		// alert(connect_url+"/socket.io/socket.io.js");
		
		// $.getScript(connect_url+"/socket.io/socket.io.js", function(){
			
		// 	$("#web_chat_dialog").show();
		// 	$("#web_chat_dialog").append(web_info['msg_dialog']);
		...
		// 	  	if($("#new_msg_dialog").size()==0) $("#web_chat_dialog").append(web_info['msg_dialog']);
		// 	  	obj = $("#new_msg_dialog");
