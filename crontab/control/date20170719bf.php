    ...
    private function _member_stat(){
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            if ($orderid_list && count($orderid_list) > 0){
                //20170620潘丙福添加筛选字段order_goods.goods_points
                $field = ' orders.add_time,orders.buyer_id,orders.buyer_name,order_goods.goods_num,order_goods.goods_pay_points ';
                $where = array();
            ...
            foreach ((array)$update_arr as $k=>$v){
                foreach ($v as $m_k=>$m_v){
                    ...
                    $ordergoods_amount = $model->statByOrderGoods($where, 'SUM(order_goods.goods_num) as gnum,SUM(order_goods.goods_pay_points) as pointsamount');
                    ...
                    if ($statmember_info){
                        ...
                        $m_v['statm_pointsamount'] = floatval($ordergoods_amount[0]['pointsamount']);
                        ...
                        $tmp['statm_pointsamount'] = floatval($ordergoods_amount[0]['pointsamount']);
            ...
            foreach ((array)$memberid_list as $k=>$v){
                ...
                $tmp['statm_pointsamount'] = (($t = intval($ordergoods_list[$tmp['statm_memberid']]['pointsamount']))?$t:0);
