...
class dateControl extends BaseCronControl {
    ...
    public function indexOp() {
        ...
        //20170716潘丙福添加确认收货三天后商家收到订单的50%货款
        $this->_order_lasthalf_amount();

        //20171030潘丙福添加确认收货7天后商家钱包2收到100%货款
        $this->_order_whole_amount();

        //20170716潘丙福添加订单分销核算
        $this->_order_distribution();
        ...
        //20171102中奖发货7天后自动完成
        $this->_sweepstakesorder();

        //20171103云豆兑换发货7天后自动完成
        $this->_pointorder();
     ...
     *判断预售商品是否到期（全款）类型商品
     */
    private function presellGoods()
    {
        $goodsModel                            = Model()->table('goods');
        $time                                  = time();
        $condition                             = array();
        $condition['is_presell']               = array('eq', 1);
        $condition['presell_deliverdate']      = array('lt', "{$time}");

        $presellGoodsList                      = $goodsModel->field('goods_id,is_presell,goods_price,goods_points,old_presell_goods_price,old_presell_goods_points,presell_deliverdate')->where($condition)->select();
        foreach ($presellGoodsList as $key => $value) {
            $data                               = array();
            $data['goods_id']                   = $value['goods_id'];
            $data['is_presell']                 = 0;
            $data['goods_price']                = $value['old_presell_goods_price'];
            $data['goods_points']               = $value['old_presell_goods_points'];
            $data['old_presell_goods_price']    = 0;
            $data['old_presell_goods_points']   = 0;
     ...
     *判断预售商品是否到期（定金）类型商品
     */
    private function bookGoods()
    {
        $goodsModel                            = Model()->table('goods');
        $time                                  = time();
        $condition                             = array();
        $condition['is_book']                  = array('eq', 1);
        $goods_info['book_down_time']          = array('lt', "{$time}");

        $bookGoodsList                         = $goodsModel->field('goods_id')->where($condition)->select();
        foreach ($bookGoodsList as $key => $value) {
    ...
    private function _order_timeout_cancel() {
        ...
        $condition['add_time'] = array('lt',TIMESTAMP - ORDER_AUTO_CANCEL_TIME * 3600);
     ...
     * refund_state = 0
     * lock_state = 0
     * finnshed_time + 60*60*24*3 < time()
     * 3天
     */
    private function _order_lasthalf_amount()
    {
        $store = Model('seller');
        $rcb   = Model('rcb_log'); 
        //查询订单的条件
        $condition = array();
        $condition['platform_payment'] = array('eq',1);
        // $condition['order_state']      = array(array('eq',40),array('eq',50),'or');
        $condition['order_state']      = array('eq',40);
       // $condition['refund_state']     = array('eq',0);
        $condition['lock_state']       = array('eq',0);
        $condition['finnshed_time']    = array('elt', time()-259200);
        $doOrders = Model()->table('orders')->where($condition)->select();
        if (count($doOrders) == 0) {
            return ;
        }
        $updateArray = array();
        foreach ($doOrders as $key => $order_info) {
            ...
            $log['description']='会员确定收货三天后，商家获得订单的50%货款'.$order_info['order_sn'];        
            $insertId = $rcb->insert($log);
            if (! $insertId) {
                continue;
            }    
            $upstore=$store->where(array('store_id'=>$order_info['store_id']))->update($dat);
            $updateArray[] = $order_info['order_id'];
        }
        //更新订单的状态
        $updateCondition = array();
        $updateCondition['order_id'] = array('in',implode(',', $updateArray));
        Model()->table('orders')->where($updateCondition)->update(array('platform_payment' => '2'));
        echo "会员确定收货三天后，商家获得订单的50%货款<br />";
    }
     ...
     * refund_state = 0
     * lock_state = 0
     * finnshed_time + 60*60*24*7 < time()
     * 7天时间从钱包1转移到钱包2
     */
    private function _order_whole_amount()
    {
        $store = Model('seller');
        $rcb   = Model('rcb_log'); 
        //查询订单的条件
        $condition = array();
        $condition['platform_payment'] = array('eq',3);
        $condition['order_state']      = array('eq',40);
        $condition['refund_state']     = array('eq',0);
        $condition['lock_state']       = array('eq',0);
        $condition['finnshed_time']    = array('elt', time()-604800);
        $doOrders = Model()->table('orders')->where($condition)->select();
        if (count($doOrders) == 0) {
            return ;
        }
        $updateArray = array();
        foreach ($doOrders as $key => $order_info) {
            //因为已完成订单状态不能锁定，所以在退款退货列表查询是否有此订单
            $panReturnInfo = Model()->table('refund_return')->where(array('order_sn' => $order_info['order_sn']))->find();
            if ($panReturnInfo['seller_state'] < 3) {
                continue;
            }
            $dat['wallet_frozen']=array('exp','wallet_frozen-'.$order_info['order_amount']);
            $dat['wallet_release']=array('exp','wallet_release+'.$order_info['order_amount']);
            $log['member_id']=$order_info['store_id'];
            $log['member_name']=$order_info['store_name'];
            $log['type']='seller';
            $log['add_time']=time();
            $log['available_amount']='-'.$order_info['order_amount'];
            $log['freeze_amount']=$order_info['order_amount'];
            $log['description']='会员确定收货7天后，商家钱包2获得订单的100%货款'.$order_info['order_sn'];        
            $insertId = $rcb->insert($log);
            if (! $insertId) {
                continue;
            }    
        ...
        $updateCondition = array();
        $updateCondition['order_id'] = array('in',implode(',', $updateArray));
        Model()->table('orders')->where($updateCondition)->update(array('platform_payment' => '4'));
        echo "会员确定收货7天后，商家钱包2获得订单的100%货款<br />";
    } 

    //中奖发货7天后自动完成
    private function _sweepstakesorder()
    {
        //查询订单的条件
        $condition = array();
        $condition['order_state'] = array('eq',30);
        $condition['send_time'] = array('elt', time()-604800);
        $data['order_state'] = 40;
        Model()->table('sweepstakes_orders')->where($condition)->update($data);
    }

    //云豆兑换发货7天后自动完成
    private function _pointorder()
    {
        //查询订单的条件
        $condition = array();
        $condition['point_orderstate'] = array('eq',30);
        $condition['point_shippingtime'] = array('elt', time()-604800);
        $data['point_orderstate'] = 40;
        Model()->table('points_order')->where($condition)->update($data);
    }
     ...
     * refund_state = 0
     * lock_state = 0
     * finnshed_time + 60*60*24*15 < time()
     * 15天时间请参考trade.model.php
     */
    private function _order_distribution()
    {
        $model_order  = Model('order');
        $member_goods = model('goods');
        $percent      =Model('chief');
        $members      = Model('member');
        $pd_log       = Model('pd_log');
        //获取要处理的订单表
        $condition = array();
        $condition['is_distribution']  = array('eq',0);
        $condition['platform_payment'] = array('eq',2);
        $condition['order_state']      = array('eq',40);
        $condition['refund_state']     = array('eq',0);
        $condition['lock_state']       = array('eq',0);
        $condition['finnshed_time']    = array('elt', time()-1296000);
        $doOrders = Model()->table('orders')->where($condition)->select();
        if (count($doOrders) == 0) {
            return;
        }
        $updateArray = array();
        foreach ($doOrders as $key => $order_info) {
            $goods = $model_order->getOrderGoodsInfo(array('order_id'=>$order_info['order_id']));
            $order_money=$order_info['goods_amount'];
            $chiefs=$percent->getfby_id(1,'chief');
            $moneys="$order_money" * "$chiefs" ;                 
            $arr=get_parent_info($order_info['buyer_id']);
            //如果是会员资格产品就不给予购物分成
            if($goods['gc_id'] !=10351){                            
                if(is_array($arr)){
                    $buyer_puid=$arr['member_id'];
                    $buyer_pname=$arr['member_name'];                           
                    $updateResult = $members->where("member_id=$buyer_puid")->setInc('available_predeposit',$moneys);
                ...
                //每个人买东西都给上级提成代码结束
                //给代理分成
                give_chief($order_info['buyer_id'],$order_money);
            }
            $updateArray[] = $order_info['order_id'];
        }
        //更新订单状态
        $updateCondition = array();
        $updateCondition['order_id'] = array('in',implode(',', $updateArray));
        $model_order->table('orders')->where($updateCondition)->update(array('is_distribution' => '1'));
        echo "下级分销核算<br />";
    }
    ...
    private function _order_reciver_provinceid_update() {
        ...
        for($i = 0; $i < 500; $i++) {
            ...
            if (!empty($order_list)) {
                ...
                $city_ids  = trim(implode(',', $city_ids), ',');
                $area_list = $model_area->getAreaList(array('area_id'=>array('in',$city_ids)),'area_parent_id,area_id');
                if (!empty($area_list)) {
                    foreach ($area_list as $v) {
                    	//20170925潘丙福添加开始--收件人reciver_city_id为省id的处理办法
                    	if ($v['area_parent_id'] == 0) {
                    		$v['area_parent_id'] = $v['area_id'];
                    	}
                    	//20170925潘丙福添加结束
                        $update = $model_order->editOrderCommon(array('reciver_province_id'=>$v['area_parent_id']),array('reciver_city_id'=>$v['area_id'], 'reciver_province_id'=>0));
                        if (!$update) {
    ...
    private function _order_chain_timeout_cancel() {
        ...
        echo '<br/>';
        echo '门店自提订单、支付方式为门店支付的订单7天内未自提自动取消--成功';
    }
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            for ($i=0; $i<$countnum; $i+=100){//每次查询100条
                ...
                if ($orderlog_list){
                    ...
                    //查询订单数据
                    $field = 'order_id,order_sn,store_id,buyer_id,buyer_name,add_time,payment_code,order_amount,order_pointsamount,shipping_fee,evaluation_state,order_state,refund_state,refund_amount,order_from';
                    ...
                    //查询订单商品
                    //20170721潘丙福添加--添加查询字段(goods_pay_points)
                    $field = 'rec_id,order_id,goods_id,goods_name,goods_price,goods_num,goods_image,goods_pay_price,goods_pay_points,store_id,buyer_id,goods_type,promotions_id,commis_rate,gc_id';
                    ...
                    foreach ((array)$ordergoods_list as $k=>$v){
                        ...
                        $tmp['goods_pay_price'] = $v['goods_pay_price'];
                        //20170721潘丙福添加开始-添加goods_pay_points
                        $tmp['goods_pay_points'] = $v['goods_pay_points'];
                        //20170721潘丙福添加结束
                    ...
                    foreach ((array)$order_list as $k=>$v){
                        ...
                        $tmp['order_pointsamount'] = $v['order_pointsamount'];
    ...
    private function _member_stat(){
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            //统计订单下单量和下单金额
            //20170724潘丙福添加了orders表的字段orders.order_pointsamount(订单云豆)
            $field = ' orders.order_id,orders.add_time,orders.buyer_id,orders.buyer_name,orders.order_amount,orders.order_pointsamount,order_log.log_orderstate,orders.payment_code';
            $where = array();
            ...
            foreach ((array)$orderlist_tmp as $k=>$v){
                if (($v['payment_code']<>'offline' && $v['log_orderstate'] == ORDER_STATE_PAY) || ($v['payment_code'] == 'offline' && $v['log_orderstate'] == ORDER_STATE_SUCCESS)){
                    ...
                    if ($addtime != $stime){//订单如果隔天支付的话，需要进行统计数据更新
                        ...
                        //20170724潘丙福添加开始-会员每日消耗云豆统计
                        $order_list[$v['buyer_id']]['pointsamount'] = floatval($order_list[$v['buyer_id']]['pointsamount']) + (($t = floatval($v['order_pointsamount'])) > 0?$t:0);
                    }
                    //记录订单ID数组
            ...
            if ($orderid_list && count($orderid_list) > 0){
                ...
                foreach ((array)$ordergoods_tmp as $k=>$v){
                    ...
                    if ($addtime != $stime){//订单如果隔天支付的话，需要进行统计数据更新
                        ...
                        //获取订单商品积分
                        // $ordergoods_list[$v['buyer_id']]['pointsamount'] = $ordergoods_list[$v['buyer_id']]['pointsamount'] + (($t = floatval($v['goods_pay_points'])) > 0?$t:0);
                    }
                }
            }
        
            //统计的预存款记录
            //20170615潘丙福修改数据库引擎为mysql
            if (C('dbdriver') == 'mysql') {
            ...
            foreach ((array)$update_arr as $k=>$v){
                foreach ($v as $m_k=>$m_v){
                    ...
                    //潘丙福添加统计云豆消费字段
                    $orderlist_amount = $model->statByOrder($where, 'SUM(order_amount) as amount,COUNT(*) as num,SUM(order_pointsamount) as pointsamount', 0, 0, 'order_id');
        
                    //查询时间时间段内的下单商品件数
                    $where = array();
                     //20170620潘丙福添加开始
                    $where['orders.buyer_id'] = array('eq', $m_k);
                    //20170620潘丙福添加结束
                    $where['orders.order_state'] = array(array('neq',ORDER_STATE_NEW), array('neq',ORDER_STATE_CANCEL),'and');//去除未支付和已取消订单
                    ...
                    if ($statmember_info){
                        ...
                        //20170724潘丙福添加开始
                        $m_v['statm_pointsamount'] = floatval($orderlist_amount[0]['pointsamount']);
                        //20170724潘丙福添加结束
                        ...
                        $tmp['statm_pointsamount'] = floatval($orderlist_amount[0]['pointsamount']);
            ...
            //查询会员信息
            //20170724潘丙福添加开始-数组去重、添加两个字段member_id,member_name
            $memberid_arr = array_unique($memberid_arr);
            $memberid_list = Model('member')->getMemberList(array('member_id'=>array('in',$memberid_arr)), 'member_id,member_name', 0);
            ...
            foreach ((array)$memberid_list as $k=>$v){
                ...
                $tmp['statm_pointsamount'] = (($t = intval($order_list[$tmp['statm_memberid']]['pointsamount']))?$t:0);
            ...
            $panInsertArr = array_chunk($insert_arr, 2000);
            foreach ($panInsertArr as $pank => $panv) {
                $model->table('stat_member')->insertAll($panv);
            }
        }
        echo "会员相关数据<br />";
    }
