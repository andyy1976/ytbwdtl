...
if ($resCode=='000000'){
    ...
    // $contents = "\nresult\n" .print_r($result, true)
    //           . "\nsign\n" .print_r(base64_decode($sign), true)
    //           . "\nsign_result\n" .print_r($sign_result, true);
    // $filename = __DIR__. '/' . time() . '.notify.verify.txt';
    // file_put_contents($filename, $contents);

    //写入日志文件
    header("Content-type: text/html; charset=utf-8");
    $file  = '/home/wwwroot/lnmp01/domain/wdtl.com/web/logs/vbill_1_zf'.date('y-m-d',time()).'.log';
    $content='\r\n订单号：'.$orderNo.'|验签状态：'.$sign_result.'|时间:'.date('y-m-d h:i:s',time());
    ...
    if ( $sign_result == '1') {
        ...
        // file_put_contents(__DIR__ . '/' . time() . '.deData.txt', $de_data);
