...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listOp() {
       ...
       $condition = array();
      /* $bs = $model_order->table('orders')->where(array('is_dm'=>1,'order_state'=>20,'buyer_id'=>$this->member_info['member_id']))->update(array('order_state'=>50));*/
       ...
       $condition['buyer_id'] = $this->member_info['member_id'];
        ...
        if(empty($map_id)){
            $condition['is_dm'] = array('eq',0);  //是否是地面商家订单
        }else{
            $condition['is_dm'] = array('neq',0);
        }
        //20170828潘丙福修改--增加order_type=2(定金预售订单)
       $condition['order_type'] = array('in',array(1,2,3));
        if ($_POST['state_type'] != '') {
        ...
        foreach ($order_list_array as $value) {
            ...
            //20170828潘丙福添加开始--取得其它订单类型的信息（定金预售）
            $model_order->getOrderExtendInfo($value);
            //20170828潘丙福添加结束
        ...
        foreach ($order_group_list as $key => $value) {
            ...
            //20170828潘丙福添加开始-增加字段最外层is_bookstep字段
            foreach ($value['order_list'] as $pank1 => $panv1) {
                //判断订单是否为定金预售订单
                if ($panv1['order_type'] == 2) {
                    foreach ($panv1['book_list'] as $pank2 => $panv2) {
                        if ($panv2['book_step'] == '阶段1：付定金' && $panv2['book_pay_time'] == 0) {
                            //定金预售第一阶段
                            $value['is_bookstep']     = '1';
                            $value['bookstep_state']  = $panv2['book_state'];
                            $value['pay_amount']      = $panv2['book_amount'];
                            break;
                        } else if ($panv2['book_step'] == '阶段2：付尾款' && $panv2['book_pay_time'] == 0) {
                            //定金预售第二阶段
                            $value['is_bookstep']     = '2';
                            $value['bookstep_state']  = $panv2['book_state'];
                            $value['pay_amount']      = $panv2['book_amount'];
        ...
        $member_info=$model_member->where(array('member_id'=>$this->member_info['member_id']))->field('agreement_id,member_level')->find();
        output_data(array('order_group_list' => $new_order_group_list,'member_info'=>$member_info), mobile_page($page_count));
    }
...
//         $data['refund_state'] = $result['data']['order_info']['refund_state'];
     ...
     * 云豆兑换订单物流跟踪
     */
    public function pountorder_deliverOp(){
        $order_id   = intval($_POST['order_id']);
        if ($order_id <= 0) {
            output_error('订单不存在');
        }
        ...
        $order_info = Model()->table('points_order')->where($condition)->find();
        if (empty($order_info)) {
            output_error('订单不存在');
        }
        ...
        $e_code = $express[$order_info['extend_order_common']['shipping_express_id']]['e_code'];
        $e_name = $express[$order_info['extend_order_common']['shipping_express_id']]['e_name'];*/
        $name = Model()->table('express')->where(array('e_code'=>$order_info['point_shipping_ecode']))->field('e_name')->find();
        $e_name = $name['e_name'];
        $deliver_info = $this->_get_express($order_info['point_shipping_ecode'], $order_info['point_shippingcode']);
        output_data(array('express_name' => $e_name, 'shipping_code' => $order_info['point_shippingcode'], 'deliver_info' => $deliver_info));
    }
}
