...
class member_chatControl extends mobileMemberControl {
	...
	获取反馈信息
	*/
	public function get_feedbackOp(){
		$member_list = array();
		 $member_id = $this->member_info['member_id'];
		   $model_feedback=Model('mb_feedback');
		 /**获取用户返回的信息*/
		$condition['member_id']=$member_id;
		$member_feedback=$model_feedback->getMbFeedbackList($condition);
		$paramb = Model("mb_backfeedback");
		if(!empty($member_feedback)){
	      foreach($member_feedback as $key=>$v){
			  	$member_feedback[$key]['ftime']=date('Y-m-d H:i:s',$member_feedback[$key]['ftime']);
			$paramc=$paramb->getbackFeedbackByID($v['id']);
			if($paramc){
				$member_feedback[$key]['bcontent']=$paramc['content'];
				$member_feedback[$key]['bftime']=date('Y-m-d H:i:s',$paramc['ftime']);
				}
			}
		}
		 output_data(array('list' => $member_feedback));
	}
	...
	*删除消息
	*/
	public function delf_msgOp(){
		 $model_feedback = Model('mb_feedback');
		  $t_id = intval($_POST['t_id']);
		  $model_feedback ->delMbFeedback($t_id);
		   output_data(1);
		}
