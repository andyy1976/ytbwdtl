...
class member_sweepstakesorderControl extends mobileMemberControl{
	...
	public function orderlistOp() {
		...
		//20171018潘丙福添加开始--订单状态大于2
		$where['order_state'] = array('gt', 2);
		//20171018潘丙福添加结束
		$model_pointorder = Model('sweepstakesorder');

		$order_list = $model_pointorder->getPointOrderList($where, '*', 10, 0, 'id desc');
		...
		if (is_array($order_list) && count($order_list)>0){
			foreach ($order_list as $k => $v){
				$point_orderid = $v['id'];
				$id = Model()->table('sweepstakes_orderaddress')->where(array('point_orderid'=>$point_orderid))->find();
				if(!empty($id)){
					$order_list[$k]['point_orderid']='1';
				}else{
					$order_list[$k]['point_orderid']='0';
				}
			}
			foreach ($order_list as $k => $v){
				$v['state_desc']='';
		...
		output_data(array('order_list'=>$order_listnew), mobile_page($page_count));		
	...
	public function receiving_orderOp(){
	    $data = Model('sweepstakesorder')->receivingPointOrder($_POST['order_id']);
		if ($data['state']){
			output_data('1');
		}else {
