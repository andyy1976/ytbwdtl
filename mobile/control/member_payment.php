...
/**
    ...
    public function pd_orderOp(){
        ...
        if (empty($payment_info) && $payment_code == 'ylwtz' ) {
            $payment_info = [];
            $payment_info['payment_code'] = 'ylwtz';
            $payment_info['payment_name'] = '银联无跳转';
        }
        if (empty($payment_info) && $payment_code == 'vbill' ) {
            $payment_info = [];
            $payment_info['payment_code'] = 'vbill';
            $payment_info['payment_name'] = '随行付';
        }

        if (empty($payment_info) && $payment_code == 'lepay') {
            $payment_info = [];
            $payment_info['payment_code'] = 'lepay';

            $payment_info['payment_name'] = '乐支付无跳转';
        }

        if(empty($payment_info) && $payment_code!='rcb_pay') {
        ...
        // if ($payment_code == 'ylwtz') {
        //     var_dump(file_exists($inc_file));
        //     echo $inc_file;
        //     echo 111;
        //     exit;
        // }

        if(!file_exists($inc_file) && $payment_code!='rcb_pay'){
            return callback(false,'系统不支持选定的支付方式');
        }
        ...
        if($recharge_count>1000000){
          
            echo "<script>alert('每日限100万！！！');window.location.href = '".WAP_SITE_URL."';</script>";          
            exit;
        }
       if($types=='1' && ($recharge_count+$amout*20)>1000000){
           
            echo "<script>alert('每日限100万！！！');window.location.href = '".WAP_SITE_URL."';</script>";          
            exit;
        }
        if($types=='2' && ($recharge_count+$amout*12.5)>1000000){
        ...
        if($types!=null){
            ...
            if($ajax){
               if($amount>1000 && $types=='1'){
                ...
                if($amount>1000 && $types=='1'){
                echo "<script>alert('每日限购1千金额！！！');window.location.href = '".WAP_SITE_URL."';</script>";          
                exit; 
                }
                if($amout<50 && $types=='1'){
                    echo "<script>alert('每日购买至少50金额！！！');window.location.href = '".WAP_SITE_URL."';</script>";          
                    exit;
                }
                if($amout<50 && $types=='2'){
                    echo "<script>alert('每日购买至少50金额！！！');window.location.href = '".WAP_SITE_URL."';</script>";          
        ...
        // }
        if($payment_code=='rcb_pay'){
            $payment_info['payment_name']='充值余额支付';
            $order_pay_info['pdr_member_id']=$this->member_info['member_id'];
            $order_pay_info['pdr_member_name']=$this->member_info['member_name'];
            $order_pay_info['pdr_amount']=$amout;
            $order_pay_info['pdr_sn']=$pdr_sn;
            $order_pay_info['pdr_type']=$types;
            $out_trade_no=$pdr_sn;
            $trade_no=$pdr_sn;
            $member=Model('member');
            $member_info=$member->where(array('member_id'=>$this->member_info['member_id']))->find();
            if($member_info['member_predeposit']<$amout){
                 echo "<script>alert('充值余额不足，无法购买！');window.location.href='/wap/tmpl/member/pdrecharge_list.html';</script>";
                 // $pay_ok_url =WAP_SITE_URL."/tmpl/member/pdrecharge_list.html";
                 // redirect($pay_ok_url); 
                 exit;
            }
            $member_update=$member->where(array('member_id'=>$this->member_info['member_id']))->setDec('member_predeposit',$amout);
            $result = $logic_payment->updatePdOrder($out_trade_no, $trade_no, $payment_info, $order_pay_info);
           
           if (empty($member_update)) {
                echo "<script>alert('购买失败，请重新购买！');window.location.href='/wap/tmpl/member/pdrecharge_list.html';</script>";

            }else{
                echo "<script>alert('购买成功！');window.location.href='/wap/tmpl/member/pdrecharge_list.html';</script>";
            }
           
            exit;   
        }

        if($payment_code=='lepay'){

            $key=$_GET['key'];
            echo "<script>window.location.href='/wap/tmpl/member/bankcard_list.html?order_sn=$pdr_sn&amount=$amout&key=$key';</script>";
            exit;
        }
    ...
    public function pay_newOp() {
	   ...
	   if (empty($payment_info) && $payment_code == 'vbill' ) {
        ...
        foreach ($order_pay_info['data']['order_list'] as $key => $value) {
            ...
            $amout=$amout+$value['order_amount'];
            //获取商品云豆总数
            $points_1=$points_1+$value['order_pointsamount'];
        }
        $order_id=implode(',',$order_id);
        // if($this->member_info['member_id']=='10088'){
        //     print_r($order_pay_info['data']);
        //     exit;
        // }
        $pd['order_id']=array('in',$order_id);
        $goodsinfo=$order_goods->where($pd)->select();
        foreach ($goodsinfo as $key => $value) {
            if($value['gc_id']=='10351'){
                $gc_id=$value['gc_id'];
            }
            if($value['goods_id']=='97414'){
...
// }
        ...
        $amout_1=$amout+$amout*0.08;
        //商品价格
        // $amout=$order_pay_info['data']['order_list'][0]['order_amount'];
        //判断云豆余额
        if($this->member_info['member_level']=='5'){
            if($_GET['pd_pay']==1 && $this->member_info['province_predeposit']<$amout_1){
               echo "<script>alert('您的余额不够，暂时无法购买！！');window.history.go(-1);</script>";
               exit;
            }
        }else{
            if($_GET['pd_pay']==1 && $this->member_info['available_predeposit']<$amout_1){
               echo "<script>alert('您的余额不够，暂时无法购买！！');window.history.go(-1);</script>";
               exit;
            }
        }
       
       if($_GET['poi_pay']==1 && $this->member_info['member_points']<$points_1){
               echo "<script>alert('您的余额不够，暂时无法购买！！');window.history.go(-1);</script>";
               exit;
        }
        //站内余额支付
        ...
        if (empty($api_pay_amount)) {
				 $ts = Model()->table('orders')->where(array('order_id'=>$order_info['order_id']))->find();
             if($ts){  //根据订单号取出goods_id
            $allgoods = Model()->table('order_goods')->where(array('order_id'=>$order_info['order_id']))->select();
            if(count($allgoods)==1){
                $goods_all=$allgoods[0]['goods_id'];
            }else{
                $goods_acc = array();
                foreach ($allgoods as $key => $value) {
                   $goods_acc[$key] = $value['goods_id'];
                }
                if(count($goods_acc)>1){
                    $goods_all = @implode(',', $goods_acc);
                }
            }
            }
            if($goods_id=='97414' && $_GET['poi_pay']==1){
            ...
            $pay_ok_url = WAP_SITE_URL.'/tmpl/member/order_list.html?#selected';
			  ...
			  if($ts['is_dm']==1){
				  $bs= Model()->table('orders')->where(array('order_id'=>$ts['order_id']))->update(array('order_state'=>50));
				  dmsm_pay($order_info['order_id']);
				  }
            $pay_ok_url = WAP_SITE_URL.'/tmpl/dm/paysuccess.html?goods_id='.$goods_all;
            }
            redirect($pay_ok_url);
        }
