...
class pointcartControl extends mobileMemberControl {
	...
	public function step1Op(){
		...
		if (empty($_POST['address_id'])) {
			//实例化收货地址模型（不显示自提点地址）
			$address_list = Model('address')->getAddressList(array(
		...
		$address_info = Model('address')->getAddressInfo(array(
			'member_id'=> $this->member_info['member_id'],
			'address_id' => $_POST['address_id'],
			'dlyp_id'=>0
		));
		// print_r($address_info);
		output_data(array('pointprod_arr'=>$data['data'],'address_info'=>$address_info));
	}
	/**
	 * 兑换订单流程第二步
	 */
