...
class member_cartControl extends mobileMemberControl {
    ...
    public function cart_listOp() {
        ...
        foreach ($cart_list as $cart) {
            ...
            $cart['goods_total_points'] = ncPriceFormat($cart['goods_points'] * $cart['goods_num']);//加入的。。。。。。。。。
            ...
            $sum_points += $cart['goods_total_points'];//加入的。。。。。。。。。
        }
        ...
        output_data(array('cart_list' => array_values($store_cart_list), 'sum' => ncPriceFormat($sum),'isadmin'=>intval($member_info['member_level']),'sum_points' => $sum_points,'pdyouzhi'=>$pdyouzhi, 'cart_count' => count($cart_list)));
    ...
    public function cart_list_oldOp() {
        ...
        foreach ($cart_list as $key => $value) {
            ...
            $cart_list[$key]['goods_sum_points'] = $value['goods_points'] * $value['goods_num'];//加入的。。。。。。。。。
            $sum += $cart_list[$key]['goods_sum'];
            $sum_points += $cart_list[$key]['goods_sum_points'];//加入的。。。。。。。。。
        }
    
        output_data(array('cart_list' => $cart_list, 'sum' => ncPriceFormat($sum), 'sum_points' =>$sum_points));
    }
    ...
    public function cart_addOp() {
        ...
        $param['goods_points'] = $goods_info['goods_points'];//加入的。。。。。。。。。
    ...
    public function cart_edit_quantityOp() {
        ...
        if ($update) {
            ...
            $return['goods_points'] =$cart_info['goods_points'];//加入的。。。。。。。。。
            $return['total_points'] = $cart_info['goods_points'] * $quantity;  //加入的。。。。。。。。。       
            output_data($return);
        } else {
            output_error('修改失败');
        }
