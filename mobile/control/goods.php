...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listOp() {
        ...
        $fieldstr .= ',is_virtual,is_presell,is_fcode,have_gift,goods_jingle,store_id,store_name,is_own_shop,goods_points';
        ...
        //获取类别广告图
        $goods_class_nav=Model('goods_class_nav');
        $goods_class_info=$goods_class_nav->getfby_gc_id($condition['gc_id'],'cn_adv1');
        //获取商品类别名称
        $goods_class=Model('goods_class');
        $goods_class_infos=$goods_class->where(array('gc_id'=>$condition['gc_id']))->find();
        //处理商品列表(抢购、限时折扣、商品图片)
        $goods_list = $this->_goods_list_extend($goods_list);
        output_data(array('goods_list' => $goods_list,'gc_id'=>$condition['gc_id'],'cn_pic'=>$goods_class_info,'gc_name'=>$goods_class_infos['gc_name']));
    }
    ...
    public function goods_detailOp() {
        ...
        $member_info =  $member->getMemberInfo(array('member_name'=>$_COOKIE['username']));//加入
        $goods_detail['goods_info']['member_level'] = $member_info['member_level'];//加入
        if (empty($goods_detail)) {
            output_error('商品不存在');
        }

        // 默认预订商品不支持手机端显示
        // 20170823潘丙福修改--增加预售商品
        if ($goods_detail['goods_info']['is_book'] == 1) {
            $goods_detail['goods_info']['old_goods_price']   = $goods_detail['goods_info']['goods_price'];
            $goods_detail['goods_info']['old_goods_points']  = $goods_detail['goods_info']['goods_points'];
            $goods_detail['goods_info']['goods_price']       = ncPriceFormat($goods_detail['goods_info']['book_down_payment'] + $goods_detail['goods_info']['book_final_payment']);
            $goods_detail['goods_info']['goods_points']      = $goods_detail['goods_info']['book_down_payment_points'] + $goods_detail['goods_info']['book_final_payment_points'];
    ...
    private function _goods_detail_extend($goods_detail) {
        ...
        //20170823潘丙福添加开始--格式化预售商品结束时间
        if ($goods_detail['goods_info']['book_down_time'] > 0) {
            $goods_detail['goods_info']['book_down_time'] = date('Y年m月d日',$goods_detail['goods_info'][book_down_time]);
        }
        //20170823潘丙福添加结束
        //整理数据
        unset($goods_detail['goods_info']['goods_commonid']);
        // unset($goods_detail['goods_info']['gc_id']);   注销
