...
class Dumper
{
    ...
    public static function dumpAsString($var, $depth = 10, $highlight = false)
    {
        ...
        if ($highlight) {
            ...
            self::$_output = preg_replace('/<code>/', 
                '<code style="font-size: .88em; font-family: Monaco, Consolas, \'Lucida Console\', \'Courier New\', monospace">'
            , self::$_output, 1);

            self::$_output = '<pre>' . str_replace('&nbsp;', ' ', self::$_output) .'</pre>';
        }
        ...
        switch (gettype($var)) {
            ...
            case 'array':
                if (self::$_depth <= $level) {
                    ...
                    foreach ($keys as $key) {
                        $keyLen = strlen($key);
                        $append = $keyLen==$maxLen ? '' : str_repeat(' ', $maxLen-$keyLen);
