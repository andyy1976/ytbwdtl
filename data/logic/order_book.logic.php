...
class order_bookLogic {
    ...
    public function buy($order_data = array(), $post_data = array()) {
        ...
        if ($post_data['book_pay_type'] == 'part') {
            ...
            //20170822潘丙福添加开始-定金云豆字段添加
            $data['book_amount_points'] = $goods_info['book_down_payment_points'] * $goods_info['goods_num'];
            //20170822潘丙福添加结束
            ...
            //20170822潘丙福添加开始-尾款云豆字段添加
            $data['book_amount_points'] = $goods_info['book_final_payment_points'] * $goods_info['goods_num'];
            //20170822潘丙福添加结束
        ...
        if (!$book_list[0]['book_step']) {
            ...
            if (empty($book_list[0]['book_pay_time'])) {
                ...
                if ($book_list[0]['book_end_time'] > TIMESTAMP) {
                    ...
                    $order_info['pay_amount'] = $book_list[0]['book_amount'];
                    //20170822潘丙福添加开始
                    $order_info['pay_amount_ppoints'] = $book_list[0]['book_amount_points'];
                    //20170822潘丙福添加结束
                    $order_info['api_pay_amount'] = $book_list[0]['book_amount'] - $book_list[0]['book_pd_amount'] - $book_list[0]['book_rcb_amount'];
                ...
                if (empty($book_list[1]['book_pay_time'])) {
                    ...
                    if ($book_list[1]['book_end_time'] > TIMESTAMP) {
                        ...
                        if ($order_info['order_state'] == ORDER_STATE_CANCEL) {
                            ...
                            $book_list[1]['book_state'] = '须在'.date('Y-m-d',$book_list[1]['book_end_time']+1).'之前付款';
                        ...
                        //20170822潘丙福添加开始
                        $order_info['pay_amount_ppoints'] = $book_list[1]['book_amount_points'];
                        //20170822潘丙福添加结束
