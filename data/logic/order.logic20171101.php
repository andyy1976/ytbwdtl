...
class orderLogic {
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            //判断小于程序上线时间运行
            if ($order_info['delay_time'] < 1500433877) {
                $update_order['platform_payment'] = 2;
            } else {
        ...
        //20170716潘丙福添加开始
            ...
            if ($order_info['delay_time'] < 1500433877) {
                ...
                $log['description']='买家确认收货--商家获得剩余订单的50%佣金-订单号：'.$order_info['order_sn'];
            } else {
                //冻结钱包加订单的50%--新结算方式
                $dat['wallet_frozen']=array('exp','wallet_frozen+'.$order_info['order_amount']*0.5);
                $log['available_amount']=$order_info['order_amount']*0.5;
                $log['description']='买家确认收货--商家获得50%佣金-订单号：'.$order_info['order_sn'];
            }                      
