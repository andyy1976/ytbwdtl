...
class orderLogic {
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            $model_order = Model('order');
            $store_info = $model_store->getStoreInfoByID($order_info['store_id']); //判断是否是实体商家店铺
             //更新订单状态
            $update_order = array();
            $update_order['finnshed_time'] = TIMESTAMP;
            $update_order['order_state'] = ORDER_STATE_SUCCESS;
            //判断程序上线时间运行-根据上线时间再确定20171103 16::00:00
            if ($order_info['payment_time'] < 1509696000) {
                $update_order['platform_payment'] = 1;
            }
            $update = $model_order->editOrder($update_order,array('order_id'=>$order_id));
            if (!$update) {
                throw new Exception('保存失败');
            }
            if($store_info['store_flag']==1){  //实体商家店铺收货
            $order_goods = $model_order->getOrderGoodsInfoArray(array('order_id'=>$_GET['order_id']));
                 foreach($order_goods as $b=>$c){
                   checkpoint($c['rec_id'],$c['store_id']);
                 }
            }else{
                //扣除订单所需云豆
                points_delete($order_info['order_id']);
            }
            //20171030潘丙福添加开始
            $store = Model('seller');
            $rcb   = Model('rcb_log');                      
            if ($order_info['payment_time'] < 1509696000) {
                //原结算方式-钱包1，钱包2各加50%货款
                ...
                $log['description']      = '买家确认收货--商家获得50%货款-订单号：'.$order_info['order_sn'];
            }
     ...
     * @param string $role 操作角色 buyer、seller、admin、system 分别代表买家、商家、管理员、系统
     * @param string $user 操作人
     * @param float $shippingCode 订单号
     * @return array
     */
    public function changeOrderShippingCode($order_info, $role, $user = '', $shippingCode, $post) {
        try {
            ...
            //修改快递公司 2017/9/26
            if($post['e_id'] != $post['shipping_express_id']){
                $data1['shipping_express_id'] = $post['e_id'];
                $b = $model_order->editOrder_common($data1,array('order_id'=>$order_id));
            }

            $data = array();
            $data['shipping_code'] = trim($shippingCode);
            $update = $model_order->editOrder($data,array('order_id'=>$order_id));
            if (!$update) {
                throw new Exception('保存失败');
            }
            ...
            $data['log_role'] = $role;
            $data['log_user'] = $user;
            $data['log_msg'] = '修改了物流单号'.'( '.$shippingCode.' )';;
            $data['log_orderstate'] = $order_info['payment_code'] == 'offline' ? ORDER_STATE_PAY : ORDER_STATE_NEW;
            $model_order->addOrderLog($data);
            return callback(true,'操作成功');
        } catch (Exception $e) {
     ...
     * @return array
     */
    public function changeOrderRefundShippingFee($order_info, $role, $user = '', $shippingFee) {
        try {
            $model_order = Model('order');
            $model_order->beginTransaction();
            ...
            $order_id = $order_info['order_id'];           
            $data = array();
            $data['is_refund_shipping_fee'] = '1';
            $data['refund_shipping_fee']    = $shippingFee;
            $update = $model_order->editOrder($data,array('order_id'=>$order_id));
            if (!$update) {
            ...
            $data['log_role'] = $role;
            $data['log_user'] = $user;
            $data['log_msg']  = '订单退还运费'.'( '.$shippingFee.' 元)';;
            $data['log_orderstate'] = $order_info['payment_code'] == 'offline' ? ORDER_STATE_PAY : ORDER_STATE_NEW;
            $orderLogResult         = $model_order->addOrderLog($data);
            if (!$orderLogResult) {
                throw new Exception('添加操作订单日志失败');
            }
            //修改商家钱包2
            $sellerUpdateResult = $model_order->table('seller')->where(array('store_id'=>$order_info['store_id']))->setDec('wallet_release', $shippingFee);
            if (!$sellerUpdateResult) {
                throw new Exception('修改商家余额失败');
            }
            //记录rcb_log日志表
            $rcbLogData = array();
            $rcbLogData['member_id']           = $order_info['store_id'];
            $rcbLogData['member_name']         = $order_info['store_name'];
            $rcbLogData['type']                = 'seller';
            $rcbLogData['add_time']            = time();
            $rcbLogData['available_amount']    = $shippingFee;
            $rcbLogData['freeze_amount']       = 0;
            $rcbLogData['description']         = "订单退运费成功，商家钱包扣除运费-{$order_info['order_sn']}";
            $rcbLogResult = $model_order->table('rcb_log')->insert($rcbLogData);
            if (!$rcbLogResult) {
                throw new Exception('添加rcb_log日志失败');
            }
            //给买家member_predeposit加退还运费
            $buyerUpdateResult = $model_order->table('member')->where(array('member_id'=>$order_info['buyer_id']))->setInc('member_predeposit', $shippingFee);
            if (!$buyerUpdateResult) {
                throw new Exception('增加买家余额失败');
            }
            //记录pd_log日志表
            $pdLogData = array();
            $pdLogData['lg_member_id']           = $order_info['buyer_id'];
            $pdLogData['lg_member_name']         = $order_info['buyer_name'];
            $pdLogData['lg_type']                = 'refund_shippingfee';
            $pdLogData['lg_av_amount']           = $shippingFee;
            $pdLogData['lg_freeze_amount']       = 0;
            $pdLogData['lg_add_time']            = time();
            $pdLogData['lg_desc']                = "确认退运费，订单号:{$order_info['order_sn']}";
            $pdLogResult = $model_order->table('pd_log')->insert($pdLogData);
            if (!$pdLogResult) {
                throw new Exception('添加pd_log日志失败');
            }            
            $model_order->commit();
            return callback(true,'操作成功');
        } catch (Exception $e) {
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array()) {
        ...
        try {
            ...
            foreach($order_list as $order_info) {
                ...
                //更新订单状态
                //20171103潘丙福添加--platform_payment字段为3
                $update_order = array();
                $update_order['order_state'] = ORDER_STATE_PAY;
                $update_order['platform_payment'] = 3;
        ...
        foreach($order_list as $order_info) {
            ...
            $sell=Model('seller');
            $member=Model('member');   
            $sell_info=$sell->where(array('store_id'=>$order_info['store_id']))->find();
               
            $members_info=$member->where(array('member_id'=>$sell_info['member_id']))->find();

            send($members_info['member_mobile'],$order_info['order_sn']);
            //非预定订单下单或预定订单全部付款完成
