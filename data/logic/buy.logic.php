...
class buyLogic {
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id, $jjg = null,$orderdiscount = 0,$member_level = 0,$ifchain = null) {
        ...
        $isCrossBorder = false;
        $isBookGoods   = false;
        //20170321潘丙福添加判断订单中是否有跨境商品、是否为预售商品
        foreach ($result['data']['goods_list'] as $panKey => $panValue) {
            $panGoodsModel = Model();
            $panArray = $panGoodsModel->table('goods')->field('is_cross_border,is_book')->find($panValue['goods_id']);
            if ($panArray['is_cross_border']) {
                $isCrossBorder = true;
            }
            if ($panArray['is_book'] == 1) {
                $isBookGoods = true;
            }
        }
        //得到页面所需数据：收货地址、发票、代金券、预存款、商品列表等信息
        $result = $this->getBuyStep1Data($member_id,$result['data'],$orderdiscount);
        $result['isCrossBorder'] = $isCrossBorder;
        $result['isBookGoods']   = $isBookGoods;
    ...
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0) {
        ...
        list($store_cart_list,$store_goods_total,$store_goods_total_points) = $this->_logic_buy_1->calcCartList($store_cart_list);
        ...
        $result['store_goods_total_points'] = $store_goods_total_points;//加入的。。。。。。。。。。。
        ...
        if (!$data['is_book']) {
            ...
            //20170921潘丙福添加开始-增加手机端预售商品结算
            if (APP_ID == 'mobile') {
                $result['store_goods_total_1'][$goods_list[0]['store_id']] = $goods_list[0]['goods_num']*$goods_list[0]['book_down_payment'];
            }
            //20170921潘丙福添加结束
     ...
     * 支付开通商城的费用
     */
    public function panBuyStep2($post, $member_id, $member_name, $member_email) {
        $this->_member_info['member_id'] = $member_id;
        $this->_member_info['member_name'] = $member_name;
        $this->_member_info['member_email'] = $member_email;
        $this->_post_data = $post;
        try {
            ...
            $model = Model('order');
            $model->beginTransaction();
            //1.判断提交的表单的用户id是否与session中的用户id相同
            if ($this->_post_data['member_id'] != $member_id) {
                throw new Exception('非法入侵，请停止您的违法行为001！');
            }
            //2.通过提交的member_id去表（store_joinin）查找数据
            $panTmp = Model()->table('store_joinin')->find($member_id);
            if (! $panTmp) {
                throw new Exception('非法入侵，请停止您的违法行为002！');
            }
            //3.生成pay_sn
            $pay_sn = $this->_logic_buy_1->makePaySn($member_id);
            $opshop_pay = array();
            $opshop_pay['openshop_pay_sn'] = $pay_sn;
            $opshop_pay['member_id'] = $member_id;
            $opshop_pay_id = Model()->table('opshop_pay')->insert($opshop_pay);
            if (!$opshop_pay_id) {
                throw new Exception('开通店铺付款订单保存失败[未生成支付单]');
            }
            //4.生成开通店铺的订单数据
            $opshop_order = array();
            $opshop_order['member_id'] = $member_id;
            $opshop_order['openshop_pay_sn'] = $pay_sn;
            $opshop_order['openshop_order_sn'] = $this->_logic_buy_1->makeOrderSn($opshop_pay_id);
            $opshop_order['pay_amount'] = $panTmp['paying_amount'];
            $opshop_order['add_time'] = TIMESTAMP;
            $opshop_order_id = Model()->table('opshop_order')->insert($opshop_order);
            if (!$opshop_order_id) {
                throw new Exception('开通店铺订单保存失败[未生成订单数据]');
            }
            $model->commit();
            
            return callback(true,'',$pay_sn);
...
//             $order_platform_store_ids[$k] = null;
    ...
    private function _createOrderStep2() {
        ...
        if ($post['ifcart']) {
            ...
            //来源于直接购买
            $goods_id = key($input_buy_items);//商品ID
            $quantity = current($input_buy_items);//商品数量
    ...
    private function _createOrderStep3() {
        ...
        $this->_order_data['store_goods_total_points'] = $store_goods_total_points;//加入的。。。。。。。。。。。
...
//             $should_goods_total = $store_final_order_total[$store_id]-$store_freight_total[$store_id]+$promotion_total;
            ...
            //20170321潘丙福添加购买者身份证号
            $order['buyer_cardid'] = $this->_post_data['buyer_cardid'];
            //20170710潘丙福添加订单云豆
            $order['order_pointsamount'] = $this->_order_data['store_goods_total_points'][$store_id];
...
//             }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $order_goods[$i]['goods_points'] = $goods_info['goods_points'];//加入的。。。。。。。。。。。。
                    ...
                    //计算每种商品的总积分
                    $goods_total_points = $goods_info['goods_points'] * $goods_info['goods_num'];//加入的。。。。。。。。。。。。。
                    //计算本件商品优惠金额
                    ...
                    $order_goods[$i]['goods_pay_points'] = $goods_total_points;//加入的。。。。。。。。。。。。
