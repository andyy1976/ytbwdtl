...
class buy_1Logic {
    ...
    public function getStoreFreightDescList($store_goods_total) {
        ...
        foreach ($store_list as $store_info) {
            $limit_price = ($store_info['store_free_price']);
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        $available_rcb_amount = ($buyer_info['member_predeposit']);
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $lso = $model_order->table('order_goods')->where(array('order_id'=>$order_info['order_id']))->find();
            ...
            if ($available_rcb_amount >= $order_amount) {
                ...
                $available_rcb_amount = ($available_rcb_amount);
                // $member_as = $model->where(array('member_id'=>$buyer_info['member_id']))->setDec('member_points',$lso['goods_pay_points']);
                ...
                if ($order_info['order_type'] == 2) {
                    $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = ncPriceFormat(($order_info['original_rcb_amount']) + $order_amount);
                } else {
                ...
                if ($available_rcb_amount > 0) {
                    ...
                    $data_order = array();
                    if ($order_info['order_type'] == 2) {
                        $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = ncPriceFormat(($order_info['original_rcb_amount']) + $available_rcb_amount);
                    } else {
    ...
    /**
     * 奖金支付,依次循环每个订单
     * 如果奖金足够就单独支付了该订单，如果不足就暂时冻结，等API支付成功了再彻底扣除
     */
    public function disPay($order_list, $input, $buyer_info) {
        $model_order = Model('order');
        $model_pd = Model('predeposit');
        $member_id = $buyer_info['member_id'];
        $member_name = $buyer_info['member_name'];
        $model = model('member');
        $points = model('points');
        $available_rcb_amount = ($buyer_info['distributor_predeposit']);
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_rcb_amount >= $order_amount) {
                //立即支付，订单支付完成
                $model_pd->changeDis('order_pay',$data_pd);
                $available_rcb_amount -= $order_amount;
                $available_rcb_amount = ($available_rcb_amount);
                // $member_as = $model->where(array('member_id'=>$buyer_info['member_id']))->setDec('member_points',$lso['goods_pay_points']);
                // if($member_as){
                //     $data['pl_memberid']=$member_id;
                //     $data['pl_membername']=$member_name;
                //     $data['pl_points']="-{$lso['goods_pay_points']}";
                //     $data['pl_addtime']=time();
                //     $data['pl_desc']='购物定单'.$order_info['order_sn'];
                //     $data['pl_stage']='order';
                //     $points->addPointsLog($data);
                // }
                //记录订单日志(已付款)
                $data = array();
                $data['order_id'] = $order_info['order_id'];
...
/**
     * 云豆支付,依次循环每个订单
     * 如果云豆足够就单独支付了该订单，如果不足就暂时冻结，等API支付成功了再彻底扣除
     */
    public function pointPay($order_list, $input, $buyer_info) {
        $model_order = Model('order');
        $model_pd = Model('predeposit');
        $member_id = $buyer_info['member_id'];
        $member_name = $buyer_info['member_name'];
        $model = model('member');
        $points = model('points');
        $points_rcb_amount = ($buyer_info['member_points']);
        
        if ($points_rcb_amount <= 0) return $order_list;
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            //货到付款的订单、已经支付的订单跳过
            if ($order_info['payment_code'] == 'offline') continue;
            if ($order_info['order_state'] > 10) continue;
            $order_amount = ($order_info['order_amount']);
            $data_pd = array();
            $data_pd['member_id'] = $member_id;
            $data_pd['member_name'] = $member_name;
            ...
            if ($points_rcb_amount >= $order_amount) {
                //立即支付，订单支付完成
                $model_pd->pointsDis('order_pay',$data_pd);
                $points_rcb_amount -= $order_amount;
                $points_rcb_amount = ($points_rcb_amount);
                // $member_as = $model->where(array('member_id'=>$buyer_info['member_id']))->setDec('member_points',$lso['goods_pay_points']);
                // if($member_as){
                //     $data['pl_memberid']=$member_id;
                //     $data['pl_membername']=$member_name;
                ...
                //暂冻结充值卡,后面还需要 API彻底完成支付
                if ($points_rcb_amount > 0) {
                    $data_pd['amount'] = $points_rcb_amount;
                    if ($order_info['order_type'] != 2) {
                        $model_pd->changeRcb('order_freeze',$data_pd);
                    } else {
                        //预定订单没有冻结，是直接扣除
                    ...
                    $data_order = array();
                    if ($order_info['order_type'] == 2) {
                        $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = ncPriceFormat(($order_info['original_rcb_amount']) + $points_rcb_amount);
                    } else {
                        $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = $points_rcb_amount;
                    }
                    $result = $model_order->editOrder($data_order,array('order_id'=>$order_info['order_id']));
                    $points_rcb_amount = 0;
                    if (!$result) {
    ...
    public function pdPay($order_list, $input, $buyer_info) {
        ...
        $available_pd_amount = ($buyer_info['available_predeposit']);
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($order_info['payment_code'] == 'offline') continue;
            $order_amount = ($order_info['order_amount']) - ($order_info['rcb_amount']);
            ...
            if ($available_pd_amount >= $order_amount) {
                ...
                $log['freeze_amount']    = 0;
                $log['description']      = '买家付款--商家钱包1获得100%货款-订单号：'.$order_info['order_sn'];
                $log['member_id']        = $order_info['store_id'];
                $log['member_name']      = $order_info['store_name'];
                $log['type']             = 'seller';
                $log['add_time']         = time();

                $panInsertLog = $rcb->insert($log);

                if (!$panInsertLog) {
                    throw new Exception('操作失败');
                }

                $panUpdateresult = $store->where(array('store_id'=>$order_info['store_id']))->update($dat);
                ...
                //订单状态 置为已支付
                $data_order = array();
                $order_list[$key]['platform_payment'] = $data_order['platform_payment'] = 3;
                $order_list[$key]['order_state'] = $data_order['order_state'] = ORDER_STATE_PAY;
                $order_list[$key]['payment_time'] = $data_order['payment_time'] = TIMESTAMP;
                $order_list[$key]['payment_code'] = $data_order['payment_code'] = 'predeposit';
                if ($order_info['order_type'] == 2) {
                    $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = ncPriceFormat(($order_info['original_pd_amount']) + $order_amount);
                } else {
                ...
                if ($available_pd_amount > 0) {
                    ...
                    $data_order = array();
                    if ($order_info['order_type'] == 2) {
                        $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = ncPriceFormat(($order_info['original_pd_amount']) + $available_pd_amount);
                    } else {
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $new_array['isdmgoods']=$goods_info['isdmgoods'];
        ...
        $new_array['goods_points'] = $goods_info['goods_points'];//加入的。。。。。。。。
        ...
        //20170818潘丙福添加开始-查询云豆(预定云豆、尾款云豆)
        $new_array['book_down_payment_points'] = $goods_info['book_down_payment_points'];
        $new_array['book_final_payment_points'] = $goods_info['book_final_payment_points'];
        //20170818潘丙福添加结束
    ...
    public function getBookInfo(& $goods_info = array()) {
        ...
        //20170821潘丙福添加开始--预售商品云豆价格处理
        $goods_info['goods_points'] = $goods_info['book_down_payment_points']+$goods_info['book_final_payment_points'];
        //20170821潘丙福添加结束
    }
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                $cart_list[$key]['goods_points'] = $goods_online_info['goods_points'];//加入的。。。。。。。。。
