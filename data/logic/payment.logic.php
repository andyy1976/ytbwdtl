...
class paymentLogic {
    ...
    public function updatePdOrder($out_trade_no,$trade_no,$payment_info,$recharge_info) {
        ...
        // $file  = '/data/wwwlogs/payment'.date('y-m-d',time()).'.log';
        // $content='\r\n接口返回次数：'.$pdr_info['pdr_auto'].'|订单号：'.$out_trade_no.'|金额：'.$recharge_info['pdr_amount'].'|ID：'.$recharge_info['pdr_member_id'].'|时间:'.date('y-m-d h:i:s',time());
        // $f  = file_put_contents($file, $content,FILE_APPEND);		
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdr_sn'] = $recharge_info['pdr_sn'];
        $condition['pdr_payment_state'] = '0';
		$recharge=Model('pd_recharge');
        //修改时间：李工，日期：2107-03-13*/
		$updateit = $model_pd->getPdRechargeCount($condition);
        $charge_state=$recharge->where($condition)->find();
        if(intval($updateit)>0 &&  $charge_state['pdr_payment_state']=='0'){
            $update = array();
            $update['pdr_payment_state'] = '1';
            $update['pdr_payment_time'] = TIMESTAMP;
            $update['pdr_payment_code'] = $out_trade_no;
            $update['pdr_payment_name'] = $payment_info['payment_name'];
            $update['pdr_trade_sn'] = $recharge_info['pdr_sn'];
           //更改充值状态
            $state = $recharge->where($condition)->update($update);
            if($state){
                //增加通联钱包和易宝钱包
                // $member_amount=$recharge_info['pdr_amount'];
                // if(trim($payment_info['payment_name'])=='通联支付'){
                //     $update_member['tl_amount']=array('exp','tl_amount+'.$member_amount);
                // }elseif(trim($payment_info['payment_name'])=='易宝支付'){
                //     $update_member['yb_amount']=array('exp','yb_amount+'.$member_amount);
                // }
                // $member=Model('member');
                // $member->where(array('member_id'=>$recharge_info['pdr_member_id']))->update($update_member);
                ...
                $member=Model('member');
                $split_id=$member->getfby_member_id($recharge_info['pdr_member_id'],'split_id');
                $data['split_id']=$split_id;
                //新增充值类型
                $data['pdr_type']=$recharge_info['pdr_type'];
                //新增充值方式 
                if(trim($payment_info['payment_name'])=='银联支付'){
                    $data['payment_name']='unionpay';
                }elseif(trim($payment_info['payment_name'])=='易宝支付'){
                    $data['payment_name']='ybzf';
                }
