...
class goodsLogic {
    ...
    public function saveGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc) {
        ...
        $store_info = $model_store->where(array('store_id'=>$store_id))->find();//判断是否是地面商家
        ...
        $common_array['goods_url'] = $param['g_url'];
        // 生成通用商品返回通用商品编号
    ...
    public function updateGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc) {
        ...
        //     'edit_counts' =>array('exp','edit_counts+1'));
        //     $countResult = Model()->table('goods_common')->update($editArray);
        // }
        // if (!$countResult) {
        //     return callback(false, '商品修改次数编辑失败');
        // }
        //20170710潘丙福添加结束
    ...
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state) {
        ...
        $store_info = $model_store->where(array('store_id'=>$store_id))->find();//判断是否是地面商家
        if($store_info['store_flag']==1){
        $goods_class = Model('goods_class')->getGoodsClassLineForTagg(intval($param['cate_id']));
        }else{
        ...
        $common_array['goods_points']        =$param['g_points'];//加入的。。。。。。。。。。。。。。
        ...
        //20170805潘丙福添加开始-增加字段ext_sup(供应商额外信息)
        if (trim($param['ext_sup'])) {
            $common_array['ext_sup']        = trim($param['ext_sup']); 
        }
        //20170805潘丙福添加结束
        $common_array['is_own_shop']        = in_array($store_id, Model('store')->getOwnShopIds()) ? 1 : 0;
        //潘丙福20170320添加字段
        $common_array['is_cross_border']    = intval($param['g_is_cross_border']);
        //熊磊增加
        $common_array['goods_max_money']    = intval($param['goods_max_money']);
        if($param['store_flag']==1){
        if(intval($param['usertc'])==2){
          $common_array['goods_points']        =$param['pointcsb'];//c套餐云豆数量
        }
        $common_array['pointsb']            =$param['quantity'];    //李志军添加，地面商家
        $common_array['usertc']             =intval($param['usertc']);
        $common_array['isdmgoods']          =1;
	$common_array['dmbuy_know']        = serialize("validate:".$param['validate']."&usertime:".$param['usertime']."&attationpeople:".$param['attationpeople']."&ruzhutime:".$param['ruzhutime']."&shopknow:".$param['shopknow']."&suitpepole:".$param['suitpepole']."&otherfree:".$param['otherfree']."&othercoupon:".$param['othercoupon']."&otherglue:".$param['otherglue']."&otherpeole:".$param['otherpeole']."&suitcontent:".$param['suitcontent']);
        $common_array['dmbuy_rule']    = "txtName|".serialize($param['txtName'])."&txtDanj|".serialize($param['txtDanj'])."&txtSulian|".serialize($param['txtSulian'])."&txtTel|".serialize($param['txtTel'])."&txtTRLastIndex:".$param['txtTRLastIndex'];
         }
        return $common_array;
    }
    /**
    ...
    private function _addGoods($param, $common_id, $common_array) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                //$goods['goods_points']       = $value['points'];//加入的。。。。。。。。。
                //20170320潘丙福添加
                $goods['is_cross_border']       = intval($param['g_is_cross_border']);
                $goods['goods_points']       = $param['g_points'];
                ...
                //20170805潘丙福添加开始--增加字段(ext_sup)
                if (trim($param['ext_sup'])) {
                    $goods['ext_sup']       = trim($param['ext_sup']);
                }
                //20170906xin增加字段(goods_url)
                $goods['goods_url']       = $param['g_url'];
                $goods['goods_key']       = $param['g_key'];
                //20170805潘丙福添加结束
                //熊磊增加
                $goods['goods_max_money']    = intval($value['goods_max_money']);
				  if($param['store_flag']==1){
	         if(intval($param['usertc'])==2){
                  $goods['goods_points']        =$param['pointcsb'];//c套餐云豆数量
                  }
                   $goods['pointsb']            =$common_array['pointsb'];    //李志军添加，地面商家
                   $goods['usertc']              =$common_array['usertc'];
                   $goods['isdmgoods']          =1;
                   $goods['dmbuy_know'] = $common_array['dmbuy_know'];
                   $goods['dmbuy_rule'] = $common_array['dmbuy_rule'];
                  }
            ...
            $goods['goods_points']       = $common_array['goods_points'];//加入的。。。。。。。。。。。
            //20170320潘丙福添加
            $goods['is_cross_border']   = intval($common_array['is_cross_border']);
    ...
    private function _editGoods($param, $common_id, $common_array, $store_id) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                $goods['goods_key']       = $param['g_key'];
                    //20170805潘丙福添加结束
                    $goods['is_cross_border']   = $param['g_is_cross_border'];
                    $goods['goods_marketprice'] = $value['marketprice'] == 0 ? $common_array['goods_marketprice'] : $value['marketprice'];
                    $goods['goods_serial']      = $value['sku'];
                    $goods['goods_storage_alarm']= intval($value['alarm']);
                    $goods['goods_spec']        = serialize($value['sp_value']);
		   ...
		   if($param['store_flag']==1){
                   $goods['pointsb']            =$common_array['pointsb'];    //李志军添加，地面商家              
		   if(intval($common_array['usertc'])==2){
                   $goods['goods_points']        =$common_array['pointcsb'];//c套餐云豆数量
                    }
                   $goods['usertc']              =$common_array['usertc'];
                   $goods['isdmgoods']          =1;
	               $goods['dmbuy_know']        = $common_array['dmbuy_know'];
                   $goods['dmbuy_rule']        = $common_array['dmbuy_rule'];
                    ...
                    $goods['goods_price']       = $value['price'];
                    $goods['goods_promotion_price']=$value['price'];
                    $goods['goods_points']       = $param['g_points'];//加入的。。。。。。。。。。
                    //20170906xin增加字段(goods_url)
                $goods['goods_url']       = $param['g_url'];
                $goods['goods_key']       = $param['g_key'];
                    //20170321潘丙福添加
            ...
            if (!empty($goods_info)) {
                ...
                $goods['goods_key']       = $param['g_key'];
                //20170321潘丙福添加
                $goods['is_cross_border']   = $common_array['is_cross_border'];
                //20170805潘丙福添加开始-增加字段(ext_sup)
                if (trim($param['ext_sup'])) {
                    $goods['ext_sup']       = trim($param['ext_sup']);
                }
                ...
                $goods['goods_max_money']    = intval($common_array['goods_max_money']);
                if ($common_array['is_virtual'] == 1) {
                    $goods['have_gift']    = 0;
                    Model('goods_gift')->delGoodsGift(array('goods_id' => $goods_id));
                    Model('goods_fcode')->delGoodsFCode(array('goods_id' => $goods_id));
    ...
    private function _initGoodsByCommonGoods($common_id, $common_array) {
        ...
        $goods['sup_id']            = $common_array['sup_id'];
        return $goods;
    }
