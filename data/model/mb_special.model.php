...
class mb_specialModel extends Model{
    ...
    public function getMbSpecialItemUsableListByID($special_id) {
        ...
        if(!empty($item_list)) {
            ...
            foreach ($item_list as $value) {
                // 熊磊  2017-7-20加入开始
                unset($value['item_data']['item']['special_id']);
                 // 熊磊  2017-7-20加入结束
                //处理图片
    ...
    private function _getMbSpecialItemList($condition, $order = 'item_sort asc') {
        ...
        foreach ($item_list as $key => $value) {
            // 熊磊  2017-7-20加入开始
            $value['item_data'] = unserialize($value['item_data']);
            $value['item_data']['item']['special_id'] = $value['special_id'];
            $value['item_data'] = serialize($value['item_data']);
             // 熊磊  2017-7-20加入结束
    ...
    private function _initMbSpecialItemGoodsData($item_data, $item_type) {
        $goods_id_string = '';
        // 熊磊  2017-7-20加入开始
        if($item_data['item']['special_id']  == 28){
            $item_data['item'] = array_slice($item_data['item'],0,2);
        }
        // 熊磊  2017-7-20加入结束
        if(!empty($item_data['item'])) {
            ...
            $data=time();
            //20170524潘丙福添加select条件以及排序
            //2017/06/22   熊磊增加字段goods_salenum
            //20170524潘丙福添加select条件以及排序    2017-7-20熊磊修改
            if($item_data['item']['special_id']  == 28){
                unset($condition);
           
                $ntime = date('Y-m-d H:i:s',strtotime(date('Y-m-d')));
                // $condition= "goods_addtime >".$ntime;
                $condition = array();
                $condition['goods_addtime'] = array('gt' , $ntime);
                $condition['goods_state']   = array('eq', 1);
                $condition['goods_verify']  = array('eq', 1);
                $field = 'goods_id,goods_name,goods_promotion_price,goods_price,goods_image,goods_points,goods_power,goods_salenum,goods_commonid,store_id';
                $order = 'goods_id desc';
                $goods_list = $model_goods->table('goods')->group('goods_commonid')->field($field)->where($condition)->order($order)->limit(50)->distinct(true)->select();
            }else{//2017-7-20熊磊修改结束
                $goods_list = $model_goods->getGoodsList($condition, 'goods_id,goods_name,goods_promotion_price,goods_price,goods_image,goods_points,goods_power,goods_salenum', '', 'goods_power desc');
            }
            //新增开始
            $xianshi = $model_xianshi->getXianshiLists($item_data['item'],'xianshi_price,goods_id');
            foreach ($goods_list as $key => $value) {

                foreach ($xianshi as   $v) {

                    if($value['goods_id'] == $v['goods_id']){
                        $value['xianshi'] = $v['xianshi_price'];
                        $goods_list[] = $value;
                        }else{
                             continue;
                        }
         ...
         //新增结束
            ...
            if($item_type=='goods2'){
                foreach ($goods_list as $key => $goods) {
                    $groupbuy_info=$groupbuy->where(array('goods_id'=>$key))->find();
                    $goods_list[$key]['groupbuy_price']=$groupbuy_info['groupbuy_price'];
                    if($groupbuy_info['start_time']>$data){
                        $goods_list[$key]['start_time']=date('H:s',$groupbuy_info['start_time']);

                    }else{
                        $goods_list[$key]['start_time']=1;
                    }
