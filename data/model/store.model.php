...
class storeModel extends Model {
    ...
    public function getStoreInfo($condition) {
        ...
        if(!empty($store_info)) {
            ...
            //20170424潘丙福添加开始
            //增加判断店铺手机功能如果没有手机号默认调取注册店铺时候添加的信息
            if (! trim($store_info['store_phone'])) {
                $panStoreCondition['member_id'] = array('eq', $store_info['member_id']);
                $panCompanyPhone = Model()->table('store_joinin')->field('company_phone')->where($panStoreCondition)->find();
                $store_info['store_phone'] = $panCompanyPhone['company_phone'];
            }
            //20170424潘丙福添加结束
            //商品数
...
/**
    ...
    private function getGoodsListBySales($store_array) {
    	...
    	foreach ($store_array as $value) {
    		$store_array[$value['store_id']]['search_list_goods'] = $model->table('goods')->field($field)->where(array('store_id'=>$value['store_id'],'goods_state'=>1))->order('goods_salenum,goods_addtime desc')->limit(8)->select();
    	}
    	return $store_array;
    }
}
