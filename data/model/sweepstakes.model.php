...
class sweepstakesModel extends Model {
     ...
     * 抽奖活动保存
     *
     * @param   array $param 抽奖活动资料
     */
    public function addPointGoods($param) {
        if(empty($param)) {
            return false;
        }
        $insert_id = $this->table('sweepstakes')->insert($param);
        if ($insert_id) {
            return $insert_id;
        } else {
            return false;
    ...
    public function getPointProdList($where = '', $field='*',$order='',$limit='',$page=''){
        ...
        $list = $this->table('sweepstakes')->field($field)->where($where)->order($order)->limit($limit)->page($page)->select();
        if (is_array($list) && count($list)>0){
            foreach ($list as $k=>$v){
                $v['sweepstakes_bgimg_old'] = $v['sweepstakes_bgimg'];
                $v['sweepstakes_bgimg_small'] = pointprodThumb($v['sweepstakes_bgimg'], 'small');
                $v['sweepstakes_bgimg'] = pointprodThumb($v['sweepstakes_bgimg'], 'mid');
    ...
    public function getPointProdInfo($where = '',$field='*'){
        $prodinfo = $this->table('sweepstakes')->where($where)->find();
        if (!empty($prodinfo)){
            $prodinfo['sweepstakes_bgimg_old'] = $prodinfo['sweepstakes_bgimg'];
            $prodinfo['sweepstakes_bgimg_max'] = pointprodThumb($prodinfo['sweepstakes_bgimg']);
            $prodinfo['sweepstakes_bgimg_small'] = pointprodThumb($prodinfo['sweepstakes_bgimg'], 'small');
            $prodinfo['sweepstakes_bgimg'] = pointprodThumb($prodinfo['sweepstakes_bgimg'], 'mid');
    ...
    public function delPointProdById($pg_id){
        ...
        if (is_array($pg_id)){
            ...
            $where['id'] = array('in',$pg_id);
            $where1['sweepstakes_id'] = array('in',$pg_id);
        } else {
            $pg_id = intval($pg_id);
            $where['id'] = $pg_id;
            $where1['sweepstakes_id'] = $pg_id;
        }
        $result = $this->table('sweepstakes')->where($where)->delete();
        //20170927潘丙福添加开始-删除抽奖活动对应的奖项
        $result1 = Model()->table('sweepstakes_award')->where($where1)->delete();
        //20170927潘丙福添加结束
        //删除抽奖活动的图片信息（转盘）
        if ($result && $result1){
    ...
    public function editPointProd($update_arr, $where){
        ...
        $result = $this->table('sweepstakes')->where($where)->update($update_arr);
        return $result;
    }
    /**
     * 获得推荐的热门兑换商品列表
