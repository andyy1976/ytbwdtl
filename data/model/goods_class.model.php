...
/**
     ...
     * )
     */
    public function getCachee() {
        if (!$data) {
            $data = array();
            foreach ((array) $this->getGoodsClassList(array('store_flag'=>'1')) as $v) {
                $id = $v['gc_id'];
                $pid = $v['gc_parent_id'];
                $data['data'][$id] = $v;
                $data['parent'][$id] = $pid;
    ...
    public function getCache() {
        ...
        if (!$data) {
            $data = array();
            foreach ((array) $this->getGoodsClassList(array('store_flag'=>'0')) as $v) {
                $id = $v['gc_id'];
                $pid = $v['gc_parent_id'];
                $data['data'][$id] = $v;
                $data['parent'][$id] = $pid;
    ...
    */
    public function getGoodsClassListByParentIdd($pid){
      $data = $this->getCachee();
      $ret = array();
...
/**
     ...
     * @param int $id 分类id
     */
    public function getGoodsClassInfoByIdd($id) {
        $data = $this->getCachee();
        return $data['data'][$id];
    }
    ...
    public function getGoodsClassNav($id = 0, $sign = 1) {
        if (intval ($id) > 0) {
            ...
            foreach ($hid as $key => $value) {
                $sll = Model('goods_class')->where(array('gc_id'=>$value))->find();
               $dmdaohang[$key]['title']=$sll['gc_name'];
               $dmdaohang[$key]['linkid']=$sll['gc_id'];
            }
            foreach ($dmdaohang as $key => $value) {
            $nav_link[] = array(
                        'title' => $dmdaohang[$key]['title'],
                        'link' => urlShop('search', 'index', array('cate_id' => $dmdaohang[$key]['linkid']))
                );
            }
...
/**
     ...
     * @return array $nav_link 返回数组形式类别导航连接
     */
    public function getGoodsClassLineForTagg($id = 0) {
        if (intval($id)> 0) {
            $gc_line = array();
            /**
             * 取当前类别信息
             */
            $class = $this->getGoodsClassInfoByIdd(intval($id));
            ...
            if ($class['gc_parent_id'] != 0) {
                $parent_1 = $this->getGoodsClassInfoByIdd($class['gc_parent_id']);
                if ($parent_1['gc_parent_id'] != 0) {
                    $parent_2 = $this->getGoodsClassInfoByIdd($parent_1['gc_parent_id']);
   ...
   private function foo($cid){     //地面商家导航递归
    $pids = '';
    $parent_id = Model('goods_class')->where(array('store_flag'=>1,'gc_id'=>$cid))->find();
    if($parent_id['gc_parent_id'] != 0){
        $pids .= $parent_id['gc_parent_id'];
        $npids =$this->foo($parent_id['gc_parent_id']);
        if(isset($npids))
            $pids .= ','.$npids;
    }
    return $pids;
