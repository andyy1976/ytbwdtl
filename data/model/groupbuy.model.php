...
class groupbuyModel extends Model{
     ...
     * @param string $field 所需字段
     * @return array 抢购列表
     * 潘丙福修改排序条件  默认就按照销量从高到低排序
     */
    public function getGroupbuyOnlineList($condition, $page = null, $order = 'buyer_count desc', $field = '*') {
        if (empty($order)) {
    ...
    //潘丙福添加
    public function getGroupbuyOnlineInfo11($condition) {
        $condition['state'] = self::GROUPBUY_STATE_NORMAL;
        // $condition['start_time'] = array('lt', TIMESTAMP);
        // $condition['end_time'] = array('gt', TIMESTAMP);
        $groupbuy_info = $this->where($condition)->find();
    ...
    public function delGroupbuy($condition){
        ...
        //20170728潘丙福添加开始--删除抢购活动时恢复商品原云豆
        $panGroupBuy = $this->where($condition)->find();
        Model()->table('goods')->where(array('goods_commonid' => $panGroupBuy['goods_commonid']))->update(array('goods_points' => $panGroupBuy['goods_points']));
        //20170728潘丙福添加结束
    ...
    public function getGroupbuyExtendInfo($groupbuy_info) {
        ...
        //20170804潘丙福添加开始--获取对应的运费
        $panGoodsFreight = Model()->table('goods')->field('goods_freight,transport_id')->find($groupbuy_info['goods_id']);
        if ($panGoodsFreight['transport_id'] == 0) {
            $groupbuy_info['goods_freight'] = $panGoodsFreight['goods_freight'];
        } else {
            $panCondition = array();
            $panCondition['transport_id'] = array('eq', $panGoodsFreight['transport_id']);
            $panGoodsTransport = Model()->table('transport_extend')->field('area_name,sprice')->where($panCondition)->select();
            $groupbuy_info['goods_freight'] = $panGoodsTransport;
        }
        //20170804潘丙福添加结束
