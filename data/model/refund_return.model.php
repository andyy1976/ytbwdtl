...
class refund_returnModel extends Model{
    ...
    public function editOrderRefund($refund,$user = '') {
        ...
        if ($refund_id > 0) {
            ...
            //20170716潘丙福添加字段--order_pointsamount
            $field = 'order_id,buyer_id,buyer_name,store_id,store_name,order_sn,order_amount,payment_code,order_state,refund_amount,rcb_amount,pd_amount,rpt_amount,shipping_fee,order_pointsamount';
            $model_order = Model('order');
            ...
            try {
                ...
                if ($predeposit_amount > 0) {//退预存款
                    ...
                    $log_array['member_name'] = $order['buyer_name'];
                    $log_array['order_sn'] = $order['order_sn'];
                    //20170716潘丙福添加开始-增加此订单的积分字段
                    $log_array['pan_points'] = $order['order_pointsamount'];
                    $log_array['refund_id'] = $refund_id;
                    //20170916潘丙福添加开始-增加此订单的状态字段
                    $log_array['pan_order_status'] = $order['order_state'];
                    $log_array['payment_code']     = $order['payment_code'];
                    //20170716潘丙福添加结束
                ...
                if ($state && $order_state == $order_paid) {
                    //20171103潘丙福添加开始-减去商家钱包1的货款
                    //因为订单状态是已付款未发货 申请退款肯定是全额退款
                    $dat['wallet_frozen']   = array('exp','wallet_frozen-'.$order_amount);
                    $panPaidLog = array(
                        'available_amount' => 0,
                        'freeze_amount'    => '-'.$refund_amount,
                        'description'      => '商品退款成功，商家钱包1扣除退款金额-'.$order['order_sn'],
                        'member_id'        => $order['store_id'],
                        'member_name'      => $order['store_name'],
                        'type'             => 'seller',
                        'add_time'         => time()
                    );
                    $store                   = Model('seller');
                    $rcb                     = Model('rcb_log'); 
                    $insertId                = $rcb->insert($panPaidLog);
                    $upstore                 = $store->where(array('store_id'=>$order['store_id']))->update($dat);
                    //20171103潘丙福添加结束
                ...
                if ($state) {
                    ...
                    if ($state && $order_state == $order_shipped && $refund_amount <= $order_refund_amount) {
                        $order_array = Model()->table('orders')->find($order_id);
                        Logic('order')->changeOrderStateReceive($order_array, 'system', $user, '商品全部退款，系统完成订单');
                    }
                    //20171031潘丙福修改--针对新的结款方式退款流程调整，同时兼顾以前的退款
                    //重新获取最新的订单状态
                    $tmpPanOrderInfo = Model()->table('orders')->field('order_id,platform_payment')->find($order_id);
                    if ($tmpPanOrderInfo['platform_payment'] == 3){
                        if ($state && $order_state >= $order_shipped && $refund_amount <= $order_refund_amount) {
                            //1.扣除钱包1$order_amount
                            //2.减去退款剩下的钱转移到钱包2（合并处理）
                            //3.日志处理
                            $dat['wallet_frozen']   = array('exp','wallet_frozen-'.$order_amount);
                            $panpanTmp = $order_amount-$refund_amount;
                            $dat['wallet_release']   = array('exp','wallet_release+'.$panpanTmp);
                            $panLog = array(
                                array(
                                    'available_amount' => '-'.$order_amount,
                                    'freeze_amount'    => '+'.$order_amount,
                                    'description'      => '由于商品退款，货款从钱包1转到钱包2-'.$order['order_sn'],
                                    'member_id'        => $order['store_id'],
                                    'member_name'      => $order['store_name'],
                                    'type'             => 'seller',
                                    'add_time'         => time(),
                                 ...
                                 array(
                                    'available_amount' => 0,
                                    'freeze_amount'    => '-'.$refund_amount,
                                    'description'      => '商品退款成功，商家钱包扣除退款金额-'.$order['order_sn'],
                                    'member_id'        => $order['store_id'],
                                    'member_name'      => $order['store_name'],
                                    'type'             => 'seller',
                                    'add_time'         => time(),
                            ...
                            $store                   = Model('seller');
                            $rcb                     = Model('rcb_log'); 
                            $insertId                = $rcb->insertAll($panLog);
                            $upstore                 = $store->where(array('store_id'=>$order['store_id']))->update($dat);  
                        }
                    } else {
                        if ($state && $order_state >= $order_shipped && $refund_amount <= $order_refund_amount) {
                            $dat['wallet_release']   = array('exp','wallet_release-'.$refund_amount);
                            $log['available_amount'] = '-'.$refund_amount;
                            $log['freeze_amount']    = 0;
                            $log['description']      = '商品退款成功，商家钱包扣除退款金额-'.$order['order_sn'];
                            $log['member_id']        = $order['store_id'];
                            $log['member_name']      = $order['store_name'];
                            $log['type']             = 'seller';
                            $log['add_time']         = time();
                            $store                   = Model('seller');
                            $rcb                     = Model('rcb_log'); 
