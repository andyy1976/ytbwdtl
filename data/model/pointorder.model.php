...
class pointorderModel extends Model{
    ...
    public function getPointOrderStateBySign(){
        //熊磊   2017-7-12  增加$pointorderstate_arr['zero'] = array(10,'待发货');
        $pointorderstate_arr = array();
        $pointorderstate_arr['canceled'] = array(2,'已取消');
        $pointorderstate_arr['zero'] = array(10,'处理中');
    ...
    public function getPointOrderState($order_state){
        ...
        $pointorderstate_arr[10] = array('zero','处理中');
    ...
    public function getPointOrderInfo($where, $field = '*', $order = '', $group = ''){
        ...
        //审核     熊磊增加代码   165-168行代码
        $tmp = $this->allowPointOrderaudit($order_info);
        $order_info['point_orderallowaudit'] = $tmp['state'];
        unset($tmp);
        //是否可以发货
    ...
    public function getPointOrderList($where, $field = '*', $page = 0, $limit = 0,$order = '', $group = '') {
        ...
        foreach ($order_list as $k=>$v){
            ...
            //审核、 熊磊增加代码   165-168行代码
            $tmp = $this->allowPointOrderaudit($v);
            $v['point_orderallowaudit'] = $tmp['state'];
            unset($tmp);
            //是否可以发货
     ...
     * @return array
     */
    public function allowPointOrderaudit($pointorder_info){
        if (!$pointorder_info){
            return array('state'=>false,'msg'=>'参数错误');
        }
        $pointorderstate_arr = $this->getPointOrderStateBySign();
        if ($pointorder_info['point_orderstate'] == $pointorderstate_arr['zero'][0]){
            return array('state'=>true);
        } else {
            return array('state'=>false);
        }
    ...
    public function cancelPointOrder($point_orderid, $member_id = 0) {
        ...
        if (!isset($_GET['admin'])) {
            //未发货时可取消
            $where['point_orderstate'] = $pointorderstate_arr['waitship'][0];            
        }
        ...
        // $order_info = $this->table('points_order')
        // ->field('point_orderid,point_ordersn,point_buyerid,point_buyername,point_allpoint')
        // ->where($where)
        // ->group('')->order('')->find();
        // print_r($where);
        // print_r($order_info);

        $result = $this->editPointOrder(
            array(
                'point_orderid' => $order_info['point_orderid'],
                'point_buyerid' => $order_info['point_buyerid']
            ),
            array(
                'point_orderstate' => $pointorderstate_arr['canceled'][0]
            )
        );
        ...
        if ($result){
            ...
            $insert_arr['pl_desc']          = '取消兑换礼品信息'.$order_info['point_ordersn'].'增加云豆';
    ...
    public function shippingPointOrder($order_id,$postarr,$order_info = array()){
        ...
        $where = array();
        $where['point_orderid'] = $order_id;
        //熊磊 2017-7-12 增加代码 631-633行
        $where['point_orderstate'] = array('in',array($pointorderstate_arr['waitship'][0],$pointorderstate_arr['waitreceiving'][0],$pointorderstate_arr['zero'][0]));//待发货和已经发货状态
        ...
        //更新发货信息
        $update_arr = array();
        //熊磊 2017-7-12 修改 $pointorderstate_arr['zero'][0] 状态10   原状态20
        if ($order_info['point_orderstate'] == $pointorderstate_arr['zero'][0]){
