...
class goodsModel extends Model{
     ...
     * @param str $panGroup 以谁分组（20170416潘丙福添加）
     * @return array|null
     */
    public function getGoodsGcStoreRandList($gcId, $storeId, $notEqualGoodsId = 0, $size = 4, $panGroup = null) {
        $condition = array(
        ...
        return $this->getGoodsOnlineList($condition, '*', 0, 'rand()', $size, $panGroup);
    }
     ...
     * 潘丙福20170609($condition['goods_verify']  = array('neq', self::VERIFY1);)
     * @param array $condition 条件
     * @param array $field 字段
     * @param string $page 分页
     * @param string $order 排序
    ...
    public function getGoodsCommonWaitVerifyList($condition, $field = '*', $page = 10, $order = "goods_commonid desc", $limit = '') {
        if (!isset($condition['goods_verify'])) {
            $condition['goods_verify']  = array('eq', self::VERIFY10);
        }
        return $this->getGoodsCommonList($condition, $field, $page, $order, $limit);
    }
	public function getGoodsCommonVerifyFailList($condition, $field = '*', $page = 10, $order = "goods_commonid desc", $limit = '') {
        if (!isset($condition['goods_verify'])) {
            $condition['goods_verify']  = array('eq', self::VERIFY0);
        }
        return $this->getGoodsCommonList($condition, $field, $page, $order, $limit);
    }
    /**
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
        //增加商品金额字段 goods_price
            $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_price', 0, 'rand()', $limit, 'goods_commonid');
            if (!empty($goods_id_list)) {
                $tmp = array();
                foreach ($goods_id_list as $v) {
                    $tmp[] = $v['goods_id'];
                }
                $goods_commend_list = $this->getGoodsOnlineList(array('goods_id' => array('in',$tmp)), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_price', 0, 'rand()', $limit);
            }
    ...
    public function editGoodsPromotionPrice($condition) {
        ...
        foreach ($goods_array as $key => $val) {
            ...
            //潘丙福添加20170518
            $groupbuy11 = $model_groupbuy->getGroupbuyOnlineInfo11(array('goods_commonid' => $key));
            if (!empty($groupbuy11)) {
                // 更新价格
                $this->editGoods(array('goods_promotion_price' => $groupbuy11['groupbuy_price'], 'goods_promotion_type' => 1, 'goods_points' => $groupbuy11['groupbuy_points']), array('goods_commonid' => $key, 'is_book' => 0));
                continue;
            }
            foreach ($val as $k => $v) {
                if ($v['is_book'] == '1') {
                ...
                //20170727潘丙福添加因为没有做定时任务查询的时候去掉时间限制--临时方法
                $xianshigoods = $model_xianshigoods->getXianshiGoodsInfo(array('goods_id' => $k));
                if (!empty($xianshigoods)) {
                    // 更新价格
                    $this->editGoodsById(array('goods_promotion_price' => $xianshigoods['xianshi_price'], 'goods_promotion_type' => 2, 'goods_points' => $xianshigoods['xianshi_points']), $k);
                    continue;
                }
    ...
    public function editProduces($condition, $update1, $update2 = array(), $updateXS = false) {
        ...
        //20170908潘丙福添加获取字段——is_own_shop（是否为自营商品）字段
        $goods_array = $this->getGoodsCommonList($condition, 'goods_commonid,is_own_shop', 0);
        ...
        foreach ($goods_array as $val) {
            $commonid_array[] = $val['goods_commonid'];
            //20170908潘丙福添加开始-判断是否为自营商品
            if ($val['is_own_shop'] == 1 && $update1['edit_counts']) {
                unset($update1['edit_counts']);
            }
            //20170908潘丙福添加结束
        }
	...
	//专用放大镜
        ...
        if ($goods_info['is_book'] != '1') {
            ...
            if (!empty($goods_info['xianshi_info'])) {
                ...
                $goods_info['promotion_price'] = $goods_info['xianshi_info']['xianshi_price'];
                //20170726潘丙福添加开始
                $goods_info['promotion_points'] = $goods_info['xianshi_info']['xianshi_points'];
                //20170726潘丙福添加结束
            ...
            if (!empty($goods_info['groupbuy_info'])) {
                ...
                $goods_info['promotion_price'] = $goods_info['groupbuy_info']['groupbuy_price'];
                //20170726潘丙福添加开始
                $goods_info['promotion_points'] = $goods_info['groupbuy_info']['groupbuy_points'];
                //20170726潘丙福添加结束
