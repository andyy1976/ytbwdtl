...
class orderModel extends Model {
     ...
     * 待付款订单数量
     * @param unknown $condition
     * 20170304潘丙福添加 修改当前id的未支付订单超过48小时状态修改为0
     */
    public function getOrderStateNewCount($condition = array()) {

        $condition['order_state'] = ORDER_STATE_NEW;
        $condition['add_time'] = array('lt',time()-172800);
        $this->table('orders')->where($condition)->update(array('order_state'=>'0'));
    ...
    public function getOrderStateSendCount($condition = array()) {
        ...
        $panUnReceive = $this->table('orders')->where($condition)->select();
        $store = Model('seller');
        $rcb   = Model('rcb_log');
        $percent = Model('chief');
