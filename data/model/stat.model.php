...
class statModel extends Model{
     ...
     * @return array
     */
    public function getYundou($where, $field = '*', $order = '', $group = '') {
        return $this->table('points_order')->field($field)->where($where)->group($group)->order($order)->find();
    }
     ...
     * @return array
     */
    public function yundouByStatorder($where, $field = '*', $page = 0, $limit = 0,$order = '', $group = '') {
        if (is_array($page)){
            if ($page[1] > 0){
                return $this->table('points_order')->field($field)->where($where)->group($group)->page($page[0],$page[1])->limit($limit)->order($order)->select();
            } else {
                return $this->table('points_order')->field($field)->where($where)->group($group)->page($page[0])->limit($limit)->order($order)->select();
            }
        } else {
            return $this->table('points_order')->field($field)->where($where)->group($group)->page($page)->limit($limit)->order($order)->select();
        }
    }
