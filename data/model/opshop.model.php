...
class opshopModel extends Model {
    ...
    public function getPdRechargeList($condition = array(), $pagesize = '', $fields = '*', $order = '', $limit = '') {
        return $this->table('opshop_order')->where($condition)->field($fields)->order($order)->limit($limit)->page($pagesize)->select();
    }
    ...
    public function editPdRecharge($data,$condition = array()) {
        return $this->table('opshop_order')->where($condition)->update($data);
    }
    ...
    public function getPdRechargeInfo($condition = array(), $fields = '*',$lock = false) {
        return $this->table('opshop_order')->where($condition)->field($fields)->lock($lock)->find();
    }
    ...
    public function changePd($change_type,$data = array()) {
        ...
        $til['pl_stage']='rechart';
        $poarrr=$poin->where($til)->sum('pl_points');
            
        $amont=$poarrr+$data['amount'];
        if($poarrr>='20000' || !empty($data['split_id'])){
            $amout=$data['amount']-$data['amount']*$draws;
        }
        elseif($amont>'20000')
        {
            $amout=$data['amount']-(20000-$poarrr)*$widthdraw-($data['amount']-20000+$poarrr)*$draws;
        } 
        else{
        ...
        switch ($change_type){
            ...
            case 'refund':
                ...
                $order_info=$order->where(array('order_sn'=>$data['order_sn']))->find();
                ...
                $data_pd['member_points'] = array('exp','member_points+'.$order_goods_info['goods_points']);
                $data_msg['av_amount'] = $data['amount'];
                $data_msg['freeze_amount'] = 0;
                $data_msg['desc'] = $data_log['lg_desc'];
                break;
    ...
    public function delPdRecharge($condition) {
        return $this->table('opshop_order')->where($condition)->delete();
    }
   ...
   */
   public function getPdCashMoneySum($condition, $field){
    ...
    public function addPdCash($data) {
        ...
        //增加
    public function addPdLog($condition){
        return $this->table('pd_log')->insert($condition);
    }
    /**
     * 编辑提现记录
     * @param unknown $data
