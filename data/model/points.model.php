...
class pointsModel extends Model {
    ...
    public function __construct(){
        parent::__construct();
        $this->stage_arr = array('regist'=>'注册','login'=>'登录','comments'=>'商品评论','order'=>'订单消费','system'=>'云豆管理','pointorder'=>'礼品兑换','app'=>'云豆兑换','signin'=>'签到');
    }
    /**
     * 操作云豆
     * @author 33Hao Develop Team
     * @param  string $stage 操作阶段 regist(注册),login(登录),comments(评论),order(下单),system(系统),other(其他),pointorder(云豆礼品兑换),app(同步云豆兑换)
     * @param  array $insertarr 该数组可能包含信息 array('pl_memberid'=>'会员编号','pl_membername'=>'会员名称','pl_adminid'=>'管理员编号','pl_adminname'=>'管理员名称','pl_points'=>'云豆','pl_desc'=>'描述','orderprice'=>'订单金额','order_sn'=>'订单编号','order_id'=>'订单序号','point_ordersn'=>'云豆兑换订单编号');
    ...
    function savePointsLog($stage,$insertarr,$if_repeat = true){
        ...
        switch ($stage){
            ...
            case 'pointorder':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '兑换礼品信息'.$insertarr['point_ordersn'].'消耗云豆';
                }
                break;
            case 'app':
                if (!$insertarr['pl_desc']){
            ...
            case 'signin':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '签到领云豆';
                }
				break;
			//邀请云豆返利 33 hao.co m v5
			case 'inviter':
				if (!$insertarr['pl_desc']){
        ...
        if ($result){
            ...
            //生成云豆余额安全码
            $obj_member_info=$obj_member->where(array('member_id'=>$insertarr['pl_memberid']))->find();
            $obj_points=$obj_member_info['member_points']+$insertarr['pl_points'];
            $points_array=['id'=>$data['member_id'],'amt'=>$obj_points];
            $points_code = Ze\Secure::encode($points_array);
            $upmember_array = array();
            $upmember_array['points_code']=$points_code;
