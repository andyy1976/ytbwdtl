...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=school_article&op=article" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3>商学院<?php echo $lang['article_index_manage'];?> - <?php echo $lang['nc_edit'];?>文章“<?php echo $output['article_array']['article_title'];?>”</h3>
        <h5>商学院<?php echo $lang['article_index_manage_subhead'];?></h5>
      </div>
    </div>
  </div>
  <form id="article_form" method="post">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        <dt class="tit">
          <label for="article_blurb"><em>*</em>文章描述</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $output['article_array']['article_blurb'];?>" name="article_blurb" id="article_blurb" class="input-txt">
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
          <label for="article_keywords"><em>*</em>文章关键字</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $output['article_array']['article_keywords'];?>" name="article_keywords" id="article_keywords" class="input-txt">
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl>
...
<!--       <dl class="row">
        <dt class="tit">
      <?php  if($output['article_array']['ac_id']!='9'){ ?>
        ...
        <dd class="opt">
           <?php   if($output['article_array']['ac_id']!='9'){showEditor('article_content',$output['article_array']['article_content']);}?> 
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl>
    <dl class="row" style="display:none" id='ke'>
        <dt class="tit"></dt>
        <dd class="opt">
          <input type="text" value="" name="article_pri" id="article_pri" class="input-txt">
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl> -->
      ...
      <dl class="row">
        <dt class="tit">文章缩略图</dt>
        <dd class="opt">
          <div class="input-file-show" id="divComUploadContainer"><span class="type-file-box">
            <input class="type-file-file" id="fileupload_img" name="fileupload_img" type="file" size="30" multiple hidefocus="true" title="点击按钮选择文件上传">
            <input type="text" name="text" id="text" class="type-file-text" />
            <input type="button" name="button" id="button" value="选择上传..." class="type-file-button" />
            </span></div>
          <div id="thumbnails" class="ncap-thumb-list">
            <h5><i class="fa fa-exclamation-circle"></i>上传的图片显示在封面上</h5>
            <!-- <ul>
        ...
        <input type="hidden" name="article_img" id="article_img" value="<?php echo $output['article_array']['article_img'];?>">
      </dl> 
          ...
          <div id="thumbnails" class="ncap-thumb-list">
            ...
            <ul>
              ...
              <li id="<?php echo $v['upload_id'];?>">
                <input type="hidden" name="file_id[]" value="<?php echo $v['upload_id'];?>" />
                <div class="thumb-list-pics"><a href="javascript:void(0);"><img src="<?php echo $v['upload_path'];?>" alt="<?php echo $v['file_name'];?>"/></a></div>
                <a href="javascript:del_file_upload('<?php echo $v['upload_id'];?>');" class="del" title="<?php echo $lang['nc_del'];?>">X</a><a href="javascript:insert_editor('<?php echo $v['upload_path'];?>');" class="inset"><i class="fa fa-trash"></i>插入图片</a> 
                <!-- 头条栏目 -->
 <!--                <?php if($output['article_array']['ac_id']=='9'){ ?>
                <a href="javascript:insert_editors('<?php echo $v['upload_path'];?>');" class="inset"><i class="fa fa-trash"></i>插入图片</a>
                <?php }?> -->
...
$(document).ready(function(){
	...
	$('#article_form').validate({
      ...
      article_keywords : {
       ...
       article_blurb : {
                required : '<i class="fa fa-exclamation-circle"></i>文章描述不能为空'
            },
      article_keywords : {
                required : '<i class="fa fa-exclamation-circle"></i>文章关键字不能为空'
            },
			ac_id : {
    ...
    $('#fileupload').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?act=school_article&op=article_pic_upload&item_id=<?php echo $output['article_array']['article_id'];?>',
    ...
    $('#fileupload_img').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?act=school_article&op=article_img_upload&item_id=<?php echo $output['article_array']['article_id'];?>',
            done: function (e,data) {
                if(data != 'error'){
                  var img = data.result.file_path;
                  $('#article_img').val(img);
                }
            }
        });
...
function insert_editors(file_path){
  var  ml = file_path;
  var ls = $('#article_pri').val(ml);
}
function del_file_upload(file_id)
{
    ...
    $.getJSON('index.php?act=school_article&op=ajax&branch=del_file_upload&file_id=' + file_id, function(result){
        if(result){
            $('#' + file_id).remove();
        }else{
