...
class memberControl extends SystemControl{
		...
		if (!chksubmit()){
          ...
          Tpl::showpage('member.predeposit.add');
		   ...
		   $obj_validate = new Validate();
            ...
            $member_predeposit=floatval($member_info['member_predeposit']);
            $freeze_predeposit=floatval($member_info['freeze_predeposit']);
            if ($_POST['operatetype'] == 2 && $money > $member_predeposit){
                showMessage(('预存款不足，会员当前充值余额').$member_predeposit,'index.php?act=member&op=predeposit_add','','error');
            }
            if ($_POST['operatetype'] == 3 && $money > $member_predeposit){
                showMessage(('可冻结预存款不足，会员当前充值余额').$member_predeposit,'index.php?act=member&op=predeposit_add','','error');
            }
            ...
            $log_msg = "操作会员[".$member_info['member_name']."]充值余额，金额为".$money.",编号为".$order_sn;
            $admin_act="sys_add_money";
            switch ($_POST['operatetype'])
            {
                case 1:
                    $admin_act="sys_add_money";
                    $log_msg = "操作会员[".$member_info['member_name']."]充值余额[增加]，金额为".$money.",编号为".$order_sn;
                    break;  
                case 2:
                    $admin_act="sys_del_money";
                    $log_msg = "操作会员[".$member_info['member_name']."]充值余额[减少]，金额为".$money.",编号为".$order_sn;
                    break;  
                case 3:
                    $admin_act="sys_freeze_money";
                    $log_msg = "操作会员[".$member_info['member_name']."]充值余额[冻结]，金额为".$money.",编号为".$order_sn;
                    break;  
                case 4:
                    $admin_act="sys_unfreeze_money";
                    $log_msg = "操作会员[".$member_info['member_name']."]充值余额[解冻]，金额为".$money.",编号为".$order_sn;
                    break;
                default:
	...
	public function checkmemberOp(){
		...
		if (is_array($member_info) && count($member_info)>0){
			// if(strtoupper(CHARSET) == 'GBK'){
			// 	$member_info['member_id'] = Language::getUTF8($member_info['member_name']);
			// }
			echo json_encode(array('id'=>$member_info['member_id'],'name'=>$member_info['member_name'],'member_predeposit'=>$member_info['member_predeposit'],'freeze_predeposit'=>$member_info['freeze_predeposit']));
		}else {
			echo ''; die;
		}
	}
	...
	//后台给会员充值	
    public function member_add_moneyOp(){
       if(intval($_POST['moneys'])>0 && intval($_POST['memberid'])){
			$uid=intval($_POST['memberid']);
			$money=	intval($_POST['moneys']);		
			$model_member = Model('member');
			$model_point = Model('points_log');
			$model_member->where("member_id=$uid")->setInc('available_predeposit',$money);
			$model_member->where("member_id=$uid")->setInc('member_points',$money);	//冲多少钱就送多少个云豆
			$model_member->where("member_id=$uid")->setInc('total_recharge',$money);//更新累计充值金额
			$info=get_member_info($uid);
			$name=$info['member_name'];
			//插入云豆记录
			$data=array('pl_memberid'=>$uid,'pl_membername'=>$name,'pl_points'=>$money,'pl_addtime'=>time(),'pl_desc'=>'充值送云豆','pl_stage'=>'recharge');
			$model_point->insert($data);
			echo "充值成功";							
		}else{
			echo "充值失败";
		}		
    ...
    //提成设置处理
    public function member_percentageOp(){
    	$percent=Model('chief'); 
    	if($_POST){  	
		...
		Tpl::showpage('member.percentage');	
    ...
    public function member_levelOp(){
    	...
    	$uid=$_POST['memberid'];    	
    	$model_member = Model('member');
    	$a=$model_member->where(array('member_id'=>$uid))->update(array('member_level'=>$level));  
    	if($a){
    		echo "设置成功";
    	}else{
    		echo "设置失败";
