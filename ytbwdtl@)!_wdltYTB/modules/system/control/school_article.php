...
class school_articleControl extends SystemControl{
    ...
    public function articleOp(){
        ...
        Tpl::showpage('school_article.index');
    }
    ...
    public function get_xmlOp(){
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?act=school_article&op=article_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function article_addOp(){
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                ...
                array("input"=>$_POST["article_blurb"], "require"=>"true", "message"=>'文章描述不能为空'),
                array("input"=>$_POST["article_keywords"], "require"=>"true", "message"=>'文章关键字不能为空'),
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                $insert_array['article_blurb'] = trim($_POST['article_blurb']);
                $insert_array['article_keywords'] = trim($_POST['article_keywords']);
                $insert_array['article_img'] = trim($_POST['article_img']);
                $insert_array['is_school'] = 1;
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?act=school_article&op=article',
                            'msg'=>"{$lang['article_add_tolist']}",
                        ),
                        array(
                            'url'=>'index.php?act=school_article&op=article_add&ac_id='.intval($_POST['ac_id']),
        ...
        $model_class = Model('article_class');
        //$parent_list = $model_class->getTreeClassList(2);
        $parent_list = Model()->table('article_class')->where(array('ac_parent_id'=>68))->select();
        if (is_array($parent_list)){
            $unset_sign = false;
            foreach ($parent_list as $key => $value) {
                $parent_list[$key]['deep'] = 1;
            }
            foreach ($parent_list as $k => $v){
                $parent_list[$k]['ac_name'] = str_repeat("&nbsp;",$v['deep']*2).$v['ac_name'];
            }
		...
		Tpl::setDirquna('system');
        Tpl::showpage('school_article.add');
    }
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                $update_array['article_blurb'] = trim($_POST['article_blurb']);
                $update_array['article_keywords'] = trim($_POST['article_keywords']);
                $update_array['article_img'] = trim($_POST['article_img']);
                // 添加的头条公告
                // if($update_array['ac_id'] =='9'){
                //         $update_array['article_content'] = trim($_POST['article_pri']);
                // }
                ...
                if ($result){
                    ...
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?act=school_article&op=article_edit&article_id='.intval($_POST['article_id']),
		...
		Tpl::setDirquna('system');
        Tpl::showpage('school_article.edit');
    }
    /**
     * 文章图片上传
     */
     ...
     * 文章图片上传
     */
    public function article_img_uploadOp(){
        /**
         * 上传图片
         */
        $upload = new UploadFile();
        $upload->set('default_dir',ATTACH_ARTICLE);
        $result = $upload->upfile('fileupload_img');
        if ($result){
            $_POST['pic'] = $upload->file_name;
        }else {
            echo 'error';exit;
