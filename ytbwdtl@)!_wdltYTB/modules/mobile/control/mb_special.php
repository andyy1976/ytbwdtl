...
Tpl::showpage('mb_special_item.list');
   ...
   /* public function special_item_saveOp() {
     ...
     public function special_item_saveOp() {
        // var_dump($_POST['item_data']);exit;
        $model_mb_special = Model('mb_special');
        //20170524潘丙福添加开始
        //手机端商品排序功能添加
        if (isset($_POST['item_data']['goodspower'])) {
            $panGoodsPower = array();
            $item = $_POST['item_data']['item'];
            $goodspower = $_POST['item_data']['goodspower'];
            foreach ($item as $key => $value) {
                $panGoodsPower[] = array('goods_id' => $value, 'goods_power' => empty($goodspower[$key]) ? 0 : $goodspower[$key]); 
            }
            foreach ($panGoodsPower as $key1 => $value1) {
                Model()->table('goods')->update($value1);
            }
            unset($_POST['item_data']['goodspower']);
            $result = $model_mb_special->editMbSpecialItemByID(array('item_data' => $_POST['item_data']), $_POST['item_id'], $_POST['special_id']);
        } else {
            
            if (isset($_POST['item_data']['item'])) {
                $newItemData = array();
                foreach ($_POST['item_data']['item'] as $key => $value) {
                    $tmpPower = $value['power'];
                    unset($value['power']);
                    $newItemData['item'][$tmpPower] = $value;
                }
                krsort($newItemData['item']);
                $newItemData['title'] = $_POST['item_data']['title'];
                $result = $model_mb_special->editMbSpecialItemByID(array('item_data' => $newItemData), $_POST['item_id'], $_POST['special_id']);
            } else {
                $result = $model_mb_special->editMbSpecialItemByID(array('item_data' => $_POST['item_data']), $_POST['item_id'], $_POST['special_id']);
            }
        }
