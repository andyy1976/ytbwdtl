  ...
  <form id="user_form" enctype="multipart/form-data" method="post">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <?php echo member_level_name($output['member_array']['member_level']);?>
        </dd>
      </dl>
      <!------AJAX会员级别设置------>
      <dl class="row">
        ...
        <dd class="opt">
          <select name="daili" id="jibie">
	          	<option value="0">请选择</option>	
	          	<option value='1'>端口</option><option value='2'>区县代理</option><option value='3'>市级代理</option><option value='4'>省级代理</option>          	
	          	<?php 
	          		/*
	          		//只显示比当前会员等级更高的选项
		        		if($output['member_array']['member_level']==0){
		        			echo "<option value='1'>端口</option><option value='2'>区县代理</option><option value='3'>市级代理</option><option value='4'>省级代理</option>";
		        		}elseif($output['member_array']['member_level']==1){
		        			echo "<option value='2'>区县代理</option><option value='3'>市级代理</option><option value='4'>省级代理</option>";
		        		}elseif($output['member_array']['member_level']==2){
		        			echo "<option value='3'>市级代理</option><option value='4'>省级代理</option>";
		        		}elseif($output['member_array']['member_level']==3){
		        			echo "<option value='4'>省级代理</option>";
		        		}
		        		*/
        		 ?>	          	
	          </select>
	          <span id='queding' style="background-color:#1BBC9D ; margin-left: 10px; color: #FFF; width:60px ; height:25px ; display: inline-block; text-align: center; cursor:pointer;" >确定</span>
        </dd>
      </dl>
      <!------------>
      <dl class="row">
...
$("#queding").click(function(){	
	var level=$("#jibie").val();	
	var uid="<?php echo $output['member_array']['member_id'];?>";	
  if(level==0){
  	showDialog('请选择级别', 'error','','','','','','','','',3);
  	exit;
  }			
	$.post('index.php?act=member&op=member_level',{memberid:uid,levels:level},function(result){
    showDialog(result, 'succ','','','','','','','','',3);
    <?php sleep(3) ;?>
    location.reload() ;
  });
})
