...
class stat_memberControl extends SystemControl{
    ...
    public function newmemberOp(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            $stat_arr['series'][0]['name'] = date('Y-m-d',$stime);
            $stat_arr['series'][0]['data'] = array_values($up_arr);
            $stat_arr['series'][1]['name'] = date('Y-m-d',$etime);
            $stat_arr['series'][1]['data'] = array_values($curr_arr);

            //统计数据标题
            $statlist['headertitle'] = array('小时',date('Y-m-d',$stime),date('Y-m-d',$etime),'同比');
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            $stat_arr['series'][0]['name'] = $up_month;
            $stat_arr['series'][0]['data'] = array_values($up_arr);
            $stat_arr['series'][1]['name'] = $curr_month;
            $stat_arr['series'][1]['data'] = array_values($curr_arr);
            //统计数据标题
            $statlist['headertitle'] = array('日期',$up_month,$curr_month,'同比');
    ...
    public function scaleOp(){
        ...
        $field = ' statm_memberid, min(statm_membername) as statm_membername, min(statm_time) as statm_time, SUM(statm_orderamount) as orderamount, SUM(statm_pointsamount) as pointsamount, SUM(statm_predincrease) as predincrease, -SUM(statm_predreduce) as predreduce, SUM(statm_pointsincrease) as pointsincrease, -SUM(statm_pointsreduce) as pointsreduce ';
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $excel_data[0][] = array('styleid'=>'s_title','data'=>'增云豆');
            $excel_data[0][] = array('styleid'=>'s_title','data'=>'减云豆');
            //data
    ...
    public function get_scale_xmlOp(){
        ...
        $field = ' statm_memberid, min(statm_membername) as statm_membername, min(statm_time) as statm_time, SUM(statm_orderamount) as orderamount, SUM(statm_pointsamount) as pointsamount, SUM(statm_predincrease) as predincrease, -SUM(statm_predreduce) as predreduce, SUM(statm_pointsincrease) as pointsincrease, -SUM(statm_pointsreduce) as pointsreduce ';
        //排序
        $order_type = array('statm_membername','orderamount','pointsamount','predincrease','predreduce','pointsincrease','pointsreduce');
        ...
        $format_array = array('orderamount','pointsamount','predincrease','predreduce');
        $statlist = array();
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
    ...
    public function member_mapOp(){
        ...
        $orderby= 'membernum desc';
         ...
         $statlist_tmp = $model->statByMember($where, $field, 0,$orderby, 'member_provinceid');
        ...
        foreach ((array)$statlist_tmp as $k => $v){
            ...
            $province_id = intval($v['member_provinceid']);
            $statlist[$province_id] = $v;
        }
        $stat_arr = array();
        foreach ((array)$province_array as $k => $v){
            if ($statlist[$k]){
                switch ($_GET['type']){
                   ...
                   case 'ordernum':
                       ...
                       break;
                   default:
                       $des = "，新增会员数：{$statlist[$k]['membernum']}";
                       break;
                }
                $stat_arr[] = array('cha'=>$k,'name'=>$v,'des'=>$des,'level'=>$statlist[$k]['level']);
            } else {
