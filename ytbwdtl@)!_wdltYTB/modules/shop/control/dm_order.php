...
class dm_orderControl extends SystemControl{
    /**
     * 每次导出订单数量
     * @var int
     */
   ...
   public function get_xmlOp(){
        ...
        $condition  = array();
        $condition['is_dm']=array('eq',2);    //实体订单与地面订单分开
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?act=dm_order&op=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            if ($order_info['if_system_receive_pay']) {
                ...
                $operation_detail .= "<li><a href=\"index.php?act=dm_order&op=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">{$op_name}</a></li>";
            }
			if($order_info['order_state']=='40'){
				 $operation_detail .= "<li><a href=\"index.php?act=dm_order&op=change_state&state_type=pay_ok&order_id={$order_info['order_id']}\">帮助买家确认订单</a></li>";
				}
            if ($operation_detail) {
                $list['operation'] .= "<span class='btn'><em><i class='fa fa-cog'></i>设置 <i class='arrow'></i></em><ul>{$operation_detail}</ul>";
            }
            $sti =($time-$order_info['payment_time'])/(24*3600);
            if($sti >='2' && $order_info['order_state'] == '20'){
                 $list['operation'] .= "<a class=\"btn green\"  href=\"javascript:void(0);\" onclick=\"get_msg({$order_info['store_id']})\"><i class=\"fa fa-list-alt\"></i>提醒商家</a>";
            }
    ...
    public function change_stateOp() {
        ...
        }elseif($_GET['state_type']=='pay_ok'){
			$result = $this->ordersuccess($order_id);
    ...
    public function export_step1Op(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?act=dm_order&op=index');
                Tpl::setDirquna('shop');
    ...
    private function _get_condition(& $condition) {
        if ($_REQUEST['query'] != '' && in_array($_REQUEST['qtype'],array('order_sn','store_name','buyer_name','buyer_id','pay_sn'))) {
        ...
        $time = time()-172800;
        if($condition['order_state'] ==50){
            $condition['order_state'] = 20;
            $condition['payment_time'] = array('lt',$time);
        }
       ...
       public  function get_smsOp()
    {
        $model = model('seller');
        $member = model('member');

        $store_id = $_GET['store_id'];
        $order_sn = "您好，你有一个订单为：".$_GET['order_sn'].'订单号没有发货请及时发货';

        if(is_numeric($store_id)){
            $store_info = $model->getSellerInfo(array('seller_id'=>$store_id));
            // $member_mobile = $member->where(array('member_id'=>$store_info['member_id']))->find();
            $member_id = $store_info['member_id'];
            $time = time();
            $member_info = $member ->getMemberInfo(array('member_id'=>$member_id));
            if($member_info['member_mobile']){
                $sms = new sms();
                $result = $sms->send($member_info["member_mobile"],$order_sn);
                $msg_info =  $member->getMemberCommonInfo(array('member_id'=>$member_id));
                if($msg_info){
                    $array = array('auth_code'=>$order_sn,'send_acode_time'=>time());
                    $where['member_id'] = $member_id;
                    $results = $member->editMemberCommon($array,$where);
                      if($result && $results){
                            exit(json_encode(array('state'=>$_GET['order_id'],'msg'=>'已经提醒商家发货')));
                        }else{
                            exit(json_encode(array('state'=>false,'msg'=>'提醒失败或者是商家没绑定手机号码无法提醒1')));
                        }
                }else{
                    $array = array('member_id'=>$member_id,'auth_code'=>$order_sn,'send_acode_time'=>time());
                    $member->addMemberCommon($array);
                }
                if($result){
                   exit(json_encode(array('state'=>$_GET['order_id'],'msg'=>'已经提醒商家发货')));
                }else{
                    ...
                    exit(json_encode(array('state'=>false,'msg'=>'提醒失败或者是商家没绑定手机号码无法提醒')));
            }
        }
    }
	
	
   private function orderState($order_info) {
...
private function ordersuccess($order_id){
	
	if(!empty($order_id)){
       $model_order = Model('order');
       $ydmbs = Model()->table('orders')->where(array('order_id'=>$order_id))->find();
   if($ydmbs['order_state']!=50&&$ydmbs['order_state']==ORDER_STATE_SUCCESS){
       $order_goods = $model_order->getOrderGoodsInfoArray(array('order_id'=>$order_id));
       $store = Model('seller');
       $rcb   = Model('rcb_log');                      
            //冻结钱包加订单的50%--新结算方式
            $dat['wallet_frozen']=array('exp','wallet_frozen+'.$ydmbs['order_amount']*0.5);
            $log['available_amount']=$ydmbs['order_amount']*0.5;
            $log['description']='地面买家确认收货--商家获得50%佣金-订单号：'.$ydmbs['order_sn'];
            $dat['wallet_release']=array('exp','wallet_release+'.$ydmbs['order_amount']*0.5);
            $log['member_id']=$ydmbs['store_id'];
            $log['member_name']=$ydmbs['store_name'];
            $log['type']='seller';
            $log['add_time']=time();
            $log['freeze_amount']=$ydmbs['order_amount']*0.5;
            $rcb->insert($log);                       
            ...
            $data['log_role'] = "系统";
            $data['log_msg'] = $this->admin_info['name']."系统管理员确认订单";
 ...
 foreach($order_goods as $b=>$c){
    ...
    $data=array(
        ...
        $dmbs = Model()->table('orders')->where(array('order_id'=>$order_id))->update($data);
        if($dmbs){
        return true;
        }else{
          return callback(false,'操作不成功！');
        }
	}
