...
EOB;
    ...
    public function groupbuy_review_passOp(){
        ...
        var_dump($panGoodsInfo);exit;
        $panUpdate = Model('goods')->editGoodsPromotionPrice(array('goods_commonid' => array('in', $panGoodsInfo['goods_commonid'])));
        if (!$panUpdate) {
    ...
    public function goods_verifyOp(){
        if (chksubmit()) {
            ...
            $update1 = array();
            $update1['goods_remark'] = trim($_POST['verify_reason']);
            $where = array();
            $where['goods_commonid'] = $commonid;

            $model_goods = Model('groupbuy');
            if (intval($_POST['verify_state']) == 0) {
                $update1['state'] = '30';
                $model_goods->editGroupbuy($update1, $where);
            } else {
                $update1['state'] = '20';
                $model_goods->editGroupbuy($update1, $where);
            }
