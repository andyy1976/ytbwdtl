...
class memberControl extends SystemControl{
    ...
    public function member_editOp(){
        ...
        if (chksubmit()){
            ...
            // }else {
                ...
                $update_array['member_bankcard']    = $_POST['member_bankcard'];
    ...
    public function get_xmlOp() {
        ...
        if ($_POST['query'] != '') {
            if (trim($_POST['qtype']) == 'member_id') {
                $condition['member_id'] = $_POST['query'];
            } else if (trim($_POST['qtype']) == 'member_bankcard') {
                $condition['member_bankcard'] = $_POST['query'];
            } else {
                $condition['member_name'] = array('like', '%' . $_POST['query'] . '%');
            }
            
            // $condition['member_mobile'] =$_POST['query'];
            // $condition['_op']='or';
        }
        $order = '';
        $param = array('member_id','member_name','member_avatar','member_email','member_mobile','member_level','member_provinceid','member_cityid','member_areaid','member_truename','member_bankcard','member_time','member_login_time','member_login_ip','member_points','member_exppoints','member_grade','available_predeposit','freeze_predeposit','available_rc_balance','freeze_rc_balance','inform_allow','is_buy','is_allowtalk','member_state'
        );
        ...
        //20170521潘丙福获取等级数组
        $memberLevel_array = $this->get_memberLevel();
        //20170521潘丙福获取区域数组
        $area_array = $this->get_area();
        ...
        foreach ($member_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn blue' href='index.php?act=member&op=member_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a><a class='btn blue' href='/member/index.php?act=login&op=login&user_name=".$value['member_name']."&password=".$value['member_passwd']."&admin=1' target='_blank'><i class='fa fa-pencil-square-o'></i>登陆</a>";
            ...
            $param['member_level'] = $memberLevel_array[$value['member_level']];
            $param['member_provinceid'] = $area_array[$value['member_provinceid']];
            $param['member_cityid'] = $area_array[$value['member_cityid']];
            $param['member_areaid'] = $area_array[$value['member_areaid']];
            ...
            // 20170918潘丙福添加--增加会员银行卡号字段
            $param['member_bankcard'] = $value['member_bankcard'];
    ...
    {
        $array = array();
        $tmp = Model('area')->getAreaList(array(), 'area_id,area_name');
        foreach ($tmp as $value) {
            $array[$value['area_id']] = $value['area_name'];
        }
        return $array;
     ...
     * 会员代理等级
     */
    private function get_memberLevel()
    {
        $array = array();
        $array[0] = '待激活';
        $array[1] = '普通会员';
