...
class stat_tradeControl extends SystemControl{
    ...
    public function get_plat_saleOp(){
        ...
        if(trim($_GET['order_type']) != ''){
            ...
            //20170803潘丙福添加--订单状态 > 10
            $where['order_state'] =  array('gt', 10);
        }
        if(trim($_GET['store_name']) != ''){
            $where['store_name'] = array('like','%'.trim($_GET['store_name']).'%');
        }
        //20170929潘丙福添加开始--云豆统计字段
        $statcount_arr = $model->getoneByStatorder($where,' COUNT(*) as ordernum, SUM(order_amount) as orderamount, SUM(order_pointsamount) as pointsamount');
        //20170729潘丙福添加结束
		echo '<div class="title"><h3>订单情况一览</h3></div>';
		echo '<dl class="row"><dd class="opt"><ul class="nc-row">';
		echo '<li title="有效总销售额：'. number_format(($statcount_arr['orderamount']),2).'元"><h4>有效总销售额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$statcount_arr['orderamount'].'"></h2><h6>元</h6></li>';
		echo '<li title="有效总订单量：'. intval($statcount_arr['ordernum']).'笔"><h4>有效总订单量</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'. $statcount_arr['ordernum'].'"></h2><h6>笔</h6></li>';
        //20170729潘丙福添加开始-云豆统计
        echo '<li title="有效总消费云豆：'. number_format(($statcount_arr['pointsamount']),2).'个"><h4>有效总消费云豆</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'. $statcount_arr['pointsamount'].'"></h2><h6>个</h6></li>';
        //20170729潘丙福添加结束
		echo '</ul></dd><dl>';
        exit();
    }
