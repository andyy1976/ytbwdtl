...
class refunddmControl extends SystemControl{
    const EXPORT_SIZE = 1000;
    private $links = array(
            array('url'=>'act=refunddm','text'=>'待处理'),
            array('url'=>'act=refunddm&op=refund_all','text'=>'所有记录'),
            array('url'=>'act=refunddm&op=reason','text'=>'退款退货原因')
    );
    ...
    public function get_manage_xmlOp() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=refunddm&op=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function get_all_xmlOp() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            if ($refund_info['refund_state'] == 2 || $refund_info['refund_state']==1) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=refunddm&op=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            }
            $list['operation'] .= "<a class=\"btn green\" href=\"index.php?act=refunddm&op=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $refund_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($refund_info['refund_amount']);
            if(!empty($refund_info['pic_info'])) {
                $info = unserialize($refund_info['pic_info']);
        ...
        if (chksubmit()) {
            ...
            if($_POST['shenhe'] == '2')
            {
                ...
                if ($state) {
                    ...
                    showMessage(Language::get('nc_common_save_succ'),'index.php?act=refunddm&op=index');
                } else {
                    showMessage(Language::get('nc_common_save_fail'));
                }
            }else{
                ...
                $refund_array['admin_time'] = time();
                $refund_array['admin_message'] = $_POST['admin_message'];
                $refund_array['refund_state'] = '4';//状态:1为处理中,2为待管理员处理,3为已完成,4为拒绝


                $state = $model_refund->editRefundReturn($condition, $refund_array);
                if ($state) {
                    showDialog(Language::get('nc_common_save_succ'),'index.php?act=refunddm&op=index');
                } else {
    ...
    public function add_reasonOp() {
        ...
        if (chksubmit()) {
            ...
            $reason_array['is_dm'] = intval($_POST['is_dm']);
            $state = $model_refund->addReason($reason_array);
            if ($state) {
                $this->log('新增退款退货原因，编号'.$state);
                showMessage(Language::get('nc_common_save_succ'),'index.php?act=refunddm&op=reason');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
    ...
    public function del_reasonOp() {
        ...
        if ($state) {
            ...
            showMessage(Language::get('nc_common_del_succ'),'index.php?act=refunddm&op=reason');
        } else {
