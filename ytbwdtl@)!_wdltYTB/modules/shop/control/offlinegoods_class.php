...
class offlinegoods_classControl extends SystemControl{
    private $links = array(
        array('url'=>'act=offlinegoods_class&op=goods_class','lang'=>'nc_manage'),
        array('url'=>'act=offlinegoods_class&op=goods_class_import','lang'=>'goods_class_index_import'),
        array('url'=>'act=offlinegoods_class&op=tag','lang'=>'goods_class_index_tag')
    );
    ...
    public function goods_classOp(){
        ...
        //列表
        //20170802潘丙福添加开始--store_flag = 1代表线下商品分类
        $condition   = array();
        $condition['store_flag'] = array('eq', 1);
        //20170802潘丙福添加结束
        $tmp_list = $model_class->getTreeClassList(3,$condition);
        ...
        if ($gc_id > 0){
            ...
            Tpl::setDirquna('shop');
            Tpl::showpage('offlinegoods_class.child_list');
        } else {
            Tpl::output('class_list',$class_list);
            Tpl::output('top_link',$this->sublink($this->links,'goods_class'));
            Tpl::setDirquna('shop');
            Tpl::showpage('offlinegoods_class.index');
        }
    }
    ...
    public function goods_class_addOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?act=offlinegoods_class&op=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
                            'msg'=>$lang['goods_class_add_again'],
                        ),
                        array(
                            'url'=>'index.php?act=offlinegoods_class&op=goods_class',
        ...
        //20170802潘丙福添加结束
        //父类列表，只取到第二级
        $parent_list = $model_class->getTreeClassList(2,$condition);
        $gc_list = array();
        ...
        Tpl::setDirquna('shop');
        Tpl::showpage('offlinegoods_class.add');
    }
    ...
    public function goods_class_editOp(){
        ...
        if (chksubmit()){
            ...
            $url = array(
                array(
                    'url'=>'index.php?act=offlinegoods_class&op=goods_class_edit&gc_id='.intval($_POST['gc_id']),
        ...
        $this->links[] = array('url'=>'act=offlinegoods_class&op=goods_class_edit','lang'=>'nc_edit');
        Tpl::output('top_link',$this->sublink($this->links,'goods_class_edit'));
                        
        Tpl::setDirquna('shop');
        Tpl::showpage('offlinegoods_class.edit');
    }
    ...
    public function goods_class_importOp(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
                ...
                showMessage($lang['nc_common_op_succ'],'index.php?act=offlinegoods_class&op=goods_class');
            }else {
    ...
    public function goods_class_exportOp(){
        ...
        $condition['store_flag'] = array('eq', 1);
        //20170802潘丙福添加结束
        $class_list = $model_class->getTreeClassList(3,$condition);
    ...
    public function tag_resetOp(){
        ...
        if(!$return){
            showMessage($lang['goods_class_reset_tag_fail'], 'index.php?act=offlinegoods_class&op=tag');
        }
        ...
        $condition['store_flag'] = array('eq', 1);
        //20170802潘丙福添加结束
        $goods_class        = $model_class->getTreeClassList(3,$condition);
        //格式化分类。组成三维数组
        if(is_array($goods_class) and !empty($goods_class)) {
            ...
            if($return){
                ...
                showMessage($lang['nc_common_op_succ'], 'index.php?act=offlinegoods_class&op=tag');
            }else{
                $this->log(L('nc_reset').'tag',0);
                showMessage($lang['nc_common_op_fail'], 'index.php?act=offlinegoods_class&op=tag');
            }
        }else{
            $this->log(L('nc_reset').'tag',0);
            showMessage($lang['goods_class_reset_tag_fail_no_class'], 'index.php?act=offlinegoods_class&op=tag');
        }
    }
    ...
    public function tag_updateOp(){
        ...
        if(is_array($tag_list) && !empty($tag_list)){
            ...
            $this->log(L('nc_update').'tag',0);
            showMessage($lang['goods_class_update_tag_fail_no_class'], 'index.php?act=offlinegoods_class&op=tag');
        }
