...
class stat_yundouControl extends SystemControl{
    private $links = array(
        array('url'=>'act=stat_yundou&op=income1','lang'=>'stat_sale_income'),
        // array('url'=>'act=stat_trade&op=predeposit','lang'=>'stat_predeposit'),
        array('url'=>'act=stat_yundou&op=sale','lang'=>'stat_yundou')
    );
    ...
    public function income1Op(){
        ...
        if($_GET['exporttype'] == 'excel'){
            ...
            Tpl::output('top_link',$this->sublink($this->links, 'income1'));
			Tpl::setDirquna('shop');
            Tpl::showpage('stat.income1');
        }
    }
    ...
    public function saleOp(){
        ...
        $where['point_addtime'] = array('between',array($curr_stime,$etime));
        $where['point_orderstate']   = array('egt',20);
        ...
        if (!empty($_GET['id']) && is_array($_GET['id'])){
            $where['point_orderid'] = array('in',$_GET['id']);
        }
        //导出Excel
        if ($_GET['exporttype'] == 'excel'){
            $order_list = $model->yundouByStatorder($where, '', 0, 0, 'point_orderid desc', '');
            //统计数据标题
            $statlist = array();
            $statlist['headertitle'] = array('订单号','买家','下单时间','兑换总积分','订单状态');

            foreach ((array)$order_list as $k => $v){
                switch ($v['point_orderstate']){
                    case 2:
                        $v['point_orderstate'] = '已取消';
                        break;
                    case 50:
                        $v['point_orderstate'] = '已完成';
                        break;
                    case 40:
                        $v['point_orderstate'] = '已收货';
                        break;
                    case 30:
                        $v['point_orderstate'] = '已发货';
                        break;
                    case 20:
                        $v['point_orderstate'] = '已兑换并扣除积分';
                        break;
                }
                $statlist['data'][$k]= $v;
            }
            ...
            foreach ((array)$statlist['data'] as $k => $v){
                $excel_data[$k+1][] = array('data'=>$v['point_ordersn']);
                $excel_data[$k+1][] = array('data'=>$v['point_buyername']);
                $excel_data[$k+1][] = array('data'=>date('Y-m-d H:i:s',$v['point_addtime']));
                $excel_data[$k+1][] = array('data'=>number_format(($v['point_allpoint']),2));
                $excel_data[$k+1][] = array('data'=>$v['point_orderstate']);
            }
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
            $excel_obj->addWorksheet($excel_obj->charset('云豆兑换统计',CHARSET));
            $excel_obj->generateXML($excel_obj->charset('云豆兑换统计',CHARSET).date('Y-m-d-H',time()));
            exit();
        } else {
            Tpl::output('top_link',$this->sublink($this->links, 'sale'));
			Tpl::setDirquna('shop');
            Tpl::showpage('stat_yundou');
        }
    }
    ...
    public function get_plat_saleOp(){
        ...
        $where['point_addtime'] = array('between',array($curr_stime,$etime));
        $where['point_orderstate']   = array('egt',20);
        $statcount_arr = $model->getYundou($where,' COUNT(*) as point_orderid, SUM(point_allpoint) as point_allpoint');
        echo '<div class="title"><h3>云豆兑换情况一览</h3></div>';
        echo '<dl class="row"><dd class="opt"><ul class="nc-row">';
        echo '<li title="总云豆兑换额：'. number_format(($statcount_arr['point_allpoint']),2).'个"><h4>总销售额</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'.$statcount_arr['point_allpoint'].'"></h2><h6>个</h6></li>';
        echo '<li title="总订单量：'. intval($statcount_arr['point_orderid']).'笔"><h4>总订单量</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="'. $statcount_arr['point_orderid'].'"></h2><h6>笔</h6></li>';
        echo '</ul></dd><dl>';
    ...
    public function get_sale_xmlOp(){
       ...
       /* $where['order_add_time'] = array('between',array($curr_stime,$etime));
        ...
        $order_type = array('point_ordersn','point_buyername','point_addtime','point_allpoint','point_orderstate');
        $sort_type = array('asc','desc');
        $sortname = trim($this->search_arr['sortname']);
        if (!in_array($sortname,$order_type)){
        ...
        $list = $model->yundouByStatorder($where, '', $page, 0, $orderby, '');

        $statlist = array();
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                $out_array = getFlexigridArray(array(),$order_type,$v);
                switch ($out_array['point_orderstate']){
                    case 2:
                        $out_array['point_orderstate'] = '已取消';
                        break;
                    case 50:
                        $out_array['point_orderstate'] = '已完成';
                        break;
                    case 40:
                        $out_array['point_orderstate'] = '已收货';
                        break;
                    case 30:
                        $out_array['point_orderstate'] = '已发货';
                        break;
                    case 20:
                        $out_array['point_orderstate'] = '已兑换并扣除积分';
                        break;
                }
                $out_array['point_addtime'] = date('Y-m-d H:i:s',$v['point_addtime']);
                $out_array['point_allpoint'] = number_format(($v['point_allpoint']),2);
                $statlist[$v['point_orderid']] = $out_array;
            }
        }
