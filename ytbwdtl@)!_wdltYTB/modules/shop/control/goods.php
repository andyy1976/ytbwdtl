...
class goodsControl extends SystemControl{
    private $links = array(
		...
		array('url'=>'act=goods&op=lackup_list','text'=>'审核不通过产品'),
	...
	public function indexOp() {
        //20170829潘丙福修改--默认为待审核商品
        // $this->goodsOp();
        $this->waitverify_listOp();
    }
	 /**
     * 商品管理
     */
	...
	审核不通过产品
	**/
	public function lackup_listOp() {
        Tpl::output('type', 'lackup');
        Tpl::output('top_link',$this->sublink($this->links,'lackup_list'));
		Tpl::setDirquna('shop');
        Tpl::showpage('goods.index');
    }
    /**
    ...
    public function get_xmlOp() {
        ...
        switch ($_GET['type']) {
			// 禁售
            case 'lackup':
                $goods_list = $model_goods->getGoodsCommonVerifyFailList($condition, '*', $page, $order);
                break;
            // 禁售
            case 'lockup':
                ...
                // 等待审核
            case 'waitverify':
                $goods_list = $model_goods->getGoodsCommonWaitVerifyList($condition, '*', $page, 'goods_addtime desc');
                break;
                // 全部商品
            default:
        ...
        foreach ($goods_list as $value) {
            ...
            //20170829潘丙福添加开始--增加运费模板显示
            if ($value['transport_id']) {
                $panCondition = array();
                $panCondition['transport_id'] = array('eq', $value['transport_id']);
                $panGoodsTransport = Model()->table('transport_extend')->field('area_name,sprice')->where($panCondition)->select();
                $panTmp = '';
                foreach ($panGoodsTransport as $k1 => $v1) {
                    $panTmp .= '运费:'.$v1['sprice'].'****'.$v1['area_name'].'<br/><hr/>';
                }
                $param['goods_freight'] = <<<EOB
...
EOB;
    ...
    public function goods_verifyOp(){
        if (chksubmit()) {
            ...
            //20170804潘丙福添加开始--商品审核次数
            $update1['edit_counts'] = array('exp', 'edit_counts+1');
            //20170804潘丙福添加结束
    ...
    public function get_goods_sku_listOp() {
        ...
        foreach ($goods_list as $key => $val) {
            ...
            foreach ($goods_spec as $k => $v) {
                ...
                $panGoodsSpace[] = $spec_name[$k].'：'.$v;
            }
            $goods_list[$key]['goods_image'] = thumb($val, '60');
            $goods_list[$key]['goods_spec'] = implode('', $spec_array);
            //20170731潘丙福添加开始
            $goods_list[$key]['pangoods_spec'] = implode(' |*| ', $panGoodsSpace);
            //20170731潘丙福添加结束
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            switch ($_GET['type']) {
                ...
                case 'waitverify':
                    ...
                    // 全部商品
				case 'lackup':
				    $count = $model_goods->getGoodsCommonVerifyFailCount($condition);
					break;
                default:
        ...
        switch ($_GET['type']) {
            ...
            case 'waitverify':
			 ...
			 //等待审核的商品
			 case 'lackup':
			 $goods_list = $model_goods->getGoodsCommonVerifyFailList($condition, '*', null, $order, $limit);
			 break;
                // 全部商品
            default:
