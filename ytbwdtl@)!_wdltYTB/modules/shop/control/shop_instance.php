...
class shop_instanceControl extends SystemControl{
    const EXPORT_SIZE = 1000;

    private $_links = array(
        array('url'=>'act=shop_instance&op=store','text'=>'签约商店管理'),
        array('url'=>'act=shop_instance&op=store_joinin','text'=>'签约商店开店申请'),
        //array('url'=>'act=shop_instance&op=store_point','text'=>'商家云豆兑换记录')
        );
    ...
    {
        Tpl::output('top_link',$this->sublink($this->_links,'search_order'));
        Tpl::setDirquna('shop');
        Tpl::showpage('store.opencharge');
    }
    ...
    *商家云豆兑换记录
    */
    public function store_pointOp(){
          //输出子菜单
        Tpl::output('top_link',$this->sublink($this->_links,'store_point'));
        Tpl::setDirquna('shop');
        Tpl::showpage('store_instance_point.index');
    }
    ...
    public function storeOp(){
        ...
        Tpl::showpage('store_instance.index');
    }
   ...
   *输出XML数据
   */
    public function get_xmlpointOp(){
        $model_points_store = Model('point_recorders');  
        $condition = array();
        if ($_GET['store_name'] != '') {
      ...
      $order = '';
      $page = $_POST['rp'];
      $store_list=$model_points_store->where($condition)->order($order)->limit($limit)->page($page)->select();
      $data = array();
      $data['now_page'] = $model_points_store->shownowpage();
      $data['total_num'] = $model_points_store->gettotalnum();
      foreach ($store_list as $value) {
         $l= 1;
        $param = array();
        $param['store_id'] = $value['store_id'];
        $param['store_name'] = $value['store_name'];
        $param['contact_man'] = $value['contact_man'];
        $param['add_time'] = date('Y-m-d', $value['add_time']);
        $param['order_suite'] = $value['order_suite'];
        $param['order_sn'] = $value['order_sn'];
        $param['need_points'] = $value['need_points'];
        $param['need_money'] = $value['need_money'];
        $param['buyer_id'] = $value['buyer_id'];
        $data['list'][$value['point_id']] = $param;
        }
    ...
    public function get_xmlOp() {
        ...
        // 设置页码参数名称
        $condition = array();
		$condition['store_flag']  = array('eq',1);
        if ($_GET['store_name'] != '') {
            $condition['store_name'] = array('like', '%' . $_GET['store_name'] . '%');
        }
        if ($_GET['member_name'] != '') {
        ...
        foreach ($store_list as $value) {
            ...
            $store_state = $this->getStoreState($value);
             if($value['store_flag']==1){
             $operation = "<a class='btn green' href='index.php?act=shop_instance&op=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?act=shop_instance&op=shencerweima&store_id=" . $value['store_id'] . "'>设置店铺二维码</a></li>";
             }elseif($value['store_flag']==2){
                $operation = "<a class='btn orange' href=\"index.php?act=shop_instance&op=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?act=shop_instance&op=shencerweima&store_id=" . $value['store_id'] . "'>设置店铺二维码</a></li>"; 
             }
            ...
            $operation .= "</ul></span><a class='btn green' href='/shop/index.php?act=seller_login&op=logins&seller_name=".$value['seller_name']."&password=".ckeck_store_pass($value['member_id'])."&m5=".$l."' target='blank' >登陆</a>";
    ...
    public function store_editOp(){
       ...
       Tpl::output('store_array',$store_array);
        Tpl::setDirquna('shop');
        Tpl::showpage('shop_instance.edit');
    }
    ...
    public function store_joininOp(){
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('store_joinin_copy');
    }
    ...
    public function get_joinin_xmlOp() {
        ...
        $condition['store_flag']=array('eq',2);
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
        }
        $order = '';
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href=\"index.php?act=shop_instance&op=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?act=shop_instance&op=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
    ...
    public function get_bind_class_applay_xmlOp() {
        ...
        $page = $_POST['rp'];
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList($condition, $page, 'state asc,bid desc');
        $storeid_array = array();
     ...
     * 注意没有写入日志
     */
    public function batchcheckOp()
    {
        $model_store_bind_class = Model('store_bind_class');
        $condition = array();
        $condition['bid'] = array('in',$_GET['batchcheck_id']);
        $condition['state'] = 0;
        $update = $model_store_bind_class->editStoreBindClass(array('state'=>1),$condition);
        if (!$update){
            exit(json_encode(array('state'=>false,'msg'=>'批量审核失败')));
        }else{
            exit(json_encode(array('state'=>true,'msg'=>'批量审核成功')));
        }
    }
    ...
    public function store_joinin_detailOp(){
	   ...
	   $joinin_detail = $model_store->where(array('store_id'=>$_GET['store_id']))->find();	
		$abs=$model_store->where(array('store_id'=>$_GET['store_id']))->find();	
			}else{
        $abs=$model_store->where(array('member_id'=>$_GET['member_id']))->find();
			}
		if(strlen($abs['custom_pointa'])<2){
			$abs['custom_pointa']=@strtotime(date('Y-m-d H:i:s'));
			}
        if(intval($abs['store_flag'])==2) {
        ...
        Tpl::output('abs',$abs);
		Tpl::setDirquna('shop');
        Tpl::showpage('store_joinin.detail_copy');
    }
    ...
    public function store_joinin_verifyOp() {
        $model_store_joinin = Model('store');
        $store_id = isset($_POST['store_id'])?$_POST['store_id']:'';
        $pass = isset($_POST['pass'])?$_POST['pass']:'0';
		$custom_pointa = isset($_POST['custom_pointa'])?$_POST['custom_pointa']:'';
        $model_store = Model('store');
        if($pass==0){
          $update['store_flag']=0;
        }else{
		   $update['custom_pointa']=$custom_pointa;
           $update['store_flag']=1;
       }
    $conditon['store_id']=$store_id;
    $data = array('store_flag'=>$update['store_flag'],'custom_pointa'=>$update['custom_pointa']);
    $isok=$model_store->table('store')->where(array('store_id'=>$store_id))->update($data);
    if($isok){
        showMessage('审核成功',getReferer());
        }else{
       showMessage('审核失败',getReferer());
            }
    }*/
     /**
     * 审核
    ...
    private function store_joinin_verify_pass($joinin_detail) {
      ...
      private function store_joinin_verify_open($joinin_detail) {
        ...
        $param = array();  
        if($_POST['store_flag']==2){
          $param['joinin_state'] =30; 
          $param['store_flag'] =1;    
        }else{
        ...
        $param['joinin_message'] = $_POST['joinin_message'];
        $model_store_joinin->modify($param, array('member_id'=>$_POST['member_id']));
        if($_POST['verify_type'] === 'pass') {//开店
            $shop_array     = array();
            if($joinin_detail['store_flag']==2){
             $shop_array['store_flag']    = 1;
             $model_member=Model('member');
             $data = array();
             $data['member_level']=1;  //激活会员
             $model_member->table('member')->where(array('member_id'=>$_POST['member_id']))->update($data);
      }
            ...
            if($state) {
                ...
                showMessage('店铺开店成功','index.php?act=shop_instance&op=store_joinin');
            } else {
                showMessage('店铺开店失败','index.php?act=shop_instance&op=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝','index.php?act=shop_instance&op=store_joinin');
        }
    }
    ...
    public function chiefOp() {
        ...
        foreach($store_list as $value){
            ...
            $total += $ticheng;
            $model_member->where(array('member_id'=>$member_pid))->setInc('available_predeposit',$ticheng);
            //资金进账记录                
            ...
            $pd_log->table('log_store')->insert($data); 
            $i++;       
        } 
        if($i > 0){
    ...
    public function edit_ydsetOp(){    //设置会员的消费积分比例
	$custom_point= isset($_GET['custom_point'])?$_GET['custom_point']:'0';
	$custom_pointis =isset($_GET['custom_pointis'])?$_GET['custom_pointis']:'0';
	$store_id = isset($_GET['store_id'])?$_GET['store_id']:'';
	$model_store = Model('store');
	$update['custom_point']=$custom_point;
	$update['custom_pointis']=$custom_pointis;
	$conditon['store_id']=$store_id;
    $data = array('custom_point'=>$custom_point,'custom_pointis'=>$custom_pointis);
    $isok=$model_store->table('store')->where(array('store_id'=>$store_id))->update($data);
	if($isok){
       echo '1';
	   exit();
	...
	public function shencerweimaOp(){
    $id=$_GET['store_id'];
    $model_store = Model('store');
	  if(!$id){
		  showMessage('非法请求');
	 }else{
	 $isok=$model_store->table('store')->where(array('store_id'=>$id))->find();	 
     if($isok && $isok['store_flag']!=1){
        showMessage('非法请求');
    } else{
      $PNG_TEMP_DIR='../../../data/upload/erweima/';
      $PNG_WEB_DIR = '../../../data/upload/erweima/';
       include  BASE_PATH."/erweim/qrlib.php";    
      $filename = $PNG_TEMP_DIR.$id.'.png';
      $errorCorrectionLevel = 'L';
      $matrixPointSize = 10;
      $value="https://".$_SERVER['HTTP_HOST']."/wap/tmpl/member/register.html?map_id=".$id;
      QRcode::png($value, $filename, $errorCorrectionLevel, $matrixPointSize, 2);    
      Tpl::output('imgsrc',$PNG_WEB_DIR.basename($filename));
      Tpl::output('store_id',$isok['store_id']);
      Tpl::setDirquna('shop');
      Tpl::showpage('shop_instance.edit');
		  }
    }
    ...
    public function okhop_addOp(){     //保存店铺二维码
      $store_id=$_GET['store_id'];  
      $model_store = Model('store');
      $isok=$model_store->table('store')->where(array('store_id'=>$store_id))->find(); 
       if(!$isok && $isok['store_flag']!=1){
        showMessage('非法请求');
    }else{
      $isokk = $model_store->table('store')->where(array('store_id'=>$store_id))->update(array('shop_erweima'=>$store_id.'.png'));
      if($isokk){
        showMessage('保存成功！');
      }else{
        showMessage('保存失败！');
