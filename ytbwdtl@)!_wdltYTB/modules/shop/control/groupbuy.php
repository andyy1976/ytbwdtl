...
class groupbuyControl extends SystemControl{
     ...
     *
     */
    public function groupbuy_list1Op()
    {
        $model_groupbuy = Model('groupbuy');
        Tpl::output('groupbuy_state_array', $model_groupbuy->getGroupbuyStateArray());

        $this->show_menu('groupbuy_list1');
                
        Tpl::setDirquna('shop');
        Tpl::showpage('groupbuy.list1');
    }
    ...
    public function groupbuy_list1_xmlOp()
    {
        $condition = array();
        ...
        foreach ($groupbuy_list as $val) {
            ...
            $o .= "<li><a class='confirm-on-click' href='javascript:void(0);' onclick=\"time_h('" . $val['groupbuy_id'] . "')\"><i class='fa fa-check-square'></i>编辑时间点</a></li>";

            if ($val['reviewable']) {
                //20171010潘丙福修改--恢复程序
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . urlAdminShop('groupbuy', 'groupbuy_review_pass', array('groupbuy_id' => $val['groupbuy_id'])) . '"><i class="fa fa-check-square"></i>批准活动</a></li>';
                //20171020潘丙福添加开始--增加拒绝活动功能
                $o .= "<li><a class='confirm-on-click' href='javascript:void(0);' onclick=\"fg_verify('" . $val['groupbuy_id'] . "')\"><i class='fa fa-check-square'></i>拒绝活动</a></li>";
                //20171020潘丙福添加结束
                // 20171010潘丙福修改--删除程序
                // $o .= "<li><a class='confirm-on-click' href='javascript:void(0);' onclick=\"fg_verify('" . $val['groupbuy_id'] . "')\"><i class='fa fa-check-square'></i>审核活动</a></li>";
            }
...
EOB;
            //20170803潘丙福添加开始--增加原始云豆、折扣云豆
            $i['groupbuy_points'] = $val['groupbuy_points'];
            $i['goods_points'] = $val['goods_points'];
            if (is_array($val['goods_freight']) && count($val['goods_freight']) > 0) {
                $panTmp = null;
                foreach ($val['goods_freight'] as $k1 => $v1) {
                    $panTmp .= '运费:'.$v1['sprice'].'****'.$v1['area_name'].'<br/><hr/>';
                }
                $i['goods_freight'] = <<<EOB
<a href="javascript:;" class="pic-thumb-tip" onMouseOut="toolTip()" onMouseOver="toolTip('{$panTmp}')">售卖区域(悬停查看)</a>
EOB;
            } else {
                $i['goods_freight'] = $val['goods_freight'];
            }
            //20170803潘丙福添加结束
...
EOB;
    ...
    public function groupbuy_review_passOp(){
        ...
        //20170517潘丙福添加开始
        $panGoodsInfo = $model_groupbuy->where(array('groupbuy_id'=>$groupbuy_id))->find();
        //添加折扣商品时修改对应goods表里面的商品的促销价格以及促销类型
        QueueClient::push('updateGoodsPromotionPriceByGoodsCommonId', $panGoodsInfo['goods_commonid']);
        //20170517潘丙福添加结束
        if($result) {
    ...
    public function groupbuy_review_failOp(){
        ...
        $groupbuy_id = intval($_REQUEST['groupbuy_id']);
        $goods_remark = trim($_REQUEST['verify_reason']);
        $model_groupbuy = Model('groupbuy');
        $result = $model_groupbuy->reviewFailGroupbuy($groupbuy_id, $goods_remark);
        return $result;
        // if($result) {
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'groupbuy_list1'=>array('menu_type'=>'link','menu_name'=>'待审核秒杀活动','menu_url'=>'index.php?act=groupbuy&op=groupbuy_list1'),
            'groupbuy_list'=>array('menu_type'=>'link','menu_name'=>'秒杀活动','menu_url'=>'index.php?act=groupbuy&op=groupbuy_list'),
            'groupbuy_quota'=>array('menu_type'=>'link','menu_name'=>'套餐管理','menu_url'=>'index.php?act=groupbuy&op=groupbuy_quota'),
            'class_list'=>array('menu_type'=>'link','menu_name'=>'秒杀分类','menu_url'=>'index.php?act=groupbuy&op=class_list'),
            'class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('groupbuy_class_add'),'menu_url'=>'index.php?act=groupbuy&op=class_add'),
            'price_list'=>array('menu_type'=>'link','menu_name'=>'秒杀价格区间','menu_url'=>'index.php?act=groupbuy&op=price_list'),
    ...
    public function goods_verifyOp(){
        if (chksubmit()) {
            $result = $this->groupbuy_review_failOp();
            if($result) {
                $this->log('拒绝抢购活动申请，抢购编号'.$_REQUEST['groupbuy_id'],null);
                showDialog(L('nc_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();CUR_DIALOG.close();');
            } else {
                showDialog('操作失败', '', 'error', '$("#flexigrid").flexReload();CUR_DIALOG.close();');
            }    
        }

        $common_info = Model('groupbuy')->getGroupbuyInfo(array('groupbuy_id'=>$_GET['id']));
        Tpl::output('common_info', $common_info);
                        
        Tpl::setDirquna('shop');
        Tpl::showpage('groupbuy.verify_remark', 'null_layout');
    }

    /**
     * 编辑时间段   2017-10-26 xin增加
     */
  public function time_hOp(){
        if (chksubmit()) {
            ...
            $update1 = array();
            $update1['time_h'] = trim($_POST['time_h']);
            $where = array();
            $where['groupbuy_id'] = $groupbuy_id;
            $b = Model()->table('groupbuy')->where($where)->update($update1);
           
            showDialog(L('nc_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();CUR_DIALOG.close();');
        }
        $common_info = Model('groupbuy')->field('time_h,goods_commonid,goods_name,groupbuy_id')->where(array('groupbuy_id'=>$_GET['id']))->find();
        Tpl::output('common_info', $common_info);
                        
        Tpl::setDirquna('shop');
        Tpl::showpage('groupbuy.verify_time_h', 'null_layout');
    }
}
