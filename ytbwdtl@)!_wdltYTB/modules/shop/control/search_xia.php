...
class search_xiaControl extends SystemControl{

    private $_links = array(
        array('url'=>'act=search_xia&op=index','text'=>'默认搜索'),
        array('url'=>'act=search_xia&op=hot','text'=>'热门搜索')
    );
    ...
    public function indexOp() {
        if (chksubmit()){
            ...
            $result = $model_setting->updateSetting(array(
                    'hot_search_xia'=>str_replace($comma,',',$_POST['hot_search'])));
            if ($result){
                showMessage('保存成功');
            }else {
                showMessage('保存失败');
    ...
    public function hotOp() {
        $model_setting = Model('setting');
        $search_info = $model_setting->getRowSetting('rec_search_xia');
    ...
    public function hot_addOp() {
        ...
        if (chksubmit()) {
            if (count($search_list) >= 10) {
                showMessage('最多可设置10个热搜词','index.php?act=search_xia&op=hot');
            }
            ...
            $result = $model_setting->updateSetting(array('rec_search_xia'=>serialize($search_list)));
            if ($result){
                showMessage('保存成功','index.php?act=search_xia&op=hot');
            }else {
                showMessage('保存失败');
            }
        }
        ...
        Tpl::showpage('search.hot_add_xia');
    }
    ...
    public function hot_delOp() {
        ...
        $result = $model_setting->updateSetting(array('rec_search_xia'=>serialize(array_values($search_list))));
        if ($result){
            showMessage('删除成功');
        }
        showMessage('删除失败');
    ...
    public function hot_editOp() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('rec_search_xia'=>serialize($search_list)));
            if ($result){
                showMessage('编辑成功','index.php?act=search_xia&op=hot');
            }
            showMessage('编辑失败');
        }
