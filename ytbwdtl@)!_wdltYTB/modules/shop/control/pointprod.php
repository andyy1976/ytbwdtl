    ...
    public function pointprod_xmlOp()
    {
        ...
        switch ($_REQUEST['sortname']) {
            ...
            case 'pgoods_salenum':
                ...
                break;
            default:
                $sort = 'pgoods_sort desc, pgoods_id';
                break;
        }
        ...
        foreach ($prod_list as $val) {
            ...
            //20170902潘丙福添加开始--调整排序位置
            $i['pgoods_sort'] = '<input type="text" name="panpgoods_sort" style="width:45px" class="panpgoods_sort" value="'.$val['pgoods_sort'].'" />';
            //20170902潘丙福添加结束
...
EOB;
    ...
    *审核   2017-7-11 熊磊增加
    ...
    public function order_auditOp(){
        ...
        $point_info = $model_pointorder->getPointOrderInfo($where);
        //修改订单状态
        if($type  == 'zero'){
            $data['point_orderstate'] = $pointorderstate_arr['zero'][0];
            $updata = $model_pointorder->where($where)->update($data);
            if($updata){
                showDialog('审核通过订单已处于处理中','index.php?act=pointprod&op=pointorder_list');
            }else{
                showDialog('审核失败','index.php?act=pointprod&op=pointorder_list','error');
            }
        }else{
            showDialog(L('admin_pointorder_parameter_error'),'index.php?act=pointprod&op=pointorder_list','error');
        }
    ...
    // 批量审核
    public function batch_approveOp()
    {
        $ids = $_POST['ids'];
        $model_pointorder = Model('pointorder');
        $condition = array(
            'point_orderid' => array('IN', explode(',', $ids))
        );
        $data['point_orderstate'] = '10'; // 状态更新为：10 - 处理中
        $result = $model_pointorder->table('points_order')->where($condition)->update($data);
        echo $result ? '1' : '0';
    }
    ...
    public function pointorder_list_xmlOp()
    {
       ...
       // 熊磊 2017-7-12  添加 651-652行代码    657-660行代码   667-674行代码
        foreach ($list as $val) {
            if($val['point_orderstatetext']=='未知'){ 
               $val['point_orderstatetext']='审核'; 
               $val['point_orderstatesign']='audit';
            }
            $o = '<a class="btn green" href="' . urlAdminShop('pointprod', 'order_info', array(
                'order_id' => $val['point_orderid'],
            )) . '"><i class="fa fa-list-alt"></i>查看</a>';
            if($val['point_orderstate'] ==20 ){
                $o .= '<a class="btn green" href="' . urlAdminShop('pointprod', 'order_audit', array(
                    'order_id' => $val['point_orderid'],'type'=>'zero'
                )) . '"><i class=""></i>审核</a>';
            }
            if (
                ...
                || $val['point_orderallowaudit']
               ) {
                $o .= '<span class="btn"><em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em><ul>';
                if($val['point_orderallowaudit']){
                     // 发货（已确认付款，待发货）
                    $o .= '<li><a href="' . urlAdminShop('pointprod', 'order_ship', array(
                        'id' => $val['point_orderid'],
                    )) . '">设置发货</a></li>';
                    // 取消订单（未发货）
                    $o .= '<li><a class="confirm-on-click" href="' . urlAdminShop('pointprod', 'order_cancel'
                        , array('id' => $val['point_orderid'], 'admin'=>'1')) . '">取消订单</a></li>';
                }
            ...
            //20170720潘丙福添加开始--增加充值手机号、买家留言
            $i['point_chargephone'] = $val['point_chargephone'];
            $i['point_ordermessage'] = $val['point_ordermessage'];
            //20170720潘丙福添加结束
 ...
 /**
     * 查看发货
     */
    public function order_ship_getOp(){
        ...
        $where = array();
        $where['point_orderid'] = $order_id;
        //熊磊 修改 $pointorderstate_arr['zero'][0]代码   原代码是  $pointorderstate_arr['waitship'][0]     
        $where['point_orderstate'] = array('in',array($pointorderstate_arr['waitreceiving'][0],$pointorderstate_arr['zero'][0]));//待发货和已经发货状态
        ...
        if (chksubmit()){
            ...
            Tpl::setDirquna('shop');
            Tpl::showpage('pointorder.shiplo');
        }
    }
    /**
     * 发货
 ...
 /**
    ...
    public function export_cash_step1Op(){
        ...
        if ($start_unixtime || $end_unixtime) {
            $condition['point_addtime'] = array('time',array($start_unixtime,$end_unixtime));
        }
        if (!empty($_GET['pointprod_state'])){
            $condition['point_orderstate'] =$_GET['pointprod_state'];
        }else{
            $condition['point_orderstate'] = $_GET['pointprod_state'];
        }
        $order = '';

            $model_pd = Model('pointorder');
            $count =$model_pd->getPointOrderList($condition); 
            ...
            foreach ($data as $key => $v) {
                $where['point_orderid'] = $v['point_orderid'];
                $point_goodsid = Model()->table('points_ordergoods')->field('point_goodsid')->where($where)->find();
                $where1['pgoods_id'] = $point_goodsid['point_goodsid'];
                $pgoods_serial = Model()->table('points_goods')->field('pgoods_id,pgoods_serial,pgoods_tag')->where($where1)->find();
                $data[$key]['pgoods_serial'] = $pgoods_serial['pgoods_serial'];
                $data[$key]['pgoods_tag']    = $pgoods_serial['pgoods_tag'];
            }
            $this->createCashExcel($data,$count,'',$condition['point_orderstate']);    
   ...
   private function createCashExcel($data = array(),$page,$amout,$predeposit_type){
        ...
        $excel_obj = new Excel();
        $excel_data = array();
        $pd_charge=Model('pointorder');

        if($predeposit_type==10 || $predeposit_type==20 || $predeposit_type==30 ||$predeposit_type==40 || $predeposit_type==2){

            $excel_data[0][] = array('data'=>'F');
            $excel_data[0][] = array('data'=>'200110000007221');
            $excel_data[0][] = array('data'=>date("Y-m-d"));
            $excel_data[0][] = array('data'=>'09900');

            $excel_data[1][] = array('data'=>'序号');
            $excel_data[1][] = array('data'=>'用户ID');
            $excel_data[1][] = array('data'=>'订单时间');
            $excel_data[1][] = array('data'=>'订单编号');
            $excel_data[1][] = array('data'=>'礼品编号');
            $excel_data[1][] = array('data'=>'礼品标签');
            $excel_data[1][] = array('data'=>'订单积分');
            $excel_data[1][] = array('data'=>'订单来源');
            $excel_data[1][] = array('data'=>'商品名称');
            $excel_data[1][] = array('data'=>'供货商');
            $excel_data[1][] = array('data'=>'商品市场价格');
            $excel_data[1][] = array('data'=>'单件商品积分价格');
            $excel_data[1][] = array('data'=>'购买数量');
            $excel_data[1][] = array('data'=>'购买者姓名');
            $excel_data[1][] = array('data'=>'数据导出类型');
            $excel_data[1][] = array('data'=>'收货人地址信');
            $excel_data[1][] = array('data'=>'手机号码');
            $excel_data[1][] = array('data'=>'留言');
            $excel_data[1][] = array('data'=>'充值的手机号码');
            // $excel_data[2][] = array('data'=>'手机号/小灵通');
            // $excel_data[2][] = array('data'=>'自定义用户号');
            // $excel_data[2][] = array('data'=>'备注');
            // $excel_data[2][] = array('data'=>'返回码');
            // $excel_data[2][] = array('data'=>'返回原因');
            foreach ((array)$data as $k=>$v){
                if($predeposit_type==10){ 
                    $v['type'] ="待发货";
                }if($predeposit_type==20){
                    $v['type'] ="处理中";
                }if($predeposit_type==30){
                    $v['type'] ="待收货";
                }if($predeposit_type==40){
                    $v['type'] ="已收货";
                }if($predeposit_type==2){
                    $v['type'] = "取消";
                }
                $member = $pd_charge->table('member')->where(array('member_id'=>$v['point_buyerid']))->find();
                $variable = $pd_charge->getPointOrderGoodsList(array('point_orderid'=>$v['point_orderid']));
                $address_info = $pd_charge->getPointOrderAddressInfo(array('point_orderid'=>$v['point_orderid']));
                //获取商品ID
                $goods = $pd_charge->table('points_ordergoods')->where(array('point_orderid'=>$v['point_orderid']))->find();
                $goods_price = $pd_charge->table('points_goods')->where(array('pgoods_id'=>$goods['point_goodsid']))->find();
                $mobile_msg = $pd_charge->table('points_order')->where(array("point_orderid"=>$goods['point_orderid']))->find();
                foreach ($variable as $key => $value) {
                if($v['point_buyerid'] == '10088'){
                    continue;
                }
                $tmp = array();
                $tmp[] = array('data'=>'000'.$k);
                $tmp[] = array('data'=>$v['point_buyerid']);
                $tmp[] = array('data'=>date("Y-m-d H:i:s",$v['point_addtime']));
                $tmp[] = array('data'=>$v['point_ordersn']);
                $tmp[] = array('data'=>$v['pgoods_serial']);
                $tmp[] = array('data'=>$v['pgoods_tag']);
                $tmp[] = array('data'=>$v['point_allpoint']);
                $tmp[] = array('data'=>"");
                $tmp[] = array('data'=>$value['point_goodsname']);
                $tmp[] = array('data'=>"");
                $tmp[] = array('data'=>$goods_price['pgoods_price']);
                $tmp[] = array('data'=>$value['point_goodspoints']);
                $tmp[] = array('data'=>$value['point_goodsnum']);
                $tmp[] = array('data'=>$address_info['point_truename']);
                $tmp[] = array('data'=>$v['type']);
                $tmp[] = array('data'=>$address_info['point_areainfo'].$address_info['point_address']);
                $tmp[] = array('data'=>$member['member_mobile']);
                $tmp[] = array('data'=>$mobile_msg['point_ordermessage']);
                $tmp[] = array('data'=>$mobile_msg['point_chargephone']);
                $tmp[] = array('data'=>'');
                $tmp[] = array('data'=>'');
                $tmp[] = array('data'=>'');
                $tmp[] = array('data'=>'');
     ...
     *ajax请求修改云豆商品排序 
     */
    public function ajax_updatesortOp()
    {
        $id = intval($_GET['id']);
        if ($id <= 0){
            echo 'false'; exit;
        ...
        $model_pointprod = Model('pointprod');
        $update_array = array();
        $update_array['pgoods_sort'] = trim($_GET['value']);
        $model_pointprod->editPointProd($update_array,array('pgoods_id'=>$id));
        echo 'true';exit;
    }
}