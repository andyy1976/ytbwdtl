...
class orderControl extends SystemControl{
    ...
    public function get_xmlOp(){
        ...
        $condition  = array();
        $condition['is_dm']=array('eq',0);    //实体订单与地面订单分开
        ...
        $order_list = $model_order->getOrderList2($condition,$_POST['rp'],'*',$order);
        $data = array();
        $time = time();
        ...
        foreach ($order_list as $order_id => $order_info) {
			...
			$list['payment_time'] = !empty($order_info['payment_time']) ? (intval(date('Y-m-d H:i:s',$order_info['payment_time'])) ? date('Y-m-d H:i:s',$order_info['payment_time']) : date('Y-m-d',$order_info['payment_time'])) : '';
