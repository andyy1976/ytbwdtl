...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        ...
        array('url'=>'act=store&op=search_order','text'=>'开店缴费记录查询')
    );
     ...
     * 输出充值XML数据
     */
    public function pan_get_xmlOp() {
        $model_pd = Model('opshop');
        $condition = array();
        // if ($_GET['member_name']) {
        //     $condition['pdr_member_name'] = array('like', '%' . $_GET['member_name'] . '%');
        // }
        if ($_GET['member_id']) {
            $condition['member_id'] = array('eq', $_GET['member_id']);
        }
        ...
        if ($_GET['pdr_payment_state'] != '') {
            $condition['order_status'] = $_GET['pdr_payment_state'] == 1 ? 1 : 0;
        }
        //2017/10/28xin
        if($condition['order_status'] == 0){
            unset($condition['pay_time']);
        ...
        $order = '';
        $param = array('id', 'member_id', 'pay_amount', 'openshop_order_sn', 'openshop_pay_sn', 'order_status', 'pay_style', 'add_time', 'pay_time', 'oper_admin');
        ...
        foreach ($recharge_list as $value) {
            ...
            $operation .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('recharge_info','查看充值编号“". $value['openshop_pay_sn'] ."”的明细','index.php?act=store&op=recharge_info&id=".$value['id']."', '640')\"><i class='fa fa-list-alt'></i>查看</a>";
            $param['operation'] = $operation;
            $param['id'] = $value['id'];
            $param['member_id'] = $value['member_id'];
            $param['openshop_order_sn'] = $value['openshop_order_sn'];
            $param['openshop_pay_sn'] = $value['openshop_pay_sn'];
            $param['pay_amount'] = ncPriceFormat($value['pay_amount']);
            $param['add_time'] = date('Y-m-d H:i:s', $value['add_time']);
            $param['pay_style'] = $value['pay_style'];
            $param['order_status'] = $value['order_status'] == '0' ? '未支付' : '已支付';
            $param['pay_time'] = $value['pay_time'] > 0 ? date('Y-m-d H:i:s', $value['pay_time']) : '';
            $param['oper_admin'] = $value['oper_admin'];
    ...
    public function recharge_infoOp(){
        ...
        if (empty($info)){
            showMessage(Language::get('admin_predeposit_record_error'),'index.php?act=store&op=search_order','','error');
        }
        Tpl::output('info',$info);
        Tpl::setDirquna('shop');
        Tpl::showpage('pd.info1', 'null_layout');
    ...
    public function recharge_editOp(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),'index.php?act=store&op=search_order','','error');
        }
        //查询充值信息
        $model_pd = Model('opshop');
        $condition = array();
        ...
        $condition = array();
        $condition['openshop_pay_sn'] = $info['openshop_pay_sn'];
        $condition['order_status'] = 0;
        $update = array();
        $update['order_status'] = 1;
        $update['pay_time'] = strtotime($_POST['payment_time']);
        $update['pay_style'] = $payment_info['payment_name'];
        // $update['pdr_payment_name'] = $payment_info['payment_name'];
        // $update['pdr_trade_sn'] = $_POST['trade_no'];
        $update['oper_admin'] = $this->admin_info['name'];
        // var_dump($update);exit;
        $log_msg = L('admin_predeposit_recharge_edit_state').','.L('admin_predeposit_sn').':'.$info['openshop_pay_sn'];
        ...
        try {
            ...
            $model_store_joinin->modify($param, array('member_id' => $info['member_id']));
            if (!$state) {
                throw Exception(Language::get('更改店铺状态失败！'));
            }

            $model_pd->commit();
            $this->log($log_msg,1);
            //记录消费日志
            QueueClient::push('addConsume', array('member_id'=>$info['member_id'],'member_name'=>$update['oper_admin'],
            'consume_amount'=>$info['pay_amount'],'consume_time'=>TIMESTAMP,'consume_remark'=>'管理员更改开通店铺费用充值单['.$info['openshop_pay_sn'].']状态,充值成功'));
            showMessage(Language::get('admin_predeposit_recharge_edit_success'),'index.php?act=store&op=search_order');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),'index.php?act=store&op=search_order','html','error');
        }
    }
    ...
    public function get_joinin_xmlOp() {
        ...
        $condition['store_flag']=array('neq',2);
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
        }
        $order = '';
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {//开店
            ...
            $shop_array['store_flag']  = $joinin_detail['store_flag'];
            //增加省旗舰店
            $shop_array['is_province']    = $joinin_detail['is_province'];
