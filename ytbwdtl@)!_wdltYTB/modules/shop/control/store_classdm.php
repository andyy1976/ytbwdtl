...
class store_classdmControl extends SystemControl{
    ...
    public function indexOp() {
        $this->store_classdmOp();
    }

    /**
     * 店铺分类
     */
    public function store_classdmOp(){
        ...
        $store_class_list = $model_class->getStoreClassList(array('sc_flag'=>1));
        Tpl::output('class_list',$store_class_list);
		Tpl::setDirquna('shop');
        Tpl::showpage('store_classdm.index');
    }
    ...
    public function store_class_addOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $insert_array['sc_bail'] = 0;
                $insert_array['sc_sort'] = intval($_POST['sc_sort']);
                $insert_array['sc_flag'] = 1;
                $result = $model_class->addStoreClass($insert_array);
                if ($result){
                    $url = array(
                    array(
                    'url'=>'index.php?act=store_classdm&op=store_class_add',
                    'msg'=>$lang['continue_add_store_class'],
                    ),
                    array(
                    'url'=>'index.php?act=store_classdm&op=store_classdm',
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('store_classdm.add');
    }
    ...
    public function store_class_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $update_array['sc_bail'] = 0;
               
                $update_array['sc_flag'] = 1;
                $update_array['sc_sort'] = intval($_POST['sc_sort']);
                $result = $model_class->editStoreClass($update_array,array('sc_id'=>intval($_POST['sc_id'])));
                if ($result){
                    $this->log(L('nc_edit,store_classdm').'['.$_POST['sc_name'].']',1);
                    showMessage($lang['nc_common_save_succ'],'index.php?act=store_classdm&op=store_classdm');
                }else {
                    showMessage($lang['nc_common_save_fail']);
                }
            }
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('store_classdm.edit');
    }
    ...
    public function store_class_delOp(){
        ...
        if (intval($_GET['sc_id']) > 0){
            ...
            if ($result) {
                 $this->log(L('nc_del,store_classdm').'[ID:'.$_GET['sc_id'].']',1);
                 showMessage($lang['nc_common_del_succ'],getReferer());
            }
        }
        showMessage($lang['nc_common_del_fail'],'index.php?act=store_classdm&op=store_classdm');
    }
