...
class stat_goodsControl extends SystemControl{
    ...
    public function goods_saleOp(){
        ...
        $field = 'goods_id,min(goods_name) as goods_name,min(store_id) as store_id,min(store_name) as store_name,min(goods_commonid) as goods_commonid,SUM(goods_num) as goodsnum,COUNT(DISTINCT order_id) as ordernum,SUM(goods_pay_price) as goodsamount,SUM(goods_pay_points) as goodstotalpoints';
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $excel_data[0][] = array('styleid'=>'s_title','data'=>'下单云豆');
            //data
            foreach ($goods_list as $k => $v){
                ...
                $excel_data[$k+1][] = array('data'=>$v['goodstotalpoints']);
            }
    ...
    public function get_goods_xmlOp(){
        ...
        $field = 'goods_id,min(goods_name) as goods_name,min(store_id) as store_id,min(store_name) as store_name,min(goods_commonid) as goods_commonid,SUM(goods_num) as goodsnum,COUNT(DISTINCT order_id) as ordernum,SUM(goods_pay_price) as goodsamount,SUM(goods_pay_points) as goodstotalpoints';
        //排序
        $order_type = array('goods_name','store_name','goods_commonid','goodsnum','ordernum','goodsamount','goodstotalpoints');
