  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        ...
        <li><a href="JavaScript:void(0);" class="current">待审核-易宝</a></li>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=predeposit&op=get_cash_xml_data&'+$("#formSearch")
          .serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=predeposit&op=get_cash_xml_data'}).flexReload();
        $("#formSearch")[0].reset();
    });
    ...
    // })
    .flexigrid({
        url: 'index.php?act=predeposit&op=get_cash_xml_data',
        colModel : [
        ...
        ,buttons : [{
              display: '<i class="fa fa-file-excel-o"></i>申请核准',
              name : 'PFA',
              bclass : 'csv',
              title : '将选定的提现申请记录提交复审',
              onpress : payForAnother
        }]
        ,rp: 20
        ,rpOptions: [20, 50, 100]
        ,onSuccess: function(e){
          // var title =  this.title + '(￥' + this.count + ')';
          // alert(this.title)
          $('.flexigrid .ftitle h3').html('待审核-易宝 (￥' + this.count + ')')
...
function payForAnotherSend(ids)
{
    ...
    $.ajax({
        type: "POST",
        url: "index.php?act=predeposit&op=do_apply_approve",
        data: {'ids': idlist},
        dataType: "json",
        success: function(data){
          if (data == 1) {
