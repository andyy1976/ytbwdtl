...
function update_flex(){
	$('.ncap-stat-general-single').load('index.php?act=stat_yundou&op=get_plat_sale&'+$("#formSearch").serialize(),
		function(){
			$('.timer').each(count);
     	});
    $("#flexigrid").flexigrid({
        url: 'index.php?act=stat_yundou&op=get_sale_xml&'+$("#formSearch").serialize(),
        colModel : [
            {display: '操作', name : 'operation', width : 60, sortable : false, align: 'center', className: 'handle-s'},
            {display: '订单号', name : 'point_ordersn', width : 150, sortable : false, align: 'center'},
            {display: '买家', name : 'buyername',  width : 120, sortable : false, align: 'center'},
            {display: '下单时间', name : 'point_addtime',  width : 150, sortable : true, align: 'center'},
            {display: '兑换总积分', name : 'point_allpoint',  width : 120, sortable : true, align: 'center'},
            {display: '订单状态', name : 'point_orderstate',  width : 120, sortable : false, align: 'center'}
            ],
        buttons : [
...
function fg_operation(name, bDiv){
    var stat_url = 'index.php?act=stat_yundou&op=sale';
