...
$(function(){
    ...
    $("#flexigrid").flexigrid({
        ...
        buttons : [
            {display: '<i class="fa fa-check-square"></i>批量审核', name : 'batchcheck', bclass : 'del', title : '将选定行数据批量审核', onpress : fg_operate }        
            ],
        searchitems : [
...
function batch_check(id) {
  if (typeof id == 'number') {
      var id = new Array(id.toString());
  };
  if(confirm('批量操作有风险，确认批量审核这 ' + id.length + ' 项吗？')){
    id = id.join(',');
  } else {
        return false;
    }
  $.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?act=store&op=batchcheck",
        data: "batchcheck_id="+id,
