            ...
            <dd>
              <label>
                <select name="order_state" class="s-select">
                  ...
                  <option value="50">48小时未发货订单</option>
...
$(function(){
    ...
    $("#flexigrid").flexigrid({
		...
		<?php }else if($output['flag']==2){ ?>
		url: 'index.php?act=dm_order&op=get_xml',
		<?php }else if($output['flag']==3){ ?>
		url: 'index.php?act=sm_order&op=get_xml',
		<?php } ?>
        colModel : [
...
function get_sms(id,order_sn,order_id) {

  if(confirm('提醒后将不能恢复，确认提醒这1项吗？')){
        $.getJSON('index.php?act=order&op=get_sms', {
          store_id:id,
          order_sn:order_sn,
          order_id:order_id
        }, function(data){
