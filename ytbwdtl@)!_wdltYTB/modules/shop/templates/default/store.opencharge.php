...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=store&op=pan_get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=store&op=pan_get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: 'index.php?act=store&op=pan_get_xml',
        colModel : [
            {display: '操作', name : 'operation', width : 150, sortable : false, align: 'center', className: 'handle'},
            {display: '充值ID', name : 'id', width : 40, sortable : true, align: 'center'},
            {display: '会员ID', name : 'member_id', width : 40, sortable : true, align: 'center'},
            {display: '订单编号', name : 'openshop_order_sn', width : 150, sortable : true, align: 'left'},
            {display: '交易编号', name : 'openshop_pay_sn', width : 150, sortable : true, align: 'left'},
            {display: '缴费金额（元）', name : 'pay_amount', width : 100, sortable : true, align: 'center'},
            {display: '添加时间', name : 'add_time', width : 150, sortable : true, align: 'center'},
            {display: '支付方式', name : 'pay_style', width : 100, sortable : true, align: 'center'},
            {display: '支付状态', name : 'order_status', width : 100, sortable : true, align: 'center'},
            {display: '支付时间', name : 'pay_time', width : 100, sortable : true, align: 'center'},
            {display: '管理员', name : 'oper_admin', width : 60, sortable : true, align: 'center'}
            ],
        // buttons : [
        //        {display: '<i class="fa fa-file-excel-o"></i>导出数据', name : 'csv', bclass : 'csv', title : '导出Excel文件', onpress : fg_operation }
        //    ],
        searchitems : [
            {display: '充值ID', name : 'id'},
            {display: '充值编号', name : 'openshop_pay_sn'},
            {display: '会员ID', name : 'member_id'},
            {display: '管理员', name : 'oper_admin'}
            ],
        sortname: "id",
        sortorder: "desc",
        title: '开通店铺费用支付账单查询'
...
function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?act=store&op=recharge_del', {id:id}, function(data){
            if (data.state) {
