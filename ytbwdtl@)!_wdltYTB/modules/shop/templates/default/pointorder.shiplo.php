...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <div class="subject">
        <h3>云豆兑换 - 设置发货</h3>
        <h5>设置商城会员云豆礼品兑换的发货信息</h5>
      </div>
    </div>
  </div>

  <?php if (is_array($output['order_info']) && count($output['order_info'])>0){ ?>
  <form id="ship_form" method="post" name="ship_form" action="index.php?act=pointprod&op=order_ship_get&id=<?php echo $_GET['id']; ?>&type=2">
    <input type="hidden" name="form_submit" value="ok"/>
    <input type="hidden" name="type" value="<?php echo $output['order_info']['type']?>">
