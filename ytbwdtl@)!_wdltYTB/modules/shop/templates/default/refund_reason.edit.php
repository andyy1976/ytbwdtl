  ...
  <form id="post_form" method="post" name="form1" action="index.php?act=refund&op=edit_reason&reason_id=<?php echo $output['reason']['reason_id']; ?>">
    ...
    <div class="ncap-form-default">
      <dl class="row">
        ...
        <dl class="row">
        <dt class="tit">
          <label for="reason_info"><em>*</em>类型</label>
        </dt>
        <dd class="opt">
          <input type="radio" name="is_dm" value="0" <?php if($output['reason']['is_dm']==0){ echo 'checked="checked"'; }?>  />否
          <input type="radio" name="is_dm" value="1"  <?php if($output['reason']['is_dm']==1){ echo 'checked="checked"'; }?>   />是
          <span class="err"></span>
          <p class="notic"> </p>
        </dd>
      </dl>
