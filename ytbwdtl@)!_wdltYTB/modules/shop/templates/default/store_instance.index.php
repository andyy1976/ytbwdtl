...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?act=shop_instance&op=get_xml',
        colModel : [
        ...
        <!--buttons : [{display: '<i class="fa fa-file-excel-o"></i>导出数据', name : 'csv', bclass : 'csv', title : '将选定行数据导出CVS文件', onpress : fg_operation }	{display: '<i class="fa fa-plus"></i>新增数据', name : 'haoshop_add', bclass : 'add', title : '添加一条新数据到列表', onpress : fg_operations }				],-->
        searchitems : [
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=shop_instance&op=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=shop_instance&op=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    // //给推荐人提成
