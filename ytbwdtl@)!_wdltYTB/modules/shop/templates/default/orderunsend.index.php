...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo '待发货订单管理';?></h3>
        <h5><?php echo '商城实物商品交易待发货订单查询及管理';?></h5>
      </div>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=unsendorder&op=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=unsendorder&op=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?act=unsendorder&op=get_xml',
        colModel : [
...
function fg_cancel(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?act=unsendorder&op=change_state&state_type=cancel",
        data: "order_id="+id,
