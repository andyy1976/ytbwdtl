...
$(function(){
    ...
    $('#export').click(function(){
      ...
      window.open("index.php?act=store_checkgoods&op=export_csv&store_name="+store_name+"&query_start_date="+query_start_date+"&query_end_date="+query_end_date,"_blank");
    });
    //20171025潘丙福添加结束

    $("#flexigrid").flexigrid({
        url: 'index.php?act=store_checkgoods&op=get_goodsdetail_xml',
        colModel : [
            ...
            {display: 'SPU', name : 'goods_commonid', width : 120, sortable : false, align : 'left'},
            {display: '商品名称', name : 'goods_name', width : 120, sortable : false, align : 'left'},
            {display: '添加时间', name : 'goods_addtime', width : 120, sortable : false, align : 'left'}
        ],
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=store_checkgoods&op=get_goodsdetail_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=store_checkgoods&op=get_goodsdetail_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
