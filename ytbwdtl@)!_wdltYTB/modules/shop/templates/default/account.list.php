  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        ...
        <li><a href="JavaScript:void(0);" class="current">账户明细</a></li>
            ...
            <dd>
              <label>
                <select name="pdr_payment_state" class="s-select">
                  ...
                  <option value="0">充值记录</option>
                  <option value="1">消费记录</option>
                  <option value="10">会员激活</option>
                  <option value="17">端口激活</option>
                  <option value="2">分成记录</option>
                  <option value="24">注册分润</option>
                  <option value="18">升级端口分润</option>
                  <option value="3">消费分成</option>
                  <option value="4">分销分成</option>
                  <option value="5">商家分成</option>
                  <option value="6">充值提现</option>
                  <option value="7">余额提现</option>
                  <option value="8">分销提现</option>
                  <option value="9">商家提现</option>
                  <option value="15">省代余额提现</option>
                  <option value="16">代理余额提现</option>
                  <option value="11">充值转云豆</option>
                  <option value="12">充值手续费</option>
                  <option value="13">5%购买云豆</option>
                  <option value="14">8%购买云豆</option>
                  <option value="19">随行付每日充值</option>
                  <option value="20">随行付每日购买</option>
                  <option value="21">银联支付每日充值</option>
                  <option value="22">银联支付每日购买</option>
                  <option value="23">乐支付每日充值</option>
                </select>
              </label>
            </dd>
          </dl>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=predeposit&op=account_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=predeposit&op=account_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: 'index.php?act=predeposit&op=account_xml',
        colModel : [
            ...
            {display: '金额（元）', name : 'pdr_amount', width : 100, sortable : true, align: 'center'},
            {display: '时间', name : 'pdr_add_time', width : 100, sortable : true, align: 'center'},
            {display: '说明', name : 'pdr_desc', width : 300, sortable : true, align: 'center'}
            ],
        buttons : [
