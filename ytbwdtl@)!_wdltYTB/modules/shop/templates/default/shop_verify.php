      ...
      <div id="searchCon" class="content">
        <div class="layout-box">
          ...
          <dl>
            ...
            <dd>
              <label>
                <select name="shop_type" id="order_type" class="s-select">
                  <option value="">-请选择-</option>
                  <option value="0">入驻商家</option>
...
function update_flex(){
  $('.ncap-stat-general-single').load('index.php?act=shop_verify&op=get_plat_sale&'+$("#formSearch").serialize(),
    function(){
      $('.timer').each(count);
      });

    $("#flexigrid").flexigrid({
        url: 'index.php?act=shop_verify&op=get_sale_xml&'+$("#formSearch").serialize(),
        colModel : [
            {display: '店铺ID', name : 'store_id', width : 60, sortable : false, align: 'center', className: 'handle-s'},
            {display: '店铺名称', name : 'store_name',  width : 200, sortable : false, align: 'center'},
            {display: '审核商品数量', name : 'totalnum', width : 150, sortable : false, align: 'center'},
            {display: '店铺类型', name : 'store_type',  width : 120, sortable : true, align: 'center'},
            ],
        buttons : [
...
function fg_operation(name, bDiv){
    var stat_url = 'index.php?act=shop_verify&op=sale';
