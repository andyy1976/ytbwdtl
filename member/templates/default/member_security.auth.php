...
<div class="wrap">
  ...
  <div class="alert alert-success">
    ...
    <ul>
      ...
      <li>1.  <a href="index.php?act=member_security&op=auth&type=modify_mobile">绑定手机</a> 后通过接收手机短信完成验证。</li>
      <li>2. 请正确输入下方图形验证码，如看不清可点击图片进行更换，输入完成后进行下一步操作。</li>
...
$(function(){
  ...
  $('#send_auth_code').on('click',function(){
    ...
    ALLOW_SEND = !ALLOW_SEND;
    $('#sending').show();
    $.getJSON('index.php?act=member_security&op=send_auth_code',{type:$('#auth_type').val(),mobile:<?php echo $output['member_info']['member_mobile']; ?>},function(data){
      if (data.state == 'true') {
        $('#sending').hide();
        $('#show_times').html(60);
