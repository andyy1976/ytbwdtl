    ...
    <table class="ncm-default-table order">
      ...
      <tbody>
        ...
        <tr>
         ...
         <!--  <td class="bdl" rowspan="<?php echo count($output['prod_list']);?>"><?php if($output['order_info']['point_orderstatetext']=='待发货'){echo '待发货';}else{ echo '处理中';} ?></td>
          <td class="bdl" rowspan="<?php echo count($output['prod_list']);?>"> -->
<!--           <?php if ($output['order_info']['point_orderallowcancel']) { ?>
            <p><a href="javascript:void(0)" class="ncbtn ncbtn-bittersweet">处理中无法申请退款</a></p>
            <?php } ?> -->
            <?php if ($output['order_info']['point_orderallowship']) { ?> 
                <p><a href="javascript:void(0)" class="ncbtn" onclick="ajax_confirm('<?php echo $lang['member_pointorder_confirmreceivingtip']; ?>','<?php echo MEMBER_SITE_URL;?>/index.php?act=member_pointorder&op=cancel_order&order_id=<?php echo $val['point_orderid']; ?>');">取消兑换订单</a></p>
            <?php }?>
