...
<script type="text/javascript" src="<?php echo LOGIN_RESOURCE_SITE_URL;?>/js/connect_sms.js" charset="utf-8">
                  ...
                  <script language="javascript" type="text/javascript">
                             ...
                             $.ajax({
                             type: "GET",
                                 url: "index.php?act=login&op=check_member&username="+username,
                                 success: function(data){
                                          //alert(data);
                              if(data=='true'){
                                $('#username-tip').html('可以注册');
                                }else{
                                $('#username-tip').html('用户名已经被占用');  
                                  }
                                    }
                              });
                             }
                        ...
                        function checkmobile(){
                             ...
                             if(!myreg.test($("#phone").val())) 
                                        { 
                                         $('#mobile-tip').html('请输入有效的手机号码');
                                          return false; 
                                       } 
                             $.ajax({
                             type: "GET",
                                 url: "index.php?act=login&op=check_moblie&phone="+phone,
                                 success: function(data){
                                          //alert(data);
                              if(data=='true'){
                                $('#mobile-tip').html('手机号可以注册');
                                }else{
                                $('#mobile-tip').html('手机号已经被占用');  
                                  }
                                    }
                              });
                             }
        ...
        <div class="nc-register-mode">
          ...
          <div id="tabs_container" class="tabs-container">
            <div id="default" class="tabs-content">
              <form id="register_form" method="post" class="nc-login-form" action="<?php echo urlLogin('login', 'usersave');?>" onsubmit="return checkallvalue()">
                ...
                <dl style="display: none">
                  ...
                  <dd>
                    <input type="text" id="user_name" name="user_name" class="text" tipMsg="<?php echo $lang['login_register_username_to_login'];?>" onblur="return checkname()" /><span id="username-tip" style="font-size: 14px; color: red; position: absolute; margin-left: 50px; height: 15px; width:400px;"></span>
                  </dd>
                </dl>
                ...
                <dl style="display: none">
                 ...
                 <dl>
                  <dt>安全密码：</dt>
                  <dd>
                    <input type="password" id="paypwd" name="paypwd" class="text" tipMsg="<?php echo $lang['login_register_password_to_login'];?>"/>
                  </dd>
                </dl>
                ...
                <dl class="mt15">
                  <dt>推荐人ID：</dt>
                  <dd>
                    <input type="text" id="member_pid" name="member_pid" class="text" tipMsg="请填写你的推荐人ID"/>
                  </dd>
                </dl>

                <dd class='member_pname' id='member_pname'   style="margin-top: -45px; position: absolute; margin-left: 410px;width:200px"></dd>
                ...
                <?php } ?> -->
                  ...
                  <dt>手机号：</dt>
                  <dd>
                    <input type="text" class="text" tipMsg="请输入手机号码" placeholder="默认为用户名" autocomplete="off" value="" name="phone" id="phone" onblur="return checkmobile();"><span id="mobile-tip" style="font-size: 14px; color: red; position: absolute; margin-left: 50px; height: 15px; width:400px;"></span>
                  </dd>
                  </dl>
                ...
                <div class="code-div" style="display: none">
                  <dl>
                    <dt><?php echo $lang['login_register_code'];?>：</dt>
                    <dd>
                      <input type="text" name="captcha" class="text w100" id="image_captcha" value="1234" size="10" tipMsg="<?php echo $lang['login_register_input_code'];?>" />
                    </dd>
                  </dl>
                  <span>
                      <img src="index.php?act=seccode&op=makecode&type=50,120&nchash=<?php echo getNchash();?>" title="<?php echo $lang['login_index_change_checkcode'];?>" name="codeimage" id="sms_codeimage">
                      <a class="makecode" href="javascript:void(0);" onclick="javascript:document.getElementById('sms_codeimage').src='index.php?act=seccode&op=makecode&type=50,120&nchash=<?php echo getNchash();?>&t=' + Math.random();"><?php echo $lang['login_password_change_code']; ?></a>
                  </span> 
                </div>

                <div class="tiptext" id="sms_text">点击<span><a href="javascript:void(0);" onclick="get_sms_captcha('1')"><i class="icon-mobile-phone"></i>发送短信验证</a></span>，并将您手机短信所接收到的“6位验证码”输入到下方短信验证，再提交下一步。</div>
                <dl>
                  <dt>短信验证：</dt>
                  <dd>
                ...
                <dl class="clause-div">
                  <dd>
                    ...
                    <?php echo $lang['login_register_agreed'];?><a href="<?php echo urlShop('document', 'index',array('code'=>'agreement'));?>" target="_blank" class="agreement" title="<?php echo $lang['login_register_agreed'];?>"><?php echo $lang['login_register_agreement'];?></a><a href="/member/index.php?act=article&op=show&article_id=43" target="_blank" class="agreement" title="隐私政策">《隐私政策》</a></dd>
                </dl>
              ...
              <script language='javascript'>
                ...
                $('#member_pid').bind('input propertychange', function(){
                    ...
                    $.post("index.php?act=login&op=checkpid",{
                      pid:pid
                    },function(data){
                      var datas=eval('('+data+')');
                      // alert(datas.area_name);

                      $('#member_pname').html('<p style="float: left">推荐人用户名：</p><h1 style="float: left;color: red">'+datas.member_name+'</h1>');
                    });
                });
          ...
          var password_confirm = password /*$('#password_confirm').val();*/
          var paypwd           = $('#paypwd').val();
          var mobile           = $('#mobile-tip').text();
          var username         = mobile /*$('#username-tip').text();*/
          var myreg            =  /^(((13[0-9]{1})|(15[0-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/;
          ...
          // return false;
					...
					if(password==""){
						window.alert('错误!不能密码为空!');
						return false;
						}
					  if(password_confirm==""){
					    window.alert('错误！确认密码不能为空！');
						return false;
					  }
					 if(paypwd==""){
						 window.alert('错误！支付确认密码不能为空！');
						return false;
						 }
					if(password!=password_confirm){
						window.alert('两次密码不一致，请检测您的密码！');
						return false;
						}
					if(!myreg.test($("#phone").val())) 
                      { 
				    ...
				    if(username.indexOf("已")>0){
						alert('用户名已经被占用！');
						return false;
						}
					  if(mobile.indexOf("已")>0){
						alert('手机号码已经被占用！');
						return false;
						}
				return true;
...
<!--             <?php if (C('sms_register') == 1){?>
      ...
      <div class="right">
        ...
        <?php } ?>
        <div class="reister-after" style="display:none;">
    ...
    $("#register_form").validate({
        ...
        rules : {
            ...
            member_pid : {
                ...
                member_pid    : true,
                remote   : {
                    url : 'index.php?act=login&op=check_pid',
                    type: 'get',
                    data:{
                        member_pid : function(){
                            return $('#member_pid').val();
                        }
                    }
                }
            },
        ...
        messages : {
            ...
            member_pid : {
                required : '<i class="icon-exclamation-sign"></i><?php echo '您的推荐人选项不能位空';?>',
                member_pid    : '<i class="icon-exclamation-sign"></i><?php echo '您的推荐人ID不存在或者没有激活会员';?>',
