...
<div class="wrap">
  ...
  <h4>云豆获得规则</h4>
<ul><li>成功注册会员：增加<em><?php echo C('points_reg');?></em>云豆；会员每天登录：增加<em><?php echo C('points_login');?></em>云豆；评价完成订单：增加<em><?php echo C('points_comments');?></em>云豆。</li>
<?php if (C('points_orderrate')) {?><li>购物并付款成功后将获得订单总价<?php printf('%d', 1/C('points_orderrate') * 100);?>%<?php if (C('points_ordermax')) {?>（最高限额不超过<?php echo C('points_ordermax');?>）<?php }?>云豆。</li><?php }?>
<li>如订单发生退款、退货等问题时，云豆将不予退还。</li></ul>
  </div> -->
