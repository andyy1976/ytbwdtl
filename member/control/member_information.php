...
class member_informationControl extends BaseMemberControl {
    ...
    public function memberOp() {
        ...
        $area = Model('area');
        $abc=$model_member->where(array('member_id'=>$_SESSION['member_id']))->find();
        if (chksubmit()){
            $member_array   = array();
            ...
            if(!$abc['member_bankname']){
               $member_array['member_bankname']    = $_POST['member_truename'];
            }
//          $area_name_arr = explode(' ',$_POST['region']);
//          $provinceid=$area->getfby_area_name($area_name_arr[0],'area_id'); 
//          if(!$provinceid){$provinceid=0;} 
//          $member_array['member_provinceid']  = $provinceid; 
//          $cityid=$area->getfby_area_name($area_name_arr[1],'area_id'); 
//          if(!$cityid){$cityid=0;}          
//          $member_array['member_cityid']      = $cityid; 
//          $areaid=$area->getfby_area_name($area_name_arr[2],'area_id');
//          if(!$areaid){$areaid=0;}
//          $member_array['member_areaid']      = $areaid;
...
public function uploadOp() {
    ...
    public function avatarOp() {
        ...
        Tpl::output('menu_sign_url','index.php?act=member_information&op=member');
        Tpl::output('menu_sign1','avatar');
        Tpl::showpage('member_profile.avatar2');
    }

    public function avatarsetOp() {
        if (isset($_GET['img'])) {
            ...
            if ( !$mobile ) {
                // MEMBER_SITE_URL . 
                $url = '/index.php?act=member&op=home';
                header('Location: '. $url);
            } else {
                echo $update ? 1 : 0;
            }
