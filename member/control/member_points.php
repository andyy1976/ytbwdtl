...
class member_pointsControl extends BaseMemberControl {
    ...
    private function profile_menu($menu_key='',$array=array()) {
        $menu_array = array(
            1=>array('menu_key'=>'points',  'menu_name'=>'云豆明细',    'menu_url'=>'index.php?act=member_points'),
            2=>array('menu_key'=>'orderlist','menu_name'=>'云豆兑换',    'menu_url'=>'index.php?act=member_pointorder&op=orderlist')
        );
        if(!empty($array)) {
            $menu_array[] = $array;
        }
