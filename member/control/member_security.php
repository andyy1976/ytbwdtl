...
class member_securityControl extends BaseMemberControl {
   ...
   $model_member = Model('member');
        ...
        if (chksubmit(false,true)) {
            ...
            Tpl::output('member_code',$_POST['auth_code']);
            ...
            if (!in_array($_GET['type'],array('modify_pwd','modify_mobile','modify_paypwd','pd_cash'))) {
                redirect('index.php?act=member_security&op=index');
            }
            ...
            $member_info = $this->member_info;
            if (!$member_info){
                $member_info = $model_member->getMemberInfo(array('member_id'=>$_SESSION['member_id']),'member_mobile,member_mobile_bind');
            }
            ...
            $member_info['member_mobile_bind'] == '0') {
                showMessage('请先绑定手机','index.php?act=member_security&op=index','html','error');
            }
    ...
    public function send_auth_codeOp() {
       ...
       if ($_GET['type'] == 'email') {
        ...
        $result = true ;
         $member = $model_member->getMemberCommonInfo(array('member_id'=>$_SESSION['member_id']));
        $s = array(
        ...
        if ($result) {
            $data = array();
            $update_data['member_id'] = $_SESSION['member_id'];
    ...
    public function modify_pwdOp() {
        ...
        if (empty($member_common_info) || !is_array($member_common_info)) {
            showMessage('验证失败1','','html','error');
        }
        if (TIMESTAMP - $member_common_info['send_acode_time'] > 2400) {
            showMessage('验证码已失效，请重新获取验证码','','html','error');
        }
        if ($member_common_info['auth_code'] != $_POST['code']) {
            showMessage('验证失败2','','html','error');
        }
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
    ...
    public function modify_mobileOp() {
        ...
        if (chksubmit()) {
            ...
            if ($member_common_info['second_code'] != intval($_POST['code'])) {
                showMessage('验证失败','','html','error');
            }
            if (TIMESTAMP - $member_common_info['send_acode_time'] > 1800) {
    ...
    public function predepositOp() {
        $model_member = Model('member');
        $member_info=$model_member->where(array('member_id'=>$_SESSION['member_id']))->field('agent_predeposit,available_predeposit,member_predeposit,distributor_predeposit,dmyd_moneyyes')->find();
        if($_POST['predeposit_type'] ==1){
            echo $member_info['available_predeposit'];
        }elseif($_POST['predeposit_type'] ==2){
            echo $member_info['member_predeposit'];
        }elseif($_POST['predeposit_type'] ==3){
            echo $member_info['distributor_predeposit'];
        }elseif($_POST['predeposit_type'] ==5){
            echo $member_info['province_predeposit'];
        }elseif($_POST['predeposit_type'] ==6){
            echo $member_info['agent_predeposit'];
        }elseif($_POST['predeposit_type'] ==8){
            echo $member_info['dmyd_moneyyes'];
        }
    }       
}
