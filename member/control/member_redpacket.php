...
class member_redpacketControl extends BaseMemberControl{
    ...
    public function rp_listOp(){
    	$member = Model('member');
    	$bankcard=$member->getfby_member_id($_SESSION['member_id'],'member_bankcard');
    	/*
        $model_redpacket = Model('redpacket');
        //更新红包过期状态
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'rp_list','menu_name'=>'我的银行卡','menu_url'=>'index.php?act=member_redpacket&op=rp_list'),
            2=>array('menu_key'=>'rp_binding','menu_name'=>'绑定/修改银行卡','menu_url'=>'index.php?act=member_redpacket&op=rp_binding'),
        );
    ...
    public function blind_cardOp() {
       ...
       $card=$_POST['card'];
       //$pwd=md5($_POST['pwd']);
       $member_bankname=$_POST['card_name'];         
       $member_info=$member->where(array('member_id'=>$_SESSION['member_id']))->find();
       if($member_info['member_bankname']!=$_POST['card_name'] && !empty($member_info['member_bankname']) ){
            $member_bankname=$member_info['member_bankname'];
       }
       //查询卡号是否被其它号绑定，如有则设置失败
       $member_card=$member->where(array('member_id'=>array('neq',$member_info['member_id']),'member_bankcard'=>$card))->find();
       if($member_card){echo '4';exit();}
       //if($pwd!=$member_info['member_paypwd']){echo '1';exit();}
       //判断是否超过修改次数
       if($member_info['bankcard_editor'] >=3){
           echo '5';exit();
       } 
       if($member_info['member_bankcard']){     
	       $update=$member->where(array('member_id'=>$_SESSION['member_id']))->update(array('member_bankcard'=>$card,'member_bankname'=>$member_bankname));
	       $member->where(array('member_id'=>$_SESSION['member_id']))->setInc('bankcard_editor',1);
	       if($update){echo '2';}else{echo '3';}
       }else{
       	   $update=$member->where(array('member_id'=>$_SESSION['member_id']))->update(array('member_bankcard'=>$card,'member_bankname'=>$member_bankname));
	       if($update){echo '2';}else{echo '3';}
       }
