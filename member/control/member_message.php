...
class member_messageControl extends BaseMemberControl {
	...
	*接收个人反馈的回复信息
	*/
	public function messagebcOp(){
		  $model_messagebc = Model('mb_feedback');
		  $model_messageback=Model('mb_backfeedback');
		  $member_id=intval($_SESSION['member_id']);
		  $page = new Page();
		  $page -> setEachNum(10);
		  $page -> setStyle('admin');
		  $message_array  = $model_messagebc->listMessage(array('member_id'=>$_SESSION['member_id']),$page);
		  //var_dump($message_array);
		  Tpl::output('message_array',$message_array);
		  Tpl::output('show_page',$page->show());
		  Tpl::output('drop_type','messageback');
          $this->profile_menu('messageback');
          Tpl::showpage('member_message.box.back');
		}
    /**
     * 收到(普通)站内信列表
     *
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
		    array('menu_key'=>'messageback', 'menu_name'=>'互动消息','menu_url'=>'index.php?act=member_message&op=messagebc'),
