...
function find_agent($member_id){
	...
	//区带分成150
	$area=$member->where(array('member_level'=>'3','member_areaid'=>$member_info['member_areaid']))->field('distributor_predeposit,member_id,member_name')->find();
	$area_update['distributor_predeposit']=array('exp','distributor_predeposit+150');
	$member->where(array('member_id'=>$area['member_id']))->update($area_update);
	
	//市代分成100
	$city=$member->where(array('member_level'=>'4','member_cityid'=>$member_info['member_cityid']))->field('distributor_predeposit,member_id,member_name')->find();
	$city_update['distributor_predeposit']=array('exp','distributor_predeposit+100');
	$member->where(array('member_id'=>$city['member_id']))->update($city_update);
	//省代分成50
	$provinceid=$member->where(array('member_level'=>'5','member_provinceid'=>$member_info['member_provinceid']))->field('distributor_predeposit,member_id,member_name')->find();
	$provinceid_update['distributor_predeposit']=array('exp','distributor_predeposit+50');
...
//升级会员
 ...
 function send($mobile,$pdr_sn){
        ...
        switch ($log_type) {                
            case '3':                    
            $log_msg .= '尊敬的店主，您有新的发货订单，订单号为：'.$pdr_sn.'为了不影响您的店铺正常运行，请您务必在一个小时内进行处理。';
                break;
            default:
                $state = false;
                $msg = '参数错误';
