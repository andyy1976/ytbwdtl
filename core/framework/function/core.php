...
function dmfclog($uid,$ismoney,$nomoney,$order_id){    //地面分成记录函数
    ...
    */
        ...
        function checkpoint($vid,$sid){
         ...
         if($store_info['store_flag']==1&&$store_info['custom_pointa']<$single_order['add_time']&&$ygoods_id['isdmgoods']==1&&$single_order['is_dm']>0){  //判断是否是实体店铺并且订单时间大于申请成为店铺的时间
        if($ygoods_id['usertc']==2){  //C套餐入库并给商家添加钱包
           $sjyd = round($lgoods_info['goods_points']);
           $data = array();  //给消费者添加云豆及日志
           $data['member_points'] = array("exp","member_points-$sjyd");
           $updatee = $member->where(array('member_id'=>$lgoods_info['buyer_id']))->update($data);   //扣除消费者消费云豆
            ...
            if($updatee){     //更新日志表
              ...
              $pd['pl_adminname']='';
              $pd['pl_addtime']=time();
              $pd['pl_desc']='订单：'.$single_order['order_sn'].',消费云豆';
              $insert=$pd_log->insert($pd);
             }
            $rcb   = Model('rcb_log');                     //冻结钱包加订单的50%--新结算方式
            $dat['wallet_release']=array('exp','wallet_release+'.$lgoods_info['goods_pay_price']);
            $log['available_amount']=$lgoods_info['goods_pay_price'];
            $log['description']='商家获得全部货款-订单号：'.$single_order['order_sn'];
            $log['member_id']=$store_info['store_id'];
            $log['member_name']=$store_info['store_name'];
            $log['type']='seller';
            $log['add_time']=time();
            $log['freeze_amount']=0;
            $rcb->insert($log);                       
            $upstore=$model_seller->table('seller')->where(array('store_id'=>$store_info['store_id']))->update($dat);

        }else{
        if($ygoods_id['usertc']==0){    //设置为A套餐
              $sjyd=round($lgoods_info['goods_pay_price']*0.5);
            }

        if($ygoods_id['usertc']==1){    //前台设置为B套餐
            if($ygoods_id['pointsb']!=0){
            $sjyd = round($lgoods_info['goods_pay_price']*$ygoods_id['pointsb']);
            }else{
            $sjyd = round($lgoods_info['goods_pay_price']*0.6);          
            } 
          }
          ...
          if($updatee){     //更新日志表
           ...
           if($ygoods_id['usertc']==0){  //A套餐的情况
              $ds=$lgoods_info['goods_pay_price']*0.05;   //修改后
            }
           if($ygoods_id['usertc']==1){  //B套餐的情况
            //$ds = $lgoods_info['goods_pay_price']*(0.5+$ygoods_id['pointsb'])*10*0.008;  
            $ds = $lgoods_info['goods_pay_price']*$ygoods_id['pointsb']*0.1;   
           }
           ...
           //$data['wallet_frozen'] =($chang_seller_wallet['wallet_release']-$ds)*0.5;
           $chang_seller_wallet_result=$model_seller->where(array('store_id'=>$store_info['store_id']))->update($data);
          if($chang_seller_wallet_result){   //商家货款入库
            $rcb   = Model('rcb_log');                     //冻结钱包加订单的50%--新结算方式
            $dat['wallet_release']=array('exp','wallet_release+'.$lgoods_info['goods_pay_price']);
            $log['available_amount']=$lgoods_info['goods_pay_price'];
            $log['description']='商家获得全部货款-订单号：'.$single_order['order_sn'];
 ...
 function port_give_point($member_id,$other_id,$other_name,$points,$pwd,$code){
	...
	// if($member_id=='10088' || $member_id=='10072'){
		...
		$member->where(array('member_id'=>$member_id))->update(array('member_points'=>array('exp','member_points-'.$points_1),'points_code'=>$points_code_1));
	   	//设置返现时间
	   	if(empty($other_info['member_points']) || empty($other_info['return_time'])){
            $date=strtotime(date("Y-m-d"));    
        }else{
            $date=$other_info['return_time'];
        } 
	    $member->where(array('member_id'=>$other_id))->update(array('member_points'=>array('exp','member_points+'.$points),'points_code'=>$points_code_2,'return_time'=>$date));
...
function dmsm_pay($order_id){         //地面商家扫描支付扣款及入库 $order_id 订单ID
	  $ydmbs = Model()->table('orders')->where(array('order_id'=> $order_id))->find();
	  $store_info = Model()->table('store')->where(array('store_id'=>$ydmbs['store_id']))->find(); 
	  if($ydmbs['is_dm']==1&&$store_info['store_flag']==1){
	   $xukoumoney = $ydmbs['order_amount']*0.05;
       $model_order = Model('order');
       $store = Model('seller');
       $rcb   = Model('rcb_log');                     //冻结钱包加订单的50%--新结算方式
       $pd_log=Model("points_log");          //日志文件
            //$dat['wallet_frozen']=array('exp','wallet_frozen+'.($ydmbs['order_amount']-$xukoumoney)*0.5);
            ...
            $data['log_role'] = $store_info['member_name'];
            $data['log_msg'] = $store_info['member_name']."确认订单";
            $data['log_user'] = $store_info['member_name'];
           ...
           $sjdata['member_points'] = array("exp","member_points+$bs");
           $sjupdatee = Model()->table('member')->where(array('member_id'=>$store_info['member_id']))->update($sjdata);
		   $datas = array();  //给消费者添加云豆及日志
		   $sjyd = round($ydmbs['order_amount']*0.5);
          ...
          give_dmchief($store_info['member_id'],$xukoumoney,$ydmbs['order_sn'],2);    //给店铺商家提成及分
		  Model()->table('orders')->where(array('order_id'=>$order_id))->update(array('order_state'=>50));
		   
			}
			
	}
  }
 function dm_orderupdateOp($order_id){
   if(!empty($order_id)){
       $model_order = Model('order');
       $ydmbs = Model()->table('orders')->where(array('order_id'=>$order_id))->find();
