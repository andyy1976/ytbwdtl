...
class UploadFile{
	...
	public function upfile($field,$oss = false){
        ...
        //20170611潘丙福添加商品图片主图必须为正方形限制
        if ($field == 'goods_image') {
        	$panImageInfo = getimagesize($this->upload_file['tmp_name']);
        	if ($panImageInfo[0] != $panImageInfo[1]) {
        		$this->setError('上传商品图片必须为正方形图片！
万店通联建议您使用尺寸800x800像素以上、大小不超过400KB的正方形图片！');
            	return false;
        	}
        }
