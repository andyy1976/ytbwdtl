...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mt10">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
      <?php if (mb_strlen($sale['buyer_name']) > 4) {
        echo mb_substr($sale['buyer_name'], 0, mb_strlen($sale['buyer_name'])-4, 'UTF-8').'****';
        } else {
          echo mb_substr($sale['buyer_name'], 0, mb_strlen($sale['buyer_name'])-2, 'UTF-8').'**';
        }
      ?>
      </td>
      <td><em class="price"><i style="color:red;"><?php echo $lang['currency'].ncPriceFormat($sale['goods_marketprice']);?></i></em></td>
