...
<link href="<?php echo SHOP_TEMPLATES_URL;?>/css/app.css" rel="stylesheet" type="text/css">
   ...
   <!-- 焦点图 -->
    ...
    <div class="ncs-goods-summary">
      <div class="name">
        ...
        <strong style="color:#c40000"><?php echo str_replace("\n", "<br>", $output['goods']['goods_jingle']);?></strong>
      </div>
            <!-- 购买云豆  START 7月13号 z-->
      <?php if($output['goods']['gc_id']=='10424' || $output['goods']['gc_id']=='10425'){ ?>
      <!-- 判断5%比例还是8%比例转换 START-->
      <?php if($output['goods']['gc_id']=='10424'){ ?>
        <input type='hidden' value='1' id='type'>
      <?php } elseif($output['goods']['gc_id']=='10425'){?>
      ...
      <div class="ncs-meta">
        <dl>
          <dt>金&nbsp;&nbsp;额：</dt>
          <dd class="price">
                        <strong><input type="text" name='money' id='money'   onkeyup="this.value=this.value.replace(/[^0-9]/g,'')" onafterpaste="this.value=this.value.replace(/[^0-9]/g," onblur='give_points()'></strong>
                      </dd>
        </dl>
        <dl>
          <dt>云&nbsp;&nbsp;豆&nbsp;&nbsp;数：</dt>
      ...
      <div class="ncs-buy">
        ...
        <div class="ncs-btn">
                    <!-- 加入购物车--> 
                              ...
                              <!-- 立即购买--> 
          <a href="javascript:void(0);" nctype="buynow_submit" class="buynow " id="buy_points" title="立即购买">立即购买</a>
    ...
    <script type="text/javascript">
        ...
        $("#buy_points").click(function(){
          ...
          window.location.href='index.php?act=goods&op=predeposit_points&money='+money+'&type='+type+'&points='+points;
        });
    </script>
      <!-- 核算云豆 END -->
      <?php }else{ ?>
      ...
      <div class="ncs-meta">
        ...
        <dl>
          ...
          <dd class="price">
            ...
            <strong><i><?php echo $lang['currency'];?></i><?php echo ncPriceFormat($output['goods']['promotion_price']).'+'.$output['goods']['goods_points'].'云豆';?></strong><em>(原售价<?php echo $lang['nc_colon'];?><?php echo $lang['currency'].ncPriceFormat($output['goods']['goods_price']);?>)</em>

            <?php } else if ($output['goods']['goods_points']) {?>
            <strong><?php echo $lang['currency'].ncPriceFormat($output['goods']['goods_price']).'+'.$output['goods']['goods_points'].'云豆';?></strong>
            <?php } else {?>
        ...
        <dl>
          ...
          <dd class="price"><strong><?php echo $lang['currency'].ncPriceFormat((floatval($output['goods']['book_down_payment']) + floatval($output['goods']['book_final_payment']))).'+'.($output['goods']['book_down_payment_points']+$output['goods']['book_final_payment_points']).'云豆';?></strong></dd>
        </dl>
        <div class="ncs-book-down">
          <div class="rule-price"><strong>定金</strong><?php echo $lang['currency'].ncPriceFormat($output['goods']['book_down_payment']).'+'.$output['goods']['book_down_payment_points'].'云豆';?></div>
          <div class="rule-symbol">+</div>
          <div class="rule-price"><strong>尾款</strong><?php echo $lang['currency'].ncPriceFormat($output['goods']['book_final_payment']).'+'.$output['goods']['book_final_payment_points'].'云豆';?> </div>
          <div class="rule-info"><a href="javascript:void(0);">预定规则<i class="icon-question-sign"></i>
            <ul>
              <span class="arrow"></span>
              <li>1.定金下单后，请在<?php echo ORDER_AUTO_CANCEL_TIME;?>小时内付款，超时将自动关闭订单。</li>
      ...
      <?php if (isset($output['goods']['promotion_type']) || !empty($output['goods']['jjg_explain']) || !empty($output['mansong_info'])) {?>
        <dl>
          ...
          <dd class="promotion-info">
            ...
            <span class="sale-rule w400" style="color:#c40000">直降<em style="color:#16ac0a"><?php echo $lang['currency'].ncPriceFormat($output['goods']['down_price']);?></em>
...
$shownLevelSkus[$sku] = true; ?>
      ...
      <?php }?>
      <!-- E 促销 -->
      <?php if($output['goods']['isdmgoods']==0){ ?>
      ...
      <div class="ncs-buy">
        ...
        <?php  if (intval($output['goods']['gc_id'])==10351){ ?>
         <input type="text" name="" id="quantity" value="1" readonly="readonly" size="3" maxlength="6">
        <?php }else{ ?>
          <input type="text" name="" id="quantity" value="1" size="3" maxlength="6" class="input-text" <?php if ($output['goods']['is_fcode'] == 1 ) {?>readonly<?php }?>>
          <a href="javascript:void(0)" class="increase" <?php if ($output['goods']['is_fcode'] != 1) {?>nctype="increase"<?php }?>>&nbsp;</a> <a href="javascript:void(0)" class="decrease" <?php if ($output['goods']['is_fcode'] != 1) {?>nctype="decrease"<?php }?>>&nbsp;</a><?php } ?> </div>
        ...
        <div class="ncs-btn">
          ...
          <!-- 加入购物车--> 
           <?php  if (intval($output['goods']['gc_id'])==10351){ ?>
           <?php }else{ ?>
          <?php if($output['goods']['isdmgoods']!=1){ ?>
        ...
        <!--地面店铺地址-->
         <?php if($output['goods']['isdmgoods']!=0){  echo "<p><br><span style='font-size:18px; color:red'>店铺地址:</span><span style='font-size:18px; color:red'>".$output['store_address']."</span></p>";
		  echo "<p><br><span style='font-size:18px; color:red'>温馨提示:该商品为线下店铺商品，购买后请到店消费！</span></p>";
		 } ?>
        <!--地面店铺地址--> 
      </div>
      <?php }else{?>
...
<div id="content" class="ncs-goods-layout expanded" >
            ...
            <tr>
              <td width="300" height="50" align="right"><table width="500" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td width="70%" height="50"><?php echo $output['validate']; ?></td>
                </tr>
                <tr>
                  <td width="300" height="50" align="right">使用时间：</td>
                  <td height="50"><?php echo $output['usertime']; ?></td>
                </tr>
                <tr>
                  <td width="300" height="50" align="right">预约提醒：</td>
                  <td height="50"><?php echo $output['attationpeople']; ?></td>
                </tr>
                <tr>
                  <td width="300" height="50" align="right">其他费用：</td>
                  <td height="50"><?php echo $output['otherfree']; ?></td>
                </tr>
                <tr>
                  <td width="300" height="50" align="right">其他优惠：</td>
                  <td height="50"><?php echo $output['othercoupon']; ?></td>
                </tr>
                <tr>
                  <td width="300" height="50" align="right">使用规则：</td>
                  <td height="50"><?php echo $output['otherglue']; ?></td>
                </tr>
               <!-- <tr>
  ...
  <tr>
    ...
    <td height="50"><?php echo $output['suitpepole']; ?></td>
  </tr>
  <tr>
  <tr>
    <td width="300" height="50" align="right">其他费用：</td>
 ...
 <tr>
  ...
  <tr>
    ...
    <td height="50"><?php echo $output['ruzhutime']; ?></td>
  </tr>
  <tr>
    <td width="300" height="50" align="right">入住须知：</td>
    <td height="50"><?php echo $output['shopknow']; ?></td>
  </tr>
  <tr>
    <td width="300" height="50" align="right">使用规则：</td>
    <td height="50"><?php echo $output['otherglue']; ?></td>
  ...
  <tr>
    ...
    <td height="50"><?php echo $output['otherpeole']; ?></td>
  </tr>
  <!--<tr>
    <td width="300" height="50" align="right">套餐内容：</td>
    <td height="50"><textarea name="suitcontent" id="suitcontent"><?php echo $output['suitcontent']; ?></textarea></td>
    ...
    <td height="50"><textarea name="suitcontent" id="suitcontent"><?php echo $output['suitcontent']; ?></textarea></td>
       ...
       <dl>
        ...
        <dd nc_type="no_spec">
         ...
         <div>
<table width="100%" border="1" cellpadding="1" cellspacing="1" id="SignFrame"  >
      <tr id="trHeader">
	  ...
	  ?>
       <tr bgcolor="#FFFFFF">
		 <td width="100" height="30" align="center" valign="middle"  ><?php echo $output['txtName'][$i]; ?></td>
        <td width="100" height="30" align="center" valign="middle" ><?php echo $output['txtDanj'][$i]; ?></td>
        <td width="100" height="30" align="center" valign="middle" ><?php echo $output['txtSulian'][$i]; ?></td>
        <td width="100" height="30" align="center" valign="middle" ><?php echo $output['txtTel'][$i]; ?></td>
        </tr>
			 <?php 
		 }} ?>
    </table>
        ...
        <!--商品详情页广告图 -->
         <!--  <script>
             window.onload = function(){
                 ...
                 <div class='test' style="text-align:center"><?php echo rec(1);?></div> -->
          <?php if (isset($output['plate_top'])) {?>
          <div class="top-template"><?php echo $output['plate_top']['plate_content']?></div>
          <?php }?>
        <!-- 视频 -->
          <?php if(!empty($output['goods']['goods_url'])){?>
          <div style="text-align:center;">
            <input type="hidden" value="<?php echo $output['goods']['goods_url']; ?>" id="goods_url"/>
          <div class="video mt">
          ...
          <?php }?>
          <style>
          .slider-img a{z-index:-1;}
          </style>
          <div class="slider">
            <div class='slider-img'>
                  <?php echo rec(11);?>
                  <!-- <?php echo loadadv(1051);?> -->
            </div>
          </div>
          
          <script type="text/javascript" src="<?php echo RESOURCE_SITE_URL;?>/js/xSlider.js"></script>
          ...
          <ol>
            ...
            <li>
              <dl>
                ...
                <dd class="price pngFix"><?php echo ncPriceFormat($g['goods_price']); ?></dd>
              </dl>
            </li>
            <?php } ?>
          </ol>
...
$(function(){
    ...
    var gc_id=<?php echo $output['goods']['gc_id'];?>;
