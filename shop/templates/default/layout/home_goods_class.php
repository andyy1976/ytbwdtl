...
<div class="category">
  <ul class="menu">
    ...
    <li style="height:40px;" cat_id="<?php echo $val['gc_id'];?>" class="<?php echo $i%2==1 ? 'odd':'even';?>">
      ...
      <span class="arrow"></span>
        ...
        <?php } ?>
              ...
              <h4><a target="_blank" href="<?php echo urlShop('channel','index',array('id'=> $val['channel_id']));?>"><?php echo $val['gc_name'];?></a></h4>
              <?php }else{?>
              <h4><a target="_blank" href="<?php echo urlShop('search','index',array('cate_id'=> $val['gc_id']));?>"><?php echo $val['gc_name'];?></a></h4>
              <span class="recommend-cn-class" style="display:none;">
	      <?php if (!empty($val['class2']) && is_array($val['class2'])) { ?>
            <?php foreach ($val['class2'] as $k => $v) { ?>
          <a target="_blank" href="<?php echo urlShop('search','index',array('cate_id'=> $v['gc_id']));?>" title="<?php echo $v['gc_name']; ?>"><?php echo $v['gc_name'];?></a>
          <?php } ?>
          <?php } ?>
	  </span>
	      <?php } ?>
         </div>
      <div class="sub-class" style="min-height:546px" cat_menu_id="<?php echo $val['gc_id'];?>">
        <div class="sub-class-content">
          <div class="recommend-class">
            <?php if (!empty($val['cn_classs']) && is_array($val['cn_classs'])) { ?>
            <?php foreach ($val['cn_classs'] as $k => $v) { ?>
            <span><a target="_blank" href="<?php echo urlShop('search','index',array('cate_id'=> $v['gc_id']));?>" title="<?php echo $v['gc_name']; ?>"><?php echo $v['gc_name'];?></a></span>
            <?php } ?>
            <?php } ?>
          </div>
          <?php if (!empty($val['class2']) && is_array($val['class2'])) { ?>
          ...
          <dl>
            <dt>
              ...
              <h3><a target="_blank" href="<?php echo urlShop('channel','index',array('id'=> $v['channel_id']));?>"><?php echo $v['gc_name'];?></a></h3>
              <?php }else{?>
             <h3><a target="_blank" href="<?php echo urlShop('search','index',array('cate_id'=> $v['gc_id']));?>"><?php echo $v['gc_name'];?></a></h3>
              <?php } ?>
            </dt>
            <dd class="goods-class">
              <?php if (!empty($v['class3']) && is_array($v['class3'])) { ?>
              <?php foreach ($v['class3'] as $k3 => $v3) { ?>
              <a target="_blank" href="<?php echo urlShop('search','index',array('cate_id'=> $v3['gc_id']));?>"><?php echo $v3['gc_name'];?></a>
              <?php } ?>
              <?php } ?>
            </dd>
          </dl>
