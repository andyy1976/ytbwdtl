  ...
  <table class="ncm-default-table order">
    <thead>
      <tr>
        <th class="w10"></th>
        <th colspan="2">商品</th>
        <th class="w110">单价+云豆/件</th>
        <th class="w40">数量</th>
        <th class="w90">优惠活动</th>
        <th class="w90">售后维权</th>
        <th class="w110">订单金额+云豆</th>
    ...
    <tbody<?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <tr>
        ...
        <td><?php  if(empty($order_info['is_dm'])) {  echo '￥'.ncPriceFormat($goods_info['goods_price']).'+'.$goods_info['goods_points'].'云豆';
		 }else{
		  echo '￥'.ncPriceFormat($goods_info['goods_price']);
		 }

		?>
          <p class="green">
            <?php if (is_array($order_info['refund_all']) && !empty($order_info['refund_all']) && $order_info['refund_all']['admin_time'] > 0 and false) {?>
            <?php echo $goods_info['goods_pay_price'];?>(退)
            <?php } elseif ($goods_info['extend_refund']['admin_time'] > 0 and false) { ?>
        ...
        <td><!-- 退款 -->
          ...
          <p><a href="index.php?act=member_refund&op=add_refund&order_id=<?php echo $order_info['order_id']; ?>&goods_id=<?php echo $goods_info['rec_id']; ?>">退款<?php if(empty($order_info['is_dm'])) { ?>/退货 <?php } ?></a></p>
          <?php }?>
        ...
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p class=""><strong><?php
		 if(empty($order_info['is_dm'])) {  
		echo $order_info['order_amount'].'+'.$order_points.'云豆'; 
		}else{
		  echo '￥'.ncPriceFormat($goods_info['goods_price']);
		 }
		...
		?></strong></p>
          ...
          <p title="<?php echo $lang['member_order_pay_method'].$lang['nc_colon'];?><?php echo $order_info['payment_name']; ?>"><?php echo $order_info['payment_name']; ?></p>
        </td>
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p><?php echo $order_info['state_desc'];  echo $order_info['evaluation_status'] ? $lang['member_order_evaluated'] : '';?></p>
          
          <!-- 订单查看 -->
          <?php if(empty($order_info['is_dm'])) { ?>
        ...
        <td class="bdl bdr" rowspan="<?php echo $order_info['goods_count'];?>"><!-- 永久删除 --> 
          ...
          <p>退款退货中</p>
          <?php } ?>
           <?php if($order_info['is_dm']==2&&$order_info['order_state']==40){?>
            <p><a class="ncbtn ncbtn-aqua" href="index.php?act=member_order&op=dm_order&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i>确认订单</a></p>
          <?php }else{ ?>
          <!-- 取消订单 -->
...
$(function(){
    ...
    var member_id=<?php echo $output['member_info']['member_id']; ?>;
    
    if(member_level>0 && agreement_id==0){
      if(confirm('成为业务员可以获得奖励，请确认是否成为业务员')){
          $.ajax({
            type:'post',
            url:"/shop/index.php?act=member_order&op=agreement_id",
