        ...
        <table class="ncc-table-style">
          ...
          <tbody>
            <?php
				  ...
				  $panShowChargePhone = false;
        foreach($output['pointprod_arr']['pointprod_list'] as $val) {
          if ($val['pgoods_id'] > 0 && $val['pgoods_id'] < 7) {
            $panShowChargePhone = true;
          }
			?>
            <tr class="shop-list ">
              <td></td>
            ...
            <?php } }?>
            <?php if ($panShowChargePhone) {?>
            <tr>      
                <td colspan="20" class="tl"><i class="required">*</i><label><?php echo '充值手机号'.$lang['nc_colon'];?>
                <input type="text" class="text w300" name="point_chargephone" value="" placeholder="必填：填写正确的充值手机号码(11位)"/>
                </label></td>
            </tr>
            <?php } ?>
            <tr>
...
<script type="text/javascript">
    ...
    if ($("input[name='point_chargephone']").length > 0) {
      var chargephone = $("input[name='point_chargephone']").val();
      if(!chooseaddress || chooseaddress <= 0){
        showDialog('请选择收货人地址');
      } else if (!(/^1(3|4|5|7|8)\d{9}$/.test(chargephone))) {
        showDialog('请填写充值的手机号码且必须为11位');
      } else {
        $('#porder_form').submit();
      }
    } else {
