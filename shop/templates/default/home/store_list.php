...
<?php foreach($output['store_list'] as $skey => $store){?>
    <li class="item">
      <dl class="shop-info">
        <dt class="shop-name"><a href="<?php echo urlShop('show_store','', array('store_id'=>$store['store_id']),$store['store_domain']);?>" target="_blank"><?php echo $store['store_name'];?></a><?php if ($store['store_name'] == '中华粮仓' || $store['store_name'] == '创意优品专营店') {$store['grade_id'] = 1;}?><img src="<?php echo SHOP_TEMPLATES_URL;?><?php echo '/images/store_grade/'.$store['grade_id'].'.png'; ?>" class="pngFix"></dt>
