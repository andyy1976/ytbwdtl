...
<table class="ncsc-default-table">
  <thead>
    ...
    <div style="padding: 5px 10px; border-bottom: 1px solid #E6E6E6;">
        <a href="index.php?act=store_goods_online&amp;op=goodsall_export" id="panTest" class="ncbtn-mini" target="_blank" title="导出数据"><i class="icon-print"></i>导出数据</a>
      </div>
      <th class="w30">&nbsp;</th>
      <th class="w50">&nbsp;</th>
     ...
     <?php if($output['store_flag']==1){ ?> 
      <th class="w100">所属套餐</th>
      <?php } ?>
    ...
    <tr>
      ...
      <td colspan="21"><label for="all" ><?php echo $lang['nc_select_all'];?></label>
  ...
  <tbody>
    ...
    <tr>
      ...
      <th colspan="21">SPU：<?php echo $val['goods_commonid'];?></th>
    </tr>
    <tr>
      ...
      <?php if($output['store_flag']==1){ ?>
      <td align="left" valign="middle" class="goods-time"><?php  if($val['usertc']==1){      
		   echo 'B套餐';
		  }else if($val['usertc']==0){ echo 'A套餐';}else if($val['usertc']==2){ echo 'C套餐';}else{ echo '暂未设定'; } 
		  ?></td>
          <?php } ?>
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
  <?php if ($val['edit_counts'] < 3) {?>
          <span>
          <a href="<?php echo urlShop('store_goods_online', 'edit_goods', array(
          'commonid' => $val['goods_commonid'],
          'goods_id' => $output['storage_array'][$val['goods_commonid']]['goods_id']));?>" class="btn-bluejeans"><i class="icon-edit"></i>
          <p><?php echo $lang['nc_edit'];?></p>
          </a>
          </span> 
        <?php }?>
        <span><a href="javascript:void(0);" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', '<?php echo urlShop('store_goods_online', 'drop_goods', array('commonid' => $val['goods_commonid']));?>');" class="btn-grapefruit"><i class="icon-trash"></i>
    ...
    <tr style="display:none;">
      <td colspan="21"><div class="ncsc-goods-sku ps-container"></div></td>
    </tr>
    <?php } ?>
    <?php } else { ?>
    <tr>
  ...
  <tfoot>
    ...
    <tr>
      ...
      <th colspan="11"><label for="all2"><?php echo $lang['nc_select_all'];?></label>
...
<script type="text/javascript">
  ...
  if (trueUrl.indexOf("&op") == '-1') {
    trueUrl += '&op=goodsall_export';
  }
  trueUrl = trueUrl.replace(/op=index/, 'op=goodsall_export');
