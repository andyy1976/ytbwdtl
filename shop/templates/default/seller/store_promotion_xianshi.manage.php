...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#div_goods_search_result').on('click', '[nctype="btn_add_xianshi_goods"]', function() {
            ...
            $('#dialog_goods_points').text($(this).attr('data-goods-points'));
            ...
            $('#dialog_xianshi_points').val($(this).attr('data-goods-points'));
            $('#dialog_add_xianshi_goods_error').hide();
        });

        //添加限时折扣商品
        //20170727潘丙福添加--云豆字段（goods_points，xianshi_poins）
        $('#div_goods_search_result').on('click', '#btn_submit', function() {
            ...
            var goods_points = Number($('#dialog_input_goods_points').val());
            var xianshi_price = Number($('#dialog_xianshi_price').val());
            var xianshi_points = Number($('#dialog_xianshi_points').val());
            if(!isNaN(xianshi_price) && xianshi_price > 0 && xianshi_price < goods_price) {
                $.post('<?php echo urlShop('store_promotion_xianshi', 'xianshi_goods_add');?>', 
                    {goods_id: goods_id, xianshi_id: xianshi_id, xianshi_price: xianshi_price,xianshi_points: xianshi_points},
                    function(data) {
                        if(data.result) {
        ...
        $('#xianshi_goods_list').on('click', '[nctype="btn_edit_xianshi_goods"]', function() {
            ...
            var xianshi_points = $edit_item.find('[nctype="xianshi_points"]').text();
            ...
            $('#dialog_edit_xianshi_points').val(xianshi_points);
            $('#dialog_edit_xianshi_goods').nc_show_dialog({width: 450, title: '修改价格与云豆'});
        });
        ...
        $('#btn_edit_xianshi_goods_submit').on('click', function() {
            ...
            var xianshi_points = Number($('#dialog_edit_xianshi_points').val());
            ...
            if(!isNaN(xianshi_price) && xianshi_price > 0 && xianshi_price < goods_price) {
                $.post('<?php echo urlShop('store_promotion_xianshi', 'xianshi_goods_price_edit');?>',
                    {xianshi_goods_id: xianshi_goods_id, xianshi_price: xianshi_price, xianshi_points: xianshi_points},
                    function(data) {
                        if(data.result) {
                            $edit_item.find('[nctype="xianshi_price"]').text(data.xianshi_price);
                            $edit_item.find('[nctype="xianshi_points"]').text(data.xianshi_points);
...
<table class="ncsc-default-table">
  ...
  <tbody id="xianshi_goods_list">
    ...
    <tr class="bd-line">
        ...
        <td><span nctype="xianshi_points"><?php echo $val['xianshi_points'];?></span></td>
...
<div id="dialog_edit_xianshi_goods" class="eject_con" style="display:none;">
    ...
    <dl><dt>商品价格：</dt><dd><span id="dialog_edit_goods_price"></dd></dl>
    ...
    <dl><dt>折扣云豆：</dt><dd><input id="dialog_edit_xianshi_points" type="text" class="text w70"><em class="add-on">个</em>
    <p id="dialog_edit_xianshi_goods_error" style="display:none;"><label for="dialog_edit_xianshi_goods_error" class="error"><i class='icon-exclamation-sign'></i>折扣云豆不能为空，且必须为正整数</label></p>
    </dl>     
    <div class="eject_con">
...
<tr class="bd-line">
    ...
    <td><span nctype="xianshi_points"><%=xianshi_points%></span></td>
