  ...
  <table class="search-form">
    ...
    <tr>
      ...
      <td><input type="checkbox" id="skip_off" value="1" <?php echo $_GET['skip_off'] == 1 ? 'checked="checked"' : null;?>  name="skip_off">
        <label for="skip_off">不显示已关闭订单</label></td>
      <?php } ?>
<?php if ($_GET['state_type'] == 'state_pay') { ?>
      ...
      <td class="w110"><input type="text" class="text w100" name="order_sn" value="<?php echo $_GET['order_sn']; ?>" /></td>
      <th><?php echo '商品名';?></th>
      <td class="w90"><input type="text" class="text w80" name="goods_name" value="<?php echo $_GET['goods_name']; ?>" /></td>
...
<div style="padding: 5px 10px; border-bottom: 1px solid #E6E6E6;">
  <a href="index.php?act=store_order&amp;op=orderall_export" id="panTest" class="ncbtn-mini" target="_blank" title="导出数据"><i class="icon-print"></i>导出数据</a>
    <?php if (trim($_GET['state_type']) == 'state_pay' && ($_SESSION['is_own_shop'] || $_SESSION['store_id'] == 2)) {?>
    <a href="index.php?act=store_order&op=importshow" id="panImport" class="ncbtn-mini" target="_blank" title="批量导入物流单号"><i class="icon-print"></i>批量导入物流单号</a>
  <?php }?>
</div>
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20"><span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?><em><?php echo $order['order_sn']; ?></em>
        ...
        <?php if ($output['stateType'] == 'state_pay'){ ?>
            <span><?php echo '付款时间'.$lang['nc_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$order['payment_time']); ?></em></span>
        <?php } else { ?>
        ...
        <?php if ($order['buyer_cardid']) {?>
        <span>订单用户身份证号：<em class="goods-time"><?php echo $order['buyer_cardid']; ?></em></span>
        <?php }?>
    ...
    <tr>
      ...
      <td class="bdl" rowspan="<?php echo $order['goods_count'];?>"><p class="ncsc-order-amount"><?php echo $order['order_amount']; ?></p>
        ...
        <p class="goods-freight">
          <?php if ($order['is_refund_shipping_fee'] == 1){?>
          (已退运费<?php echo $order['refund_shipping_fee'];?>)
          <?php }?>
        </p>        <p class="goods-pay" title="<?php echo $lang['store_order_pay_method'].$lang['nc_colon'];?><?php echo $order['payment_name']; ?>"><?php echo $order['payment_name']; ?></p></td>
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>">
       <?php if($order['is_dm']!=0){
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><?php if($order['if_store_cancel']) { ?>
        ...
        <!-- 发货 -->
        <?php 
		if($order['is_dm']==1 && $order['order_state']==20 && $order['lock_state']==0){ ?>
		<p><a class="ncbtn ncbtn-mint mt10" href="index.php?act=store_deliver&op=sendzjdm&order_id=<?php echo $order['order_id']; ?>"/><i class="icon-truck" ></i><?php echo '确认金额'; ?></a></p>
	 <?php  }elseif($order['is_dm']!=0 &&$order['order_state']==20 && $order['lock_state']==0){ ?>
			<p><a class="ncbtn ncbtn-mint mt10" href="index.php?act=store_deliver&op=senddm&order_id=<?php echo $order['order_id']; ?>"/><i class="icon-truck" ></i><?php echo '确认订单'; ?></a></p>
			<?php }
			else if($order['is_dm']!=0 && $order['order_state']==30){?> 
            <?php echo '等待消费者确认订单'; ?>
			<?php }
			else if($order['is_dm']!=0 && $order['order_state']==40){?> 
            <?php echo '等待消费者确认订单'; ?>
            <?php }
			else if($order['is_dm']!=0 && $order['order_state']==50){?> 
            <?php echo '订单已完成'; ?>
             <?php }
			else if($order['is_dm']!=0 && $order['order_state']==0){?> 
            <?php echo '订单已取消'; ?>
			<?php }else{?>
        ...
        <!-- 修改物流单号潘丙福20170613 -->
        <?php if($order['if_deliver']) { ?>
        <p><a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit mt5" nc_type="dialog" uri="index.php?act=store_order&op=change_state&state_type=shipping_code&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="修改物流单号" dialog_id="seller_order_adjust_fee" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-remove-circle"></i><?php echo '改运单号';?></a></p>
        <?php } ?>
 <!-- 退运费潘丙福20170830 -->
        <?php if ($order['shipping_fee'] > 0 && $order['if_deliver'] && $order['is_refund_shipping_fee'] == 0) { ?>
        <p><a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit mt5" uri="index.php?act=store_order&op=change_state&state_type=refund_shipping_fee&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="退还运费" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>退还运费</a></p>
        <?php }?>
        <!-- 锁定 -->
...
<script type="text/javascript">
  ...
  if (trueUrl.indexOf("&op") == '-1') {
    trueUrl += '&op=orderall_export';
  }
  trueUrl = trueUrl.replace(/op=index/, 'op=orderall_export');
  $('#panTest').attr('href', trueUrl);
</script> 
