...
<?php } ?>
    ...
    <dl>
      <dt><i class="required">*</i>抢购时间点</dt>
      <dd>
        <select id="time_h" name="time_h" class="w80">
    ...
    <!-- 20170726潘丙福添加显示商品原云豆 -->
    <dl nctype="groupbuy_goods_info" style="display:none;">
      <dt><?php echo '店铺云豆'.$lang['nc_colon'];?></dt>
      <dd><span nctype="groupbuy_goods_points"></span><input id="input_groupbuy_goods_points" type="hidden"></dd>
    </dl>
    ...
    <!-- 20170726潘丙福添加显示商品促销云豆 -->
    <dl>
      <dt><i class="required">*</i><?php echo '抢购云豆'.$lang['nc_colon'];?></dt>
      <dd>
        <input class="w70 text" id="groupbuy_points" name="groupbuy_points" type="text" value=""/><em class="add-on">个</em> <span></span>
        <p class="hint">抢购云豆为该商品参加活动时的促销云豆<br />必须是1~999之间的数字(单位：个)</p>
      </dd>
    </dl>
    ...
    $('#div_goods_search_result').on('click', '[nctype="btn_add_groupbuy_goods"]', function() {
        ...
        $.get('<?php echo urlShop('store_groupbuy', 'groupbuy_goods_info');?>', {goods_commonid: goods_commonid}, function(data) {
            if(data.result) {
                ...
                $('[nctype="groupbuy_goods_points"]').text(data.goods_points);
                $('#input_groupbuy_goods_points').val(data.goods_points);
                $('#groupbuy_points').val(data.goods_points);
...
<?php } ?>
            ...
            groupbuy_points: {
                required : '<i class="icon-exclamation-sign"></i>请填写抢购云豆',
                number : '<i class="icon-exclamation-sign"></i>抢购云豆必须为整数字',
                min : '<i class="icon-exclamation-sign"></i>抢购云豆最少为1个',
                max : '<i class="icon-exclamation-sign"></i>抢购云豆最大为10000个'
            },
            virtual_quantity: {
