  ...
  <table class="ncc-table-style">
    <thead>
      <tr>
        ...
        <th class="w150">预定单价(<?php echo $lang['currency_zh'];?>)</th>
    ...
    <tbody>
      ...
      <tr id="cart_item" class="shop-list <?php echo ($cart_info['state'] && $cart_info['storage_state']) ? '' : 'item_disabled';?>">
        ...
        <td><em class="goods-price">定金：<?php echo $cart_info['book_down_payment'].'+'.$cart_info['book_down_payment_points'].'云豆';?></em>
          <p>+</p>
          <em class="goods-price"> 尾款：<?php echo $cart_info['book_final_payment'].'+'.$cart_info['book_final_payment_points'].'云豆';?></em></td>
        ...
        <td class="td-border-right"><?php if ($cart_info['state'] && $cart_info['storage_state']) {?>
          <em class="goods-subtotal"><?php echo $cart_info['goods_total'].'元|'.($cart_info['book_down_payment_points']+$cart_info['book_final_payment_points'])*$cart_info['goods_num'].'云豆'; ?></em> <span id="no_send_tpl" style="color: #F00;display:none">无货</span>
          ...
          <div class="ncc-store-account">
            ...
            <dl nctype="book_pay_content_part" class="total">
              ...
              <dd class="rule"><i>*</i>
                ...
                同意支付定金 (不退)</span></dd>
              <!-- <dd class="sum"><em><?php echo ncPriceFormat($cart_info['book_down_payment']*$cart_info['goods_num']).'+'.$cart_info['book_down_payment_points']*$cart_info['goods_num'].'云豆';?></em></dd> -->
...
function calcOrder() {
    ...
    if ($('#book_pay_part').prop('checked')) {
    	$('#orderBookTotal').html('<?php echo ncPriceFormat($cart_info['book_down_payment'] * $cart_info['goods_num'])."元+".$cart_info['book_down_payment_points'] * $cart_info['goods_num'].'云豆';?>');
    } else {
    	...
    	$('#orderBookTotal').html(number_format(allTotal,2) + '元+' + '<?php echo ($cart_info['book_down_payment_points']+$cart_info['book_final_payment_points']) * $cart_info['goods_num'].'云豆';?>');
    }
}
