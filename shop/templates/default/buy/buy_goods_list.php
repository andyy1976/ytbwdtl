  ...
  <table class="ncc-table-style">
    <thead>
      <tr>
        ...
        <th class="w100">云豆/件</th>
        <th class="w100"><?php echo $lang['cart_index_amount'];?></th>
        <th class="w150"><?php echo $lang['cart_index_sum'].'('.$lang['currency_zh'].'|'.'云豆'.')';?></th>
      </tr>
    </thead>
...
>
        ...
        <!--商品云豆加入的。。。。。。。。-->
        <td><?php echo $cart_info['goods_points']; ?></td>
        ...
        <td class="td-border-right"><?php if ($cart_info['state'] && $cart_info['storage_state']) {?>
          <em cart_id="<?php echo $cart_info['cart_id']; ?>" goods_id="<?php echo $cart_info['goods_id'];?>" nc_type="eachGoodsTotal<?php echo $store_id?>" 
          	tpl_id="<?php echo $cart_info['transport_id']?>" class="goods-subtotal"><?php echo $cart_info['goods_total'].'元'.'|'.$cart_info['goods_total_points'].'云豆'; ?>          		
          </em>         
        ...
        <td colspan="20"><div class="ncc-msg">买家留言：
          ...
          <div class="ncc-store-account">
            ...
            <dl>
              <dt>商品云豆：</dt>
              <dd class="rule"></dd>
              <dd class="sum"><em id="eachStoreGoodsTotal_points_<?php echo $store_id;?>"><?php echo $output['store_goods_total_points'][$store_id];?></em></dd>
            </dl>
    ...
    <tfoot>
      ...
      <tr>
        <td colspan="20"><?php if(!empty($output['ifcart'])){?>
          ...
          <?php }?>
          <div class="ncc-all-account">订单总金额：<em id="orderTotal">....</font></em><?php echo $lang['currency_zh'];?> | 订单总云豆：<em id="orderTotalPoints">....</font></em></div>
...
function submitNext(){
	//判断云豆是否足够加入的。。。。。。。。。。。。。。。。。。	
  var points=<?php $info=get_member_info($_SESSION['member_id']) ; echo $info['member_points'] ;?>;    	 
	var totalPoints=$('#orderTotalPoints').text();
	//alert(points);exit;
	<?php if($output['isdmgoods']!=1){ ?>
	 if(parseInt(totalPoints) > parseInt(points)){					
		showDialog('您的云豆不足，充值可获得云豆', 'error','','','','','','','','',3);
		return;
	}
	<?php } ?>
	if (!SUBMIT_FORM) return;
	if ($('input[name="buyer_cardid"]').length > 0) {
		if ($.trim($('input[name="buyer_cardid"]').val()).length == 0) {
			alert('请填写身份证号');
			window.getSelection().removeAllRanges();
			$('input[name="buyer_cardid"]').focus();
			return;
		}
	}
...
function calcOrder() {
    allTotal = 0;
    allTotalPoints = 0;//加入的。。。。。。。。。。
    $('em[nc_type="eachStoreTotal"]').each(function(){
        store_id = $(this).attr('store_id');
        var eachTotal = 0;
        var eachTotalPoints = 0;//加入的。。。。。。。。。。
        ...
        if ($('#eachStoreGoodsTotal_'+store_id).length > 0) {
        	...
        	eachTotalPoints += parseFloat($('#eachStoreGoodsTotal_points_'+store_id).html());//加入的
	    }
        ...
        allTotal += eachTotal;
        allTotalPoints += eachTotalPoints;//加入的。。。。。。。
    ...
    $('#orderTotalPoints').html(allTotalPoints.toFixed(2));//加入的。。。。。。。
