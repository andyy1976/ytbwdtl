...
class store_joininControl extends BaseHomeControl {
        ...
        if(!empty($joinin_detail)) {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['op'], array('step0','step1', 'step2', 'step3', 'step4','ajax_upload_image'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?act=store_joinin&op=step0');
                    }
                    break;
   ...
   public function stepdmOp(){
          $member=Model('member');
          $member_info=$member->where(array('member_id'=>$_SESSION['member_id']))->find();
          $count=$member->where(array('member_id'=>$_SESSION['member_id']))->count(); 
         if($count<1){
            showMessage('请会员再申请商家入驻', '', 'html', 'error');
            exit;
        }
    ...
    public function step0Op() {
        ...
        if($member_info['member_level']<'1'){
            showMessage('请先激活会员再申请商家入驻', '', 'html', 'error');
            exit;
        }
    ...
    public function step3Op() {
          ...
          Tpl::output('store_flag',$store_flag['store_flag']);//商品分类-地面商家
   ...
   if($store_flag['store_flag']==2){   //线下产品店铺分类
        $store_class = $model_store->where(array('sc_flag'=>'1','is_level'=>'0'))->select();
       }else{
        //$store_class = $model_store->getStoreClassList(array(),'',false);
        $store_class = $model_store->where(array('sc_flag'=>'0','is_level'=>'0'))->select();
    }
    ...
    private function step3_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['bank_account_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"22","message"=>"银行账号不能为空且必须小于23个字"),
            ...
            array("input"=>$param['settlement_bank_account_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"22","message"=>"银行账号不能为空且必须小于23个字"),
    ...
    public function step4Op() {
        ...
        if($joinin_detail['store_flag']==2){//地面商家特有 李志军
         $param['sg_id']=1000;
         $param['sg_name'] ='线下商家';
         $param['sg_info'] = serialize(array('sg_price' => 0));
