    ...
    public function indexOp() {
        ...
        foreach ($order_list as $key => $value) {
            if($value['order_state'] == '20'){
                ...
                $store = model('store');
                //获取商品信息
                $store_info = $store->getStoreInfo(array('store_id'=>$value['store_id']));
                //获取手机号码
                $member_info = $model->where(array('member_id'=>$store_info['member_id']))->find();
                $member_ss = $model->getMemberCommonInfo(array('member_id' => $member_info['member_id']));
                //商品ID 是通联宝的不给发短信
                if($value['goods_list'][0]['goods_id']=='7172'){
                    continue;
                }
                if(($time-$value['payment_time'])/(24*3600) >= 1){
                    $s = $value['order_id'].'/'.$value['order_sn'];
                    $arr[]=$s;
                }
               
                if(($time-$member_ss['send_acode_time'])/(3600) >=24 && $value['rcb_amount'] !=1){

                foreach ($arr as $key => $sking) {
                              $arr_str[]= explode('/',$sking);  
                        }
                        foreach ($arr_str as $key => $v) {
                            foreach ($v as $ky => $c) {
                                if (strlen($c) >= 4 && strlen($c) <= 8) {
                                    $t = "尊敬的店主，您有新的发货订单已经超过24小时未发货，订单号为：";
                                   
                                }
                                if (strlen($c) >= 12 && strlen($c) <= 19 ) {
                                    $c = $t. $c . "为了不影响您的店铺正常运行，请您务必在一个小时内进行处理。";
                                    
                                    $sms = new Sms();
                                    $result = $sms->send($member_info["member_mobile"], $c);
                                    $data = array('rpt_amount'=>1);
                                    $wheres['order_id'] = $value['order_id'];
                                    $model_order->editOrder($data,$wheres);
                                     if ($result) {
                                            
                                         if($member_ss) {
                                            $where = array('member_id' => $member_info['member_id']);
                                             $model->editMemberCommon(array(
                                                 'auth_code' => $c,
                                                 'send_acode_time' => time()
                                             ),$where);
                                         }else{
                                             $model->addMemberCommon(array(
        ...
        $condition['delay_time'] = array('lt', time()-60*60*24*7);
        $condition['store_id'] = $_SESSION['store_id'];
        $panUnReceive = $model_order->table('orders')->where($condition)->select();

        foreach ($panUnReceive as $panUnReceiveKey => $panUnReceiveValue) {

            $model_order->table('orders')->where("order_id={$panUnReceiveValue['order_id']}")->update(array('order_state'=>'40'));
            //商家获得50%佣金           
            $dat['wallet_frozen']=array('exp','wallet_frozen-'.$panUnReceiveValue['order_amount']*0.5);
            $dat['wallet_release']=array('exp','wallet_release+'.$panUnReceiveValue['order_amount']*0.5);
            $log['member_id']=$panUnReceiveValue['store_id'];
            $log['member_name']=$panUnReceiveValue['store_name'];
            $log['type']='seller';
            $log['add_time']=time();
            $log['available_amount']='-'.$panUnReceiveValue['order_amount']*0.5;
            $log['freeze_amount']=$panUnReceiveValue['order_amount']*0.5;
            $log['description']='会员确定收货，商家获得剩下50%货款';        
            $rcb->insert($log);    
            $upstore=$store->where(array('store_id'=>$panUnReceiveValue['store_id']))->update($dat);

            //每个人买东西都给上级提成代码开始   
                    
            $order_money=$panUnReceiveValue['order_amount'];
            $chiefs=$percent->getfby_id(1,'chief');
            $moneys="$order_money" * "$chiefs" ;                    
            $arr=get_parent_info($panUnReceiveValue['buyer_id']);

            $goods = $model_order->getOrderGoodsInfo(array('order_id'=>$panUnReceiveValue['order_id']));

            
            $goods_id = $member_goods->getGoodsInfo(array('goods_id'=>$goods['goods_id']));
            
            if($goods_id['gc_id'] !=10351){
                //如果是会员资格就不给予购物分成                             
                if(is_array($arr)){
                ...
                give_chief($_SESSION['member_id'],$order_money);//给代理分成
                points_delete($order_id);//扣除购买该订单产品所需要的云豆
            }
        }
        unset($store);
        unset($rcb);
