    ...
    public function indexOp() {
		...
		*/
		...
		if(!empty($cart_list)){
			$times=0;   //判断出现次数
			$b=array();
       foreach($cart_list as $key=>$vale){
		     $goodsid=$cart_list[$key]['goods_id'];
			if(!empty($goodsid)){
				$findgc=$model_goods->getGoodsInfoByID($goodsid);
			    $cart_list[$key]['gc_id']=$findgc['gc_id'];
				if($findgc['gc_id']==10351){
					$times=$times+1;
					$b[]=$findgc['goods_id'];
					}
				}
		...
		if($meber_info['member_level']>0 && $times>0){  //如果是激活会员并且最一次购买银联宝，即提示用户不能最次购买，并删掉网站的购物车信息
		   if(count($b)>0){
		     showMessage("亲，会员资格只能购买一次不能重复购买,请在购物车中删除会员资格商品！");
			 }
		}
		if($meber_info['member_level']==0 && $times>1){
				 
			 showMessage("亲，会员资格只能购买一次不能重复购买,请保留一件会员资格的商品！");
		}
