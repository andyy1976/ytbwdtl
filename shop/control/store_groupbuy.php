...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function groupbuy_listOp() {
        ...
        if(!empty($_GET['groupbuy_state'])) {
            ...
            //20170819潘丙福添加--时间限制(因为定时任务没有开启需要做一下时间筛选)
            //正常状态
            if ($condition['state'] == 20) {
                $condition['end_time'] = array('gt', time());
            }
            //已结束状态
            if ($condition['state'] == 32) {
    ...
    public function groupbuy_saveOp() {
        ...
        //20170726潘丙福添加-goods_points字段
        $goods_info = $model_goods->getGoodsInfoByID($goods_id, 'goods_id,goods_commonid,goods_name,goods_price,goods_points,store_id,virtual_limit');
        ...
        //20170626潘丙福添加开始-添加抢购云豆字段groupbuy_points
        $param['groupbuy_points'] = intval($_POST['groupbuy_points']);
        //20170626潘丙福添加结束
        ...
        //20170626潘丙福添加开始-添加原商品云豆字段goods_points
        $param['goods_points'] = $goods_info['goods_points'];
        //20170626潘丙福添加结束
        $param['store_id'] = $_SESSION['store_id'];
        $param['store_name'] = $_SESSION['store_name'];
        //20171024xin
        $param['time_h'] = $_POST['time_h'];
    ...
    public function groupbuy_goods_infoOp() {
        ...
        //20170726潘丙福添加开始--增加云豆字段（goods_points）
        $data['goods_points'] = $goods_info['goods_points'];
        //20170726潘丙福添加结束
