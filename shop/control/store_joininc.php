...
class store_joinincControl extends BaseHomeControl {
	...
	public function __construct() {
        ...
        if($_GET['op'] != 'check_seller_name_exist' && $_GET['op'] != 'checkname' && $_GET['op'] != 'panbingfu') {
       ...
       if(!empty($joinin_detail)) {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['op'], array('step0','step1', 'step2', 'step3', 'step4'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?act=store_joininc&op=step0');
                    }
                    break;
    ...
    public function stepdmstep1Op(){
       $dmcontent = $this->find_joinin($_SESSION['member_id']);
 //省旗舰店
      /* $member_provinceid = Model()->table('member')->where(array('member_id'=>$_SESSION['member_id']))->field('member_provinceid')->find();
        $provinceid = Model()->table('area')->where(array('area_id'=>$member_provinceid['member_provinceid']))->field('area_name')->find();
        $province = $provinceid['area_name'];*/
        Tpl::output('province', $province);       Tpl::output('dm',$dmcontent);
       Tpl::output('step', 'step2');
       Tpl::output('sub_step', 'step1');
       Tpl::output('flag',1);
       Tpl::showpage('store_joininc_apply');
    }
    public function stepdmstep2Op(){
        $dmcontent = $this->find_joinin($_SESSION['member_id']);
       Tpl::output('dm',$dmcontent);
        if(!empty($_POST)) {
            $param = array();
            ...
            if(!empty($_POST['idcard'])&&$_FILES['business_licence_number_elc']['name']==''){
            $param['business_licence_number_elc']=$_POST['idcard'];
            }else{
            $param['business_licence_number_elc'] = $this->upload_image('business_licence_number_elc');
            }
            if(!empty($_POST['yycard'])&&$_FILES['business_licence_number_elcc']['name']==''){
            $param['organization_code_electronic'] =$_POST['yycard'];
            }else{
             $param['organization_code_electronic'] = $this->upload_image('business_licence_number_elcc');  //地面商家的使用组织机构代码证
            }
        
            $param['general_taxpayer'] = $this->upload_image('general_taxpayer');
            $param['store_flag'] = $_POST['store_flag'];
    ...
    exit;
     ...
     public function stepdmstep3Op(){
         if(!empty($_POST)) {
            ...
            if(!empty($_POST['zidcard'])&&$_FILES['tax_registration_certif_elc']['name']==''){
            $param['tax_registration_certif_elc']=$_POST['zidcard'];
            }else{
            $param['tax_registration_certif_elc']=$this->upload_image('tax_registration_certif_elc');
            }
             if(!empty($_POST['fidcard'])&&$_FILES['bank_licence_electronic']['name']==''){
            $param['bank_licence_electronic']=$_POST['fidcard'];
            }else{
        ...
        $model_store_joinin = Model('store_joinin');
        $store_flag = $model_store_joinin->where(array('member_id'=>$_SESSION['member_id']))->find();   //判断是否是线下商家
        Tpl::output('store_flag',$store_flag['store_flag']); 
         $gc = Model('goods_class');//商品分类
        
         $gc_list   = $gc->getGoodsClassListByParentIdd(0);
        Tpl::output('gc_list',$gc_list);

        //店铺等级
       $store_data = Model('store_grade');
        $grade_list = $store_data->where(array('sg_flag'=>'1'))->select();
        //附加功能
        ...
        $store_class = $model_store->where(array('sc_flag'=>'1'))->select();
    ...
    public function step2Op() {
        if(!empty($_POST)) {
            ...
            $param['general_taxpayer'] = $this->upload_image('general_taxpayer');
            $param['store_flag'] = 0;
            /*if($_POST['store_flag']==2){    //地面商家不需要审核，不要缴费用
    ...
    public function step3Op() {
        ...
        Tpl::output('store_flag',$store_flag['store_flag']); 
         $gc = Model('goods_class');//商品分类
         if($store_flag['store_flag']==2){   //线下商家的商品分类
         $gc_list   = $gc->getGoodsClassListByParentIdd(0);
         }else{
            $gc_list	= $gc->getGoodsClassListByParentId(0);
            //20170816潘丙福添加-去除会员资格、电影、云豆购买5%，云豆购买8%分类
            foreach ($gc_list as $panKey => $panValue) {
                if ($panValue['gc_id'] == 10351 || $panValue['gc_id'] == 10424 || $panValue['gc_id'] == 10425 || $panValue['gc_id'] == 10417) {
                    unset($gc_list[$panKey]);
                }
            }
        }
        Tpl::output('gc_list',$gc_list);
		...
		//店铺等级
       $store_data = Model('store_grade');
        if($store_flag['store_flag']==2){   //线下
            $grade_list = $store_data->where(array('sg_flag'=>'1'))->select();
        }else{                              //线上
            //$grade_list = rkcache('store_grade',true);
            $grade_list = $store_data->where(array('sg_flag'=>'0'))->select();
            //20171024潘丙福添加开始--隐藏白金店铺与钻石店铺
            foreach ($grade_list as $grade_list_key => $grade_list_value) {
                if ($grade_list_value['sg_name'] == '白金店铺' || $grade_list_value['sg_name'] == '钻石店铺') {
                    unset($grade_list[$grade_list_key]);
                }
            }
            //20171024潘丙福添加结束
        }
    ...
    //省旗舰店店铺条件
        if($_SESSION['member_level'] == 5){
           foreach ($grade_list as $key => $value) {
                if($value['is_province'] !=1){
                    unset($grade_list[$key]);
                }
            } 
        }
        if($_SESSION['member_level'] != 5){
		...
		Tpl::output('grade_list', $grade_list);
		 ...
		 //店铺分类
        $model_store = Model('store_class');
         if($store_flag['store_flag']==2){   //线下产品店铺分
            $condition['sc_flag'] = 1;
       }else{
        ...
        //增加省旗舰店条件
        if($_SESSION['member_level'] == 5){
            $condition['is_level'] = 1;
        }else{
            $condition['is_level'] = 0;
        }
        $store_class = $model_store->where($condition)->select();
        Tpl::output('store_class', $store_class);
    ...
    public function step4Op() {
        ...
        $param['joinin_state'] = STORE_JOIN_STATE_VERIFY_SUCCESS;    //修改过
        }
    ...
    //20170623潘丙福添加开始-支付开通店铺费用
    public function panbingfuOp()
    {
        $logic_buy = logic('buy');
        $result = $logic_buy->panBuyStep2($_POST, $_SESSION['member_id'], $_SESSION['member_name'], $_SESSION['member_email']);
        if(!$result['state']) {
            showMessage($result['msg'], 'index.php?act=cart', 'html', 'error');
        }
        //转向到商城支付页面
        redirect('index.php?act=buy&op=pan_pay&pay_sn='.$result['data']);
    }
    ...
    * 根据ID查找内容
    */
    private function find_joinin($id){
        $model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(array('member_id'=>$id,'store_flag'=>'2'));
        if(!empty($joinin_detail)){
