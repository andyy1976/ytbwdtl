...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        $store_online_info = Model('store')->getStoreOnlineInfoByID($_SESSION['store_id']);
        $store_flag = $store_online_info['store_flag'];   //店铺开起开关表示
        $where = array();
        ...
        $conditon=array();
         ...
         $conditon['goods_commonid']=$value['goods_commonid'];
          $zxcg = $model_goods->getGoodsInfo($conditon);
          $goods_list[$key]['map_actives'] = $zxcg['map_actives'];
      }
        ...
        Tpl::output('store_flag',$store_flag);
        $this->profile_menu('goods_list', 'goods_list');
        Tpl::showpage('store_goods_list.online');
    }

     //20170811xin添加  导出excel表格
    public function goodsall_exportOp()
    {
        $model_goods = Model('goods');
        $store_online_info = Model('store')->getStoreOnlineInfoByID($_SESSION['store_id']);
        $store_flag = $store_online_info['store_flag'];   //店铺开起开关表示
        ...
        $where['goods_state'] = 1;
        $goods_list= Model()->table('goods')->where($where)->select();
        if (!is_numeric($_GET['curpage'])){
           $count=count($goods_list);
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::showpage('good_export.excel');
            }else{  //如果数量小，直接下载
                $data= Model()->table('goods')->where($where)->order('goods_commonid desc')->limit($limit2)->select();
                foreach ($data as $key => $value) {
                    $goods_commonid = $value['goods_commonid'];
                    $goods_serials  = Model()->table('goods_common')->where(array('goods_commonid'=>$goods_commonid))->field('goods_serial')->find();
                    $data[$key]['goods_serials'] = $goods_serials['goods_serial'];
                }
            ...
            if (! $limit1) {
                ...
                $data= Model()->table('goods')->where($where)->order('goods_commonid desc')->limit($limit1,$limit2)->select();               
            }
            foreach ($data as $key => $value) {
                    $goods_commonid = $value['goods_commonid'];
                    $goods_serials  = Model()->table('goods_common')->where(array('goods_commonid'=>$goods_commonid))->field('goods_serial')->find();
    ...
    private function createExcel($data = array(), $supList = array()){
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'商品公共表id');
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'商品添加时间');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'商家货号');

        foreach ((array)$data as $k=>$goods_info){
            $tmp = array();
                $tmp[] = array('data'=>$goods_info['goods_id']);
                $tmp[] = array('data'=>$goods_info['goods_commonid']);
                $tmp[] = array('data'=>$goods_info['goods_name']);
                $tmp[] = array('data'=>$goods_info['goods_price']);
                $tmp[] = array('data'=>$goods_info['goods_storage']);
                $tmp[] = array('data'=>$goods_info['goods_addtime'] ? date('Y-m-d H:i:s', $goods_info['goods_addtime']) : '数据异常');
                $tmp[] = array('data'=>$goods_info['goods_serials']);
                $excel_data[] = $tmp;
        }
        $excel_data = $excel_obj->charset($excel_data,CHARSET);
        $excel_obj->addArray($excel_data);
        $excel_obj->addWorksheet($excel_obj->charset('在线商品',CHARSET));
    ...
    public function edit_goodsOp() {
        ...
        $goods = Model()->table('goods')->where(array('goods_id'=>$goods_id))->field('goods_url,goods_key')->find();
        Tpl::output('goodss',$goods);
        ...
        $model_goods = Model('goods');
        //熊磊加入数据
        $goods_fo = $model_goods->getGoodsInfo(array('goods_id'=>$goods_id));
        $goods_max_money = $goods_fo['goods_max_money'];
        $goodscommon_info = $model_goods->getGoodsCommonInfoByID($common_id);
        if(!empty($goodscommon_info['dmbuy_know'])){   //是否有购买须知这一项，没有则不处理。
           $edit =unserialize($goodscommon_info['dmbuy_know']);
           $editall = explode('&', $edit);
           if($editall){
       ...
       if(!empty($goodscommon_info['dmbuy_rule'])){     //是否有购买规则这一项，没有则不处理。
        $editr = $goodscommon_info['dmbuy_rule'];
        ...
        TPL::output('txtName',$txtName);
        TPL::output('txtDanj',$txtDanj);
        TPL::output('txtSulian',$txtSulian);
        TPL::output('txtTel',$txtTel);
        //TPL::output('txtTRLastIndex',$txtTRLastIndex);
        TPL::output('txtTRLastIndex',count($txtName));
       
        
      }

    //*选取店铺信息，判断是不是地面商家 李志军*/
        if(!empty($goodscommon_info['gc_id_1'])){
             $yt_id = $goodscommon_info['gc_id_1'];    //获取第一级ID比较类名并分配地面模板 
          $ygoods = $model_goodsclass->where(array('gc_id'=>$yt_id))->find();
        if($ygoods){
            if($ygoods['gc_name']=='美食') {Tpl::output('gcm', 1);}
            if($ygoods['gc_name']=='丽人')  {Tpl::output('gcm', 2);}  
            if($ygoods['gc_name']=='酒店住宿')  {Tpl::output('gcm', 3);}     
            if($ygoods['gc_name']=='休闲娱乐')  {Tpl::output('gcm', 4);}  
            if($ygoods['gc_name']=='生活服务')  {Tpl::output('gcm', 5);}   
               
            }
        }
    
        $model_store = Model('store');
        $store_info = $model_store->getStoreInfoByID($goodscommon_info['store_id']);
        Tpl::output('store_info',$store_info);
        //熊磊加入数据
        if(empty($goodscommon_info['goods_max_money']) || !isset($goodscommon_info['goods_max_money'])){
            $goodscommon_info['goods_max_money'] = $goods_max_money;
        }
        
        if (empty($goodscommon_info) || $goodscommon_info['store_id'] != $_SESSION['store_id'] || $goodscommon_info['goods_lock'] == 1) {
            showMessage(L('wrong_argument'), '', 'html', 'error');
        }
        //20170711潘丙福添加开始-判断当前spu编辑次数
        if ($goodscommon_info['edit_counts'] > 2) {
            showMessage('每个商品每周限编辑2次！', '', 'html', 'error');
        }
        //20170711潘丙福添加结束
        ...
        if($store_info['store_flag']==1){ //地面商家与实体商家
        $goods_class = Model('goods_class')->getGoodsClassLineForTagg($goodscommon_info['gc_id']);}else{
    ...
    public function edit_classOp() {
        ...
        // 商品分类
        $model_store = Model('store');
        $is_instance = $model_store->getStoreInfoByID($_SESSION['store_id']);
        // 商品分类
        if($is_instance['store_flag']==1){
        $condition = array();
        $condition['store_flag']=1;
        $condition['gc_parent_id']=0;
        $goods_class = $model_goodsclass->where($condition)->select();   
        }else{
        ...
        Tpl::output('goods_class', $goods_class);
        Tpl::output('is_instance',$is_instance);
   ...
   * 设置商品佣金
    ...
    /**编辑实体店
       铺套餐分类a*/
    public function edit_a_packageOp(){
        $commonid = isset($_GET['commonid'])?$_GET['commonid']:'';
       if(!empty($commonid)){
         $tjcom = explode(',',$commonid);
         $update = array();
         $goods_id = array();
         $update['map_actives']=1;
         $model_goods = Model('goods');
         foreach ($tjcom as $k => $v) {
            $goods_id['goods_commonid']=$v;
            $zxcg = $model_goods->getGoodsInfo($goods_id);
            $goodsxg = $model_goods->editGoodsById($update, $zxcg['goods_id']);
           }
          $this->recordSellerLog('设置A套餐，SPU：'.$common_id);
         showDialog('设置A套餐成功！', 'reload', 'succ');
        }else{
         showDialog('设置A套餐失败！', '', 'error');
       }
    }
    /**编辑实体店铺
       套餐分类b*/
    public function edit_b_packageOp(){
        $commonid = isset($_GET['commonid'])?$_GET['commonid']:'';
        if(!empty($commonid)){
         $tjcom = explode(',',$commonid);
         $update = array();
         $goods_id = array();
         $update['map_actives']=2;
         $model_goods = Model('goods');
         foreach ($tjcom as $k => $v) {
            $goods_id['goods_commonid']=$v;
            $zxcg = $model_goods->getGoodsInfo($goods_id);
           ...
           $this->recordSellerLog('设置B套餐，SPU：'.$common_id);
          showDialog('设置B套餐成功！', 'reload', 'succ');
        }else{
          showDialog('设置B套餐失败！', '', 'error');
        }
     }
     /**
        取消套餐
     */
    public function cancel_packageOp(){
        $commonid = isset($_GET['commonid'])?$_GET['commonid']:'';
        if(!empty($commonid)){
         $tjcom = explode(',',$commonid);
         $update = array();
         $goods_id = array();
         $update['map_actives']=0;
         $model_goods = Model('goods');
         foreach ($tjcom as $k => $v) {
            $goods_id['goods_commonid']=$v;
            $zxcg = $model_goods->getGoodsInfo($goods_id);
           ...
           $this->recordSellerLog('取消套餐，SPU：'.$common_id);
            showDialog('取消套餐成功！', 'reload', 'succ');
        }else{
           showDialog('取消套餐失败！', '', 'error');  
        }
     }
}
