...
class store_billControl extends BaseSellerControl {
    ...
    public function store_cashOp(){

        $seller=Model('seller');
        $seller_info=$seller->where(array('store_id'=>$_SESSION['store_id']))->find();        
        Tpl::output('seller_info',$seller_info);
        Tpl::showpage('store_cash');
        
   }
   public function store_add_cashOp(){
        $seller=Model('seller');
        $chief=Model('chief');
        $model_pd = Model('predeposit');
        $member=Model('member');
        $seller_info=$seller->where(array('store_id'=>$_SESSION['store_id']))->find();        
        $fee=$chief->getby_id(19,'chief');         
        $pdc_amount=trim($_POST['pdc_amount']);   
        if(empty($_POST['pdc_amount'])){showMessage('请输入充值金额');exit;}
        if(!is_numeric($_POST['pdc_amount'])){showMessage('请输入正确充值金额');exit;}
        if($pdc_amount>$seller_info['wallet_release']){showMessage('提现金额不足，请重新输入！');exit;}
        $dats['wallet_release']=array('exp','wallet_release-'.$pdc_amount);            
        
        $member_info=$member->where(array('member_id'=>$_SESSION['member_id']))->find();
        if(empty($member_info['member_bankname']) || empty($member_info['member_bankcard'])){showMessage('请绑定农行卡！！！','member/index.php?act=member_redpacket&op=index');}
        ...
        $data['pdc_amount'] =$pdc_amount-$pdc_amount*$fee['chief'];
        $data['pdc_bank_name'] ='农业银行';
        $data['predeposit_type'] =4;//加入的。。。。。。
        ...
        $seller_update=$seller->where(array('store_id'=>$_SESSION['store_id']))->update($dats);
   ...
   public function store_rcbOp(){
        $rcb=Model('rcb_log');
        $where['member_id']=$_SESSION['store_id'];
        $where['type']='seller';
        $where['add_time']= array('gt', time()-60*60*24*30);
        $rcb_info=$rcb->where($where)->page(15)->order('add_time desc')->select();
        Tpl::output('show_page', $rcb->showpage());
        Tpl::output('rcb_info',$rcb_info);
        Tpl::showpage('store_rcb');
   }
    public function store_cash_infoOp(){
        $cash=Model('pd_cash');
        $where['predeposit_type']='4';
        $where['pdc_member_id']=$_SESSION['member_id'];
        $cash_info=$cash->where($where)->page(15)->order('pdc_add_time desc')->select();
       
        Tpl::output('show_page', $cash->showpage());
        Tpl::output('cash_info',$cash_info);
        Tpl::showpage('store_cash_info');
   }
}
