    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'info' => array('name' => '会员资料', 'child' => array(
                ...
                'member_teamindex'   => array('name' => '分销信息', 'url'=>urlMember('member', 'team')),
                 'member_snsindex'   => array('name' => '注册会员', 'url'=>urlMember('member', 'registerr')),
            )),
            'property' => array('name' => '财产中心', 'child' => array(
                ...
                // 'give_points'    => array('name' => '云豆互转', 'url'=>urlMember('predeposit', 'give_points')),
                // 'points'     => array('name' => '云豆兑换', 'url'=>urlMember('predeposit', 'rcb_log_list')),
                'member_redpacket'  => array('name' => '绑定银行卡', 'url'=>urlMember('member_redpacket', 'index'))
            )),
        );
        $member=Model('member');
        $member_info=$member->where(array('member_id'=>$_SESSION['member_id']))->find();
        if($member_info['member_level']=='2'){
            $menu_list['property']['child']['port_points']=array('name' => '云豆互转', 'url'=>urlMember('predeposit', 'port_points'));
        }
        return $menu_list;
    }
}
...
class BaseGoodsControl extends BaseStoreControl {
    ...
    protected function getStoreInfo($store_id, $goods_info = null) {
        ...
        //潘丙福20170609潘丙福添加获取申请开通店铺时填写的手机号
        if (empty($store_info['store_phone'])) {
            $panModel = Model();
            $panStoreJoinin = $panModel->table('store_joinin')->find($store_info['member_id']);
            $store_info['store_phone'] = $panStoreJoinin['contacts_phone'];
        }
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        ...
        if ($_GET['act'] !== 'seller_login') {
            ...
            //20170704潘丙福添加获取店铺的保证金
            //字段 member_id
            //表 store_joinin
            $panCondition = array();
            $panCondition['member_id'] = array('eq', $this->store_info['member_id']);
            $pansc_bail = Model()->table('store_joinin')->field('sc_bail')->where($panCondition)->find();
            if ($pansc_bail) {
                $this->store_info['sc_bail'] = $pansc_bail['sc_bail'];
            } else {
                $this->store_info['sc_bail'] = 0;
            }
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'promotion' => array('name' => '促销', 'child' => array(
                array('name' => '秒杀管理', 'act'=>'store_groupbuy', 'op'=>'index'),
            ...
            'statistics' => array('name' => '统计结算', 'child' => array(
                ...
                // array('name' => '商家提现', 'act'=>'store_bill', 'op'=>'store_cash'),
                // array('name' => '实物结算', 'act'=>'store_bill', 'op'=>'index'),
                // array('name' => '虚拟结算', 'act'=>'store_vr_bill', 'op'=>'index'),
            )),
            //20170622潘丙福添加开始-商家提现单独显示在菜单栏
            'panbingfu' => array('name' => '商家提现', 'child' => array(
                array('name' => '商家提现', 'act'=>'store_bill', 'op'=>'store_cash'),
            )),
            //20170622潘丙福添加结束
...
class BasePointShopControl extends Control {
    ...
    /**
     * 获得云豆中心会员信息包括会员名、ID、会员头像、会员等级、经验值、等级进度、云豆、已领代金券、已兑换礼品、礼品购物车
     */
