...
class store_orderControl extends BaseSellerControl {
    ...
    private function _order_shipping_code($order_info, $post) {
        ...
        if(!chksubmit()) {
            ...
            return $logic_order->changeOrderShippingCode($order_info,'seller',$_SESSION['member_name'],$post['shipping_code']); 
        }
    }
    ...
    public function orderall_exportOp()
    {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                $data = $model_order->getOrderList($condition,'','*', 'order_id desc',self::EXPORT_SIZE, array('order_goods','order_common'));
            ...
            if (! $limit1) {
                $data = $model_order->getOrderList($condition,'','*', 'order_id desc', "{$limit2}",array('order_goods','order_common'));
            }else {
                $data = $model_order->getOrderList($condition,'','*', 'order_id desc',"{$limit1},{$limit2}",array('order_goods','order_common'));                
            }
            $this->createExcel($data);
        }
    }
