...
class statistics_goodsControl extends BaseSellerControl {
    ...
    public function goodslistOp(){
        ...
        //20170810潘丙福添加开始--统计总销量去掉时间限制
        unset($where['order_add_time']);
        //获取goods_id数组
        $goodsIdArray = array();
        foreach ($stat_ordergoods as $k1 => $v1) {
            $goodsIdArray[] = $v1['goods_id'];
        }
        $where['goods_id'] = array('in', trim(implode(',',$goodsIdArray),','));

        $panfield = ' min(goods_id) as goods_id,SUM(goods_num) as ordergoodsnum ';
        $panStat_ordergoods = Model()->table('stat_ordergoods')->field($panfield)->where($where)->group('goods_id')->select();
        
        $newPanStat_ordergoods = array();

        foreach ($panStat_ordergoods as $k2 => $v2) {
            $newPanStat_ordergoods[$v2['goods_id']] = $v2;
        }
        foreach ($stat_ordergoods as $k3 => $v3) {
            $stat_ordergoods[$k3]['ordertotalgoodsnum'] = $newPanStat_ordergoods[$v3['goods_id']]['ordergoodsnum'];
        }
        //20170810潘丙福添加结束
    ...
    public function goodsall_exportOp()
    {
        ...
        $orderby = trim($this->search_arr['orderby']).',goods_id';
        $stat_ordergoods = $model->statByStatordergoods($where, $field, $countnum, 0, $this->search_arr['orderby'], 'goods_id');
        unset($where['order_add_time']);
        //获取goods_id数组
        $goodsIdArray = array();
        foreach ($stat_ordergoods as $k1 => $v1) {
        ...
        $this->createExcel($stat_ordergoods);   
    }
    //20170812潘丙福添加
    private function createExcel($data = array(), $supList = array())
    {
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'近30天下单商品数');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'近30天下单金额(元)');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'商品总销量');

        foreach ((array)$data as $k => $goods_info){

            $list['goods_name']         = $goods_info['goods_name'];
            $list['goods_price']        = $goods_info['goods_price'];
            $list['ordergoodsnum']      = $goods_info['ordergoodsnum'];
            $list['ordergamount']       = $goods_info['ordergamount'];
            $list['ordertotalgoodsnum'] = $goods_info['ordertotalgoodsnum'];

            $tmp          = array();
            $tmp[]        = array('data'=>$list['goods_name']);
            $tmp[]        = array('data'=>$list['goods_price']);
            $tmp[]        = array('data'=>$list['ordergoodsnum']);
            $tmp[]        = array('data'=>$list['ordergamount']);
            $tmp[]        = array('data'=>$list['ordertotalgoodsnum']);
            $excel_data[] = $tmp;
 
        }
        $excel_data = $excel_obj->charset($excel_data,CHARSET);
        $excel_obj->addArray($excel_data);
        $excel_obj->addWorksheet($excel_obj->charset('商家产品销量统计表',CHARSET));
        $excel_obj->generateXML('order-'.date('Y-m-d-H',time()));
