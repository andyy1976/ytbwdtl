...
class goodsControl extends BaseGoodsControl {
    ...
    public function indexOp() {
		...
		*/
		...
		$member_isactive=Model('member');
		$meber_info=$member_isactive->getMemberInfoByID($_SESSION['member_id']);
		...
		if($meber_info['member_level']>0 && $goods_info['gc_id']=='10351' && $goods_info['goods_id']!='30587'){  //如果是激活会员并且最一次购买银联宝，即提示用户不能最次购买，并删掉网站的购物车信息
		 
		     showMessage("亲，会员资格只能购买一次不能重复购买！");
        ...
        $goods_rand_list = $model_goods->getGoodsGcStoreRandList($goods_info['gc_id_1'], $goods_info['store_id'], $goods_info['goods_id'], $size, 'goods_commonid');
        ...
        //2017.8.4xin增加
        $xin_name = explode("#",$goods_info['goods_name']);
        $goods_info['goods_name']=$xin_name[0];
        //2017.8.4xin
        if(!empty($goods_info['goods_url'])){
            $a=$goods_info['goods_url'];
            $pos1 = strpos($a, '/id_')+4;
            $pos2 = strpos($a, '.html');
            $goods_info['goods_url'] = substr($a, $pos1, $pos2-$pos1);
        }// 现暂时改为从匿名“自营店铺专属等级”中判断
        if(!empty($goods_info['dmbuy_know'])){
         $model_goodsclass = Model('goods_class');  //根据判断
         $gc_find = $goods_info['gc_id_1'];
         if($gc_find==0){
            $gc_find = $goods_info['gc_id'];
         }
		 $ygoods = $model_goodsclass->where(array('gc_id'=> $gc_find))->find();
        if($ygoods){
            switch ($ygoods['gc_name']) {
                case '美食':
                   Tpl::output('gcm', 1);
                    break;
                case '丽人':
		...
		$g_info = unserialize($goods_info['dmbuy_know']);
        $p_info = @explode('&', $g_info);
        foreach ($p_info as $k => $v) {
          $z_info=@explode(':', $v);
          foreach ($z_info as $key => $value) {

              Tpl::output($z_info[0],$z_info[1]);
          }
            
        }
    }
           if(!empty($goods_info['dmbuy_rule'])){     //是否有购买规则这一项，没有则不处理。
        $editr = $goods_info['dmbuy_rule'];
		...
		if(!empty($goods_info['store_id'])){    //显示地面商家地址
         $bs = Model()->table('store_map')->where(array('store_id'=>$goods_info['store_id']))->find();
         if($bs){
            TPL::output('store_address',$bs['address_info']);
         }else{
            TPL::output('store_address','商家未添加,请联系网站客服');
         }
        }
        ...
        // 当前位置导航
        if($goods_info['isdmgoods']==1){ //地面商家导航  最多三级
        $nav_link_list = Model('goods_class')->getGoodsClassNav($goods_info['gc_id'], 3);
        }else{
       $nav_link_list = Model('goods_class')->getGoodsClassNav($goods_info['gc_id'], 0);  
        ...
        if(!empty($goods_info['goods_key'])){
            $seo_keywords = $goods_info['goods_key'];
        }else{
            $seo_keywords = $goods_info['goods_name'];
        }
    ...
    public function salelogOp() {
        ...
        //20170416潘丙福添加循环匹配出商品的市场价格
        $panSaleGoods = Model('goods');
        foreach ($sales as $panSaleKey => $panSaleValue) {
            $panSaleGoodsPriceArray = $panSaleGoods->field('goods_marketprice')->find($panSaleValue['goods_id']);
            $sales[$panSaleKey]['goods_marketprice'] = $panSaleGoodsPriceArray['goods_marketprice'];
        }
    ...
    public function show_chainOp() {
      ...
      // 云豆购买  7月14号
    public function predeposit_pointsOp(){
        ...
        if($type=='1' && ($recharge_count+$money*20)>1000000){
            showMessage('每日限100万！！！');
        }
        if($type=='2' && ($recharge_count+$money*12.5)>1000000){
            showMessage('每日限100万！！！');
        }
