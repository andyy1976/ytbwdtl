...
class buytestControl extends BaseBuyControl {
    ...
    public function payOp() {
        ...
        if($member_fo['gc_id'] == 10351 && $mem['member_level'] >0 && $member_fo['goods_id']!='30587'){
            showDialog('亲,您已经购买过会员资格了请勿再次购买会员资格只能购买一次');
        }
    ...
    public function pd_payOp() {
       ...
       if($type=='1' && ($recharge_count+$money*20)>1000000 && $_SESSION['member_id']!='146986'){
        ...
        $model_payment = Model('payment');
        $condition = array();
        $condition['payment_code'] = array('not in', array(
            'wxpay','offline','predeposit',
            'chinabank','tenpay','alipay',
            'tlzf', 'ybzf'
        ));
