    ...
    public function choosed_goodsOp() {
        ...
        if (chksubmit()) {
            ...
            if ($_POST['type'] == 'book') {
                ...
                //20170818潘丙福添加开始--增加定金云豆、尾款云豆
                $update['book_down_payment_points']  = intval($_POST['down_payment_points']);
                $update['book_final_payment_points'] = intval($_POST['final_payment_points']);
                //20170818潘丙福添加结束
                ...
                //20170820潘丙福添加开始-全款预售商品价格、云豆
                $update['goods_price']              = floatval($_POST['total_payment']);
                $update['goods_points']             = intval($_POST['down_payment_points']);
                $presellGoods = Model()->table('goods')->field('goods_price,goods_points')->find($gid);
                $update['old_presell_goods_price']  = $presellGoods['goods_price'];
                $update['old_presell_goods_points'] = $presellGoods['goods_points'];
                //20170820潘丙福添加结束
