...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        //20170303潘丙福修改添加时间过滤48小时
        if ($_GET['state_type'] == 'state_new') {
            $condition['chain_code'] = 0;
            $condition['add_time'] = array('egt',time()-60*60*48);
        }
        ...
        $member_info=$model_member->where(array('member_id'=>$_SESSION['member_id']))->find();
    ...
    public function change_stateOp() {
       ...
       if($order_info['order_state']=='40'){
            showDialog('该订单已确认收货','','error','',5);
            exit;
        }
       
        $goods = $model_order->getOrderGoodsInfo(array('order_id'=>$_GET['order_id']));//加入的。。。。。。。。。。。。
        $goods_id = $member_goods->getGoodsInfo(array('goods_id'=>$goods['goods_id']));//加入的。。。。。。。。。。。。
   ...
   if(dm_orderupdateOp($order_id)){
            echo '<script>window.location="index.php?act=member_order"</script>';
        }else{
            echo "<script>window.alert('操作不成功！');</script>";
