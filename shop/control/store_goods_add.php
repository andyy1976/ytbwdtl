...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function add_step_oneOp() {
        ...
        $is_instance = $model_store->getStoreInfoByID($_SESSION['store_id']);  //判断是否是实体商家
        // 商品分类
        if($is_instance['store_flag']==1){  //如果是地面商家
           $condition = array();
    ...
    public function add_step_twoOp() {
         ...
         //*选取店铺信息，判断是不是地面商家 作者：李志军 日期：2017-07-31*/
        $model_store = Model('store');
        ...
        // 更新常用分类信息
        if($store_info['store_flag']==1){
        $yt_id = isset($_GET['yt_id'])?$_GET['yt_id']:'';    //获取第一级ID比较类名并分配地面模板 
        ...
        $goods_class = $model_goodsclass->getGoodsClassLineForTagg($gc_id);
        }else{
    ...
    public function ajax_goods_classOp() {
        ...
        $model_goodsclass = Model('goods_class');
        if($flag==1){  //地面商家
            $list = $model_goodsclass->where(array('gc_parent_id'=>$gc_id))->select(); 
