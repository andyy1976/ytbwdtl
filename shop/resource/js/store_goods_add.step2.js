$(function(){
    $('body').on('hover','.panceil',function(){$(this).find('.pandelete').toggle();});
    $('body').on('click','.pandelete',function(){$(this).parent().remove();});
    ...
    $('a[nctype="specAddSubmit"]').click(function(){
        ...
        $.getJSON(data_str.url, {gc_id : data_str.gc_id , sp_id : data_str.sp_id , name : _input.val()}, function(data){
            if (data.done) {    
                _parent.before('<li class="panceil"><span nctype="input_checkbox"><input type="checkbox" name="sp_val[' + data_str.sp_id + '][' + data.value_id + ']" nc_type="' + data.value_id + '" value="' +_input.val()+ '" /></span><span nctype="pv_name">' + _input.val() + '</span><span class="pandelete"></span></li>');
                _input.val('');
            }
        });
    });
