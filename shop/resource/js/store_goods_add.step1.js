...
function selClass($this){
	...
	$('#flag').val(data_str.flag);
    $('#dataLoading').show();
    var deep = parseInt(data_str.deep) + 1;
    if(data_str.deep==1){
		$('#yt_id').val(data_str.gcid);
		}
    $.getJSON('index.php?act=store_goods_add&op=ajax_goods_class', {gc_id : data_str.gcid, deep: deep,flag:data_str.flag}, function(data) {
        if (data != null) {
            ...
            $.each(data, function(i, n){
                $('#class_div_' + deep).children('ul').append('<li data-param="{gcid:'
                        + n.gc_id +',deep:'+ deep +',tid:'+ n.type_id +',flag:'+data_str.flag+'}"><a class="" href="javascript:void(0)"><i class="icon-double-angle-right"></i>'
                        + n.gc_name + '</a></li>')
