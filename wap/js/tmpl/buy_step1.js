...
$(function() {
            ...
            success:function(result){
               ...
               if ($.isEmptyObject(result.datas.address_info)) {
                ...
                for (var k in result.datas.store_final_total_list) {
                    // 总价
                    // 20170828潘丙福修改
                    if (result.datas.isBookGoods) {
                        $('#storeTotal' + k).html(parseFloat(result.datas.store_cart_list[k]['goods_list'][0]['book_down_payment']*result.datas.store_cart_list[k]['goods_list'][0]['goods_num']).toFixed(2));
                        totals += parseFloat(result.datas.store_cart_list[k]['goods_list'][0]['book_down_payment']*result.datas.store_cart_list[k]['goods_list'][0]['goods_num']); 
                    } else {
    ...
    var insertHtmlAddress = function (address_info, address_api) {
        ...
        } else if($.trim($('.panidentity').val()).length == 0 && $('.panidentity').length > 0) {
            $('#ToBuyStep2').parent().removeClass('ok');
        } else {
    ...
    $('#ToBuyStep2').click(function(){
        if (!$(this).parent('.check-out').hasClass('ok')) {
            return false;
        }
		var flag  = $('#flag').val();
        var buyer_cardid = $('#buyer_cardid').val();
        var book_pay_type = $('#book_pay_type').val();
        var msg = '';
...
//20170330潘丙福添加判断是否填写了正确的身份证号
function isCardNo(card) {  
   var panpattern = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
   return panpattern.test(card);
}
function panisVard(){
    if($.trim($('.panidentity').val()).length == 0) {

        $('.panidentity').attr('placeholder', '身份证号码没有输入');
        $('#ToBuyStep2').parent().removeClass('ok');

    } else if (!isCardNo($.trim($('.panidentity').val()))) {

        $('.panidentity').val('');
        $('.panidentity').attr('placeholder', '身份证号码格式不正确');
        $('#ToBuyStep2').parent().removeClass('ok');
