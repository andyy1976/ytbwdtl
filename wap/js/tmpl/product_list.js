...
$(function(){
    $.getJSON(ApiUrl + '/index.php?act=goods_class&op=goods_class_san', {gc_id:gc_id}, function(result){
        //var data = result.datas;
        /*var src = "<ul>"
                    for(var i=0;i<data.length;i++){
                        src+= "<li><a href='product_list.html?gc_id="+data[i]['gc_id']+"'>"+data[i]['gc_name']+"</a></li>";
                                                  
                    }
                    src+="</ul>";*/
                    var src = template.render('three_lei', result.datas);
                    $("#panpan").append(src);
...
function get_list() {
    ...
    $.getJSON(ApiUrl + '/index.php?act=goods&op=goods_list' + window.location.search.replace('?','&'), param, function(result){
        ...
        var gc_id=result.datas.gc_id;
        var pic=result.datas.cn_pic;
        var gc_name=result.datas.gc_name;
        if(pic==null){
            result.datas.cn_pic='xinpin.jpg';
        }
        if(gc_id=='10351'){
            $('.title').html('会员专享');
        }else{
