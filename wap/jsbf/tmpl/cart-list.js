    ...
    if(!key){
        ...
        if(goodsarr.length>0){
            for(var i=0;i<goodsarr.length;i++){
                ...
                sum_points += parseFloat(data.goods_sum_points);
            }
        }
        var rData = {cart_list:cart_list, sum:sum.toFixed(2), sum_points:sum_points,cart_count:goodsarr.length, check_out:false};
        ...
        function editQuantity(self,type){
            ...
            var goodsPoints = sPrents.find(".goods-points");//加入的。。。。。。。。。
                ...
                success:function (res){
                    if(checkLogin(res.login)){
                        if(!res.datas.error){
                            ...
                            goodsPoints.html('<em>' + res.datas.goods_points + '</em>');//加入的。。。。。。。。。
                            calculateTotalPrice();
                        }else{
                            $.sDialog({
                                skin:"red",
...
function calculateTotalPrice() {
    ...
    $('.cart-litemw-cnt').each(function(){
        if ($(this).find('input[name="cart_id"]').prop('checked')) {
            ...
            totalPoints += parseFloat($(this).find('.goods-points').find('em').html()) * parseInt($(this).find('.value-box').find('input').val());//加入的没用。。。。
        }
    });
    $(".total-money").find('em').html(totalPrice.toFixed(2));   
    $(".total-points").find('em').html(totalPoints);//加入的没用。。。。
    check_button();
    return true;
}
...
function getGoods(goods_id, goods_num){
    ...
    $.ajax({
        ...
        success:function(result){
            ...
            data.goods_points = result.datas.goods_info.goods_points;
            data.goods_num = goods_num;
            data.goods_image_url = pic[0];
            data.goods_sum = (parseInt(goods_num)*parseFloat(result.datas.goods_info.goods_price)).toFixed(2);
            data.goods_sum_points= parseInt(goods_num) * parseInt(result.datas.goods_info.goods_points);
        }
    });
    return data;
}
