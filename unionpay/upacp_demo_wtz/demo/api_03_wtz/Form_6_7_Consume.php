...
include_once '/home/wwwroot/lnmp01/domain/wdtl.com/web/unionpay' . '/upacp_demo_wtz/sdk/acp_service.php';
 ...
 * 版本： 1.0.0
 * 版权： 中国银联<br>
 * 说明：以下代码只是为了方便商户测试而提供的样例代码，
 * 商户可以根据自己需要，按照技术文档编写。该代码仅供参考，
 * 不提供编码性能规范性等方面的保障<br>
 * 交易说明:
 * 
 * 1）确定交易成功机制：
 * 商户需开发后台通知接口或交易状态查询接口（Form03_6_5_Query）确定交易是否成功，
 * 建议发起查询交易的机制：可查询N次（不超过6次），每次时间间隔2N秒发起,
 * 即间隔1，2，4，8，16，32S查询（查询到03，04，05继续查询，否则终止查询）
 *       
...
$params = array(
		...
		'txnType'       => '01',		           //交易类型
		'txnSubType'    => '01',		           //交易子类
		'bizType'       => '000301',		       //业务类型 000301 认证支付 2.0
		...
		'orderId' => $_POST["orderId"],//$_POST["orderId"],	//商户订单号，如上送短信验证码，请填写获取验证码时一样的orderId，此处默认取demo演示页面传递的参数
		'txnTime' => $_POST["orderId"],//$_POST["txnTime"],	//订单发送时间，如上送短信验证码，请填写获取验证码时一样的txnTime，此处默认取demo演示页面传递的参数
